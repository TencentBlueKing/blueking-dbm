...
<template>
  <div class="machine-selector-preview-result">
    <div class="header">
      <span>{{ t('结果预览') }}</span>
      ...
      <template
        ...
        <CollapseMini
          ...
          <div
            ...
            <span
              v-overflow-tips
              v-test="{ type: 'span', value: 'hostSelectorPreviewItem' }"
              class="text-overflow">
              {{ item.ip }}
            </span>
            <DbIcon
...
<script setup lang="ts" generic="T extends ISupportClusterType">
  ...
  export interface Props<C extends ISupportClusterType> {
    clusterTypes: C[];
    lastValues: { [key in C]: MachineModel<C>[] };
  }
  ...
  const isEmpty = computed(() =>
    Object.values<MachineModel<T>[]>(props.lastValues).every((values) => values.length === 0),
  );
  ...
  const handleClear = () => {
    ...
    emits(
      'change',
      Object.fromEntries(props.clusterTypes.map((key) => [key, [] as MachineModel<T>[]])) as Props<T>['lastValues'],
    );
  };
  ...
  const handleCopyInstances = () => {
    if (isEmpty.value) {
      messageWarn(t('没有可复制主机'));
      return;
    }

    const copyData = Object.values<MachineModel<T>[]>(props.lastValues).flatMap((lastValuesItem) =>
      lastValuesItem.map((item) => item.ip),
    );
    execCopy(copyData.join('\n'), t('复制成功，共n条', { n: copyData.length }));
  };
</script>
<style lang="less">
  .machine-selector-preview-result {
    display: flex;
    height: 100%;
    max-height: 625px;
    padding: 12px 24px;
