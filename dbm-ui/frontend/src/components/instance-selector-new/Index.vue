...
<script setup lang="ts" generic="T extends ISupportClusterType">
  ...
  export interface Props<C extends ISupportClusterType> {
    ...
    sourceDataParams?: { [key in C]?: Record<string, any> };
    uniquePanelSettings?: {
      enable: boolean;
      tip?: string;
    };
  ...
  const currentPanelTab = ref(props.clusterTypes[0] as ISupportClusterType);
  const lastValues = ref<{ [key in ISupportClusterType]?: InstanceModel[] }>({});

  const currentTableData = computed(() => lastValues.value[currentPanelTab.value] || []);
  const isEmpty = computed(() => Object.values(lastValues.value).every((values) => values.length === 0));
