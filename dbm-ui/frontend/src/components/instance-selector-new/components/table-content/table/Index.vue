...
<script setup lang="tsx" generic="T extends ResourceInstance">
  ...
  const props = withDefaults(defineProps<Props>(), {
    ...
    role: '',
    isManul: false,
    getTableList: () => ({} as unknown as (params: Record<string, any>) => ListBase<T[]>),
  });
  ...
  const renderManualData = computed(() => {
    if (searchValue.value === '') return props.manualTableData;
    return props.manualTableData.filter(item => (
      item.instance_address.includes(searchValue.value)
    ));
  });
  ...
  const triggerChange = () => {
    if (props.isManul) {
      ...
      for (const item of Object.values(checkedMap.value)) {
        lastValues.tendbcluster.push(item);
      }
