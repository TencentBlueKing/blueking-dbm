...
<script setup lang="ts">
  ...
  interface Props {
    lastValues: { [key in ISupportClusterType]?: InstanceModel[] };
  }
  ...
  const isEmpty = computed(() => keys.value.every((key) => props.lastValues[key]!.length === 0));
  const showTitle = computed(() => keys.value.length > 1);

  const handleClear = () => {
    if (isEmpty.value) {
      return;
    }
    emits('change', Object.fromEntries(keys.value.map((key: ISupportClusterType) => [key, [] as InstanceModel[]])));
  };
  ...
  const handleCopyInstances = () => {
    ...
    const copyData = Object.values(props.lastValues).flatMap((lastValuesItem) =>
