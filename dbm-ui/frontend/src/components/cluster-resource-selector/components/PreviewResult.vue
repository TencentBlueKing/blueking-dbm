          ...
          <div
            ...
            <div class="result-item-operations">
              <DbIcon
                ...
                @click="execCopy(item.master_domain)" />
              <DbIcon
...
<script setup lang="ts">
  ...
  const handleRemove = (item: IValue) => {
    selected.value = selected.value.filter((cur) => cur.master_domain !== item.master_domain);
  };

  const handleCopy = () => {
    if (isEmpty.value) {
      messageWarn(t('没有可复制域名'));
      return;
    }

    const copyData = selected.value.map((item) => item.master_domain);
