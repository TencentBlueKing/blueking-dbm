...
<template>
  <div class="cluster-selector-panel-tab">
    <div
      v-for="item in panelList"
      :key="item.id"
      v-bk-tooltips="{
        content: tipContent,
        disabled: !disabled || modelValue === item.id,
        theme: 'light',
      }"
      class="tab-item"
      :class="{
...
<script setup lang="ts">
  ...
  interface Props {
    ...
    isEmpty: boolean;
    nameMap?: {
      [key in ISupportClusterType]?: string;
    };
    uniquePanelSettings?: {
      enable: boolean;
      tip?: string;
  ...
  const panelList = props.clusterTypes.map((clusterType) => ({
    id: clusterType,
    name: props.nameMap?.[clusterType] || tabListMap[clusterType],
  }));

  const disabled = computed(() => !props.isEmpty && Boolean(props.uniquePanelSettings?.enable));
  const tipContent = computed(() => props.uniquePanelSettings?.tip || t('仅可选择一种集群类型'));
