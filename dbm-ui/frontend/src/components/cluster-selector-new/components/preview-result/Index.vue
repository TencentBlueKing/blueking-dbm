...
<template>
  <div class="cluster-selector-preview-result">
    <div class="header">
      <span>{{ t('结果预览') }}</span>
    ...
    <div
      ...
      <template
        ...
        <CollapseMini
          ...
          :title="nameMap?.[key] || tabListMap[key]">
          <div
...
<script setup lang="ts" generic="T extends ISupportClusterType">
  ...
  export interface Props<C extends ISupportClusterType> {
    clusterTypes: C[];
    lastValues: { [key in C]: ClusterModel<C>[] };
    nameMap?: {
      [key in C]?: string;
    };
  }
  ...
  const isEmpty = computed(() =>
    Object.values<ClusterModel<T>[]>(props.lastValues).every((values) => values.length === 0),
  );
  ...
  const handleClear = () => {
    ...
    emits(
      'change',
      Object.fromEntries(props.clusterTypes.map((key) => [key, [] as ClusterModel<T>[]])) as Props<T>['lastValues'],
    );
  };
  ...
  const handleCopyCluster = () => {
    ...
    const copyData = Object.values<ClusterModel<T>[]>(props.lastValues).flatMap((lastValuesItem) =>
      lastValuesItem.map((item) => item.master_domain),
    );
    execCopy(copyData.join('\n'), t('复制成功，共n条', { n: copyData.length }));
  };
</script>
<style lang="less">
  .cluster-selector-preview-result {
    display: flex;
    height: 100%;
    max-height: 625px;
    padding: 12px 24px;
