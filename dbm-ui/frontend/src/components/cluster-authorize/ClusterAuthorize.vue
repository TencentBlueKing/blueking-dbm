...
<script lang="tsx">
  ...
  import ClusterSelectorNew from '@components/cluster-selector-new/Index.vue';
...
<script setup lang="tsx">
  ...
  const selectedList = shallowRef<MongoPermissonAccountModel[]>([]);
  ...
  const ticketTypeMap = {
    ...
    [AccountTypes.MONGODB]: TicketTypes.TENDBCLUSTER_AUTHORIZE_RULES,
  };
  ...
  const handleAccountRulesChange = (value: MongoPermissonAccountModel[]) => {
    selectedList.value = value;
  };

  const handleRowDelete = (value: MongoPermissonAccountModel[]) => {
    selectedList.value = value;
  };
