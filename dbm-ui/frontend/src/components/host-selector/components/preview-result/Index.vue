      ...
      <BkDropdown
        ...
        <template #content>
          <BkDropdownMenu>
            ...
            <BkDropdownItem @click="handleCopyHosts">
              {{ t('复制所有 IP') }}
            </BkDropdownItem>
          </BkDropdownMenu>
        </template>
...
<script setup lang="ts" generic="T extends ISupportHostType">
  ...
  import type { HostModel, ISupportHostType } from '../../types';
  ...
  export interface Props<C extends ISupportHostType> {
    clusterTypes: C[];
    lastValues: { [key in C]: HostModel<C>[] };
  }
  ...
  const isEmpty = computed(() =>
    Object.values<HostModel<T>[]>(props.lastValues).every((values) => values.length === 0),
  );
  ...
  const handleClear = () => {
    ...
    emits(
      'change',
      Object.fromEntries(props.clusterTypes.map((key) => [key, [] as HostModel<T>[]])) as Props<T>['lastValues'],
    );
  };
  ...
  const handleCopyHosts = () => {
    ...
    const copyData = Object.values<HostModel<T>[]>(props.lastValues).flatMap((lastValuesItem) =>
