...
export function useBizDbDisplay() {
  ...
  const isLoading = computed(() => countLoading.value);

  const { data: clusterInstanceCount, loading: countLoading } = useRequest(queryClusterInstanceCount, {
    defaultParams: [
      {
  ...
  const tabList = computed(() => {
    ...
    //   const topList = topDbTypes.reduce<InfoItem[]>((prevList, topItem) => {
