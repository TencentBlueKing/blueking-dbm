...
export const useColumnFilter = (pool: ComputedRef<ServiceParameters<typeof queryDirtyMachineAttrs>['pool']>) => {
  const baseFilter = {
    ips: {
  ...
  const { run: runQueryDirtyMachineAttrs } = useRequest(queryDirtyMachineAttrs, {
    manual: true,
    onSuccess(result) {
