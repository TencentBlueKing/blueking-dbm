...
<template>
  <div class="hdfs-cluster-shrink-node-box">
    <div class="header">
      {{ data.label }}
    </div>
    <BkAlert
...
<script setup lang="tsx">
  ...
  const nodeTableData = shallowRef<TNodeInfo['nodeList']>(props.data.nodeList || []);
  ...
  const tableColumns = [
    {
      label: t('节点 IP'),
      field: 'ip',
      render: ({ data }: {data:TNodeInfo['nodeList'][0]}) => data.ip || '--',
    },
    {
      label: t('Agent状态'),
      field: 'alive',
  ...
  watch(localTargetDisk, () => {
    ...
    let calcDisk = 0;
    const nodeList: TNodeInfo['nodeList'] = [];
  ...
  // 添加节点
  const handleSelectChange = (nodeList: TNodeInfo['nodeList']) => {
    nodeTableData.value = nodeList;
    emits('change', nodeList);
  };

  // 删除选择的节点
  const handleRemoveHost = (data: TNodeInfo['nodeList'][0]) => {
    const nodeList = nodeTableData.value.reduce((result, item) => {
      if (item.bk_host_id !== data.bk_host_id) {
        result.push(item);
      }
      return result;
    }, [] as TNodeInfo['nodeList']);
