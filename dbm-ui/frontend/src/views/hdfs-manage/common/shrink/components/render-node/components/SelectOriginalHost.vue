...
<script setup lang="tsx">
  ...
  interface Props {
    modelValue: TNodeInfo['nodeList'],
    isShow: boolean,
    originalNodeList: TNodeInfo['nodeList'],
    targetDisk: number,
  }

  interface Emits {
    (e: 'change', value: TNodeInfo['nodeList']): void;
  ...
  const checkedNodeMap = shallowRef<Record<number, TNodeInfo['nodeList'][0]>>({});
  const selectNodeDiskTotal = computed(() => Object.values(checkedNodeMap.value)
    .reduce((result, item) => result + item.disk, 0));

  const checkNodeDisable = (node: TNodeInfo['nodeList'][0]) => {
    const options = {
      disabled: false,
      tooltips: {
        disabled: true,
  ...
  const tableColumns = [
    {
      ...
      render: ({ data }: {data: TNodeInfo['nodeList'][0]}) => {
    ...
    },
    {
      label: 'CPU',
      field: 'cpu',
      render: ({ data }: {data:TNodeInfo['nodeList'][0]}) => (data.cpu ? `${data.cpu} ${t('核')}` : '--'),
    },
    {
      label: t('内存_MB'),
      field: 'mem',
      render: ({ data }: {data:TNodeInfo['nodeList'][0]}) => data.mem || '--',
    },
    {
      label: t('磁盘_GB'),
      field: 'disk',
      render: ({ data }: {data:TNodeInfo['nodeList'][0]}) => data.disk || '--',
    },
  ];
  ...
  const handleSelectAll = (checked: boolean) => {
    const checkedMap = {} as Record<number, TNodeInfo['nodeList'][0]>;
    if (checked) {
  ...
  // 选中节点
  const handleSelect = (checked: boolean, data: TNodeInfo['nodeList'][0]) => {
