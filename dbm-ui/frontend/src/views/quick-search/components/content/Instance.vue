...
<script setup lang="tsx">
  ...
  const handleExport = (clusterType: string, dataList: QuickSearchInstanceModel[]) => {
    const formatData = dataList.map((dataItem) =>
      Object.fromEntries(
        [
          { label: 'IP', value: dataItem.ip },
          { label: t('IP端口'), value: String(dataItem.port) },
          { label: t('实例角色'), value: dataItem.role },
          { label: t('城市'), value: dataItem.bk_idc_area },
          { label: t('机房'), value: dataItem.bk_idc_name },
          { label: t('集群ID'), value: dataItem.cluster_id },
          { label: t('集群类型'), value: dataItem.cluster_type },
          { label: t('主域名'), value: dataItem.cluster_domain },
          { label: t('主版本'), value: dataItem.major_version },
          { label: t('业务ID'), value: String(dataItem.bk_biz_id) },
