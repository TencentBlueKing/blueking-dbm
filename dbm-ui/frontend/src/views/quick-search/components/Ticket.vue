...
<script setup lang="tsx">
  ...
  const columns = computed(() => [
    ...
    },
    {
      ...
      render: ({ data }: Column) => data.ticket_type_display || '--',
    },
    {
      label: t('单据状态'),
      field: 'status',
      render: ({ data }: Column) => (
        <DbStatus
          type="linear"
          theme={data.getTagTheme()}>
          {t(data.getStatusText())}
        </DbStatus>
      ),
    },
    {
      ...
      render: ({ data }: Column) => renderBizNameMap.value[data.bk_biz_id] || '--',
    },
    // {
    //   label: t('耗时'),
    //   field: 'bk_idc_name',
    //   render: ({ data }: Column) => data.bk_idc_name || '--',
    // },
    {
      label: t('申请人'),
      field: 'creator',
      render: ({ data }: Column) => data.creator || '--',
    },
    {
      label: t('申请时间'),
      field: 'create_at',
      render: ({ data }: Column) => data.create_at || '--',
    },
  ]);
