...
<template>
  <div class="password-instance-selector-render-manual-host">
    <BkInput
      v-model.trim="search"
      clearable
      :placeholder="t('请输入实例')" />
...
<script setup lang="tsx">
  ...
  const triggerChange = () => {
    ...
    for (const item of Object.values(checkedMap.value)) {
      lastValues[item.cluster_type as ClusterTypeOpts].push(item);
    }
  ...
  const handleRowClick = (data: InstanceInfos) => {
...
<style lang="less">
  .password-instance-selector-render-manual-host {
    padding: 0 24px;
