...
<template>
  <div class="influxdb-replace-render-host-list">
          ...
          <td>
            <div class="original-ip-box">
              <div
                ...
                class="ip-tag">
                <span>{{ nodeItem.instance_address }}</span>
                <DbIcon
                  class="remove-btn"
                  type="close"
...
<script setup lang="tsx">
  ...
  const nodeDiskTotal = computed(() => props.nodeList
    .reduce((result, item) => result + item.bk_disk, 0));
    ...
    getValue() {
      return {
        old_nodes: props.nodeList.map((nodeItem) => {
          const [ip, port] = nodeItem.instance_address.split(':');
          return {
...
<style lang="less" scoped>
  .influxdb-replace-render-host-list {
    position: relative;
