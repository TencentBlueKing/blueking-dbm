...
<template>
  <div class="expansion-node-status-list">
    <div
      ...
      <div class="node-item-name">
        {{ getDisplayName(nodeItem) }}
      </div>
      <template v-if="validateStatusMemo[nodeItem]">
        <div
          v-if="nodeInfo[nodeItem].expansionDisk"
          class="disk-tips">
          <span class="number">{{ nodeInfo[nodeItem].expansionDisk }}</span>
          <span>G</span>
        </div>
        <div
          v-else
...
<script setup lang="ts">
  ...
  const validateStatusMemo = reactive(nodeList.reduce((result, item) => ({
    ...result,
    [item]: false,
