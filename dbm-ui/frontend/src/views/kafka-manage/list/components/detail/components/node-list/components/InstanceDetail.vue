...
<script setup lang="tsx">
  ...
  const formatRequestData = (data: Array<KafkaInstanceModel>) => data.map((item) => {
  ...
  const tableData = shallowRef<Array<KafkaInstanceModel>>([]);
  const batchSelectNodeMap = shallowRef<Record<number, KafkaInstanceModel>>({});
  ...
  const columns = [
    ...
    {
      label: t('上次重启时间'),
      render: ({ data }: {data:KafkaInstanceModel}) => data.restart_at || '--',
    },
    {
      label: t('操作'),
      width: 116,
  ...
  // 选择单台
  const handleSelect = (data: { checked: boolean, row: KafkaInstanceModel }) => {
  ...
  // 重试单台
  const handleRestartOnde = (data: KafkaInstanceModel) => {
