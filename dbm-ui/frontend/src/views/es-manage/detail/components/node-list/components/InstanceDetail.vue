...
<script setup lang="tsx">
  ...
  const formatRequestData = (data: Array<EsInstanceModel>) => data.map((item) => {
  ...
  const tableData = shallowRef<Array<EsInstanceModel>>([]);
  const batchSelectNodeMap = shallowRef<Record<number, EsInstanceModel>>({});
  ...
  const columns = [
    ...
    },
    {
      ...
      render: ({ data }: {data:EsInstanceModel}) => (
        <div>
    ...
    {
      label: t('上次重启时间'),
      render: ({ data }: {data:EsInstanceModel}) => data.restart_at || '--',
    },
    {
      label: t('操作'),
      width: 116,
  ...
  // 选择单台
  const handleSelect = (data: { checked: boolean, row: EsInstanceModel }) => {
  ...
  // 重试单台
  const handleRestartOnde = (data: EsInstanceModel) => {
