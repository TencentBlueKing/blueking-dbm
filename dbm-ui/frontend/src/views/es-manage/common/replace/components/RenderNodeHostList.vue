...
<template>
  <div class="es-replace-render-host-list">
    <DbOriginalTable
      class="mt16"
      :columns="columns"
      :data="localList" />
...
<script setup lang="tsx">
  ...
  import EditNodeInstance from '@components/cluster-common/big-data-host-table/es-host-table/components/EditHostNode.vue';
  import IpSelector from '@components/ip-selector/IpSelector.vue';

  interface ILocalNode {
    old_ip: string,
    old_instance_num: number,
  ...
  interface Props {
    data: Array<EsNodeModel>,
    instnaceNumberEditable?: boolean
  }
    ...
    props.instnaceNumberEditable && {
      ...
      render: ({ data }: {data:ILocalNode}) => (
        <EditNodeInstance
          ...
          onChange={value => handleNodeInstanceNumChange(value, data)} />
      ),
    },
    {
      label: t('被替换的节点IP'),
  ...
  ].filter(_ => _) as InstanceType<typeof Table>['$props']['columns'];
  ...
  const handleIpListChange = (hostList: Array<HostDetails>) => {
    ...
    localList.value = hostList.reduce((result, item, index) => {
      ...
      result.push({
        ...localNode,
        ip: item.ip,
        instance_num: localNode.instance_num,
        bk_host_id: item.host_id,
        bk_cloud_id: item.cloud_id,
        machine_type: localNode.machine_type,
      });
      return result;
    }, [] as Array<ILocalNode>);
  };
  ...
  // 编辑节点实例
  const handleNodeInstanceNumChange = (value: number, nodeData: ILocalNode) => {
    window.changeConfirm = true;
    const list = [...localList.value];
    list.forEach((hostDataItem) => {
      if (hostDataItem.bk_host_id === nodeData.bk_host_id) {
