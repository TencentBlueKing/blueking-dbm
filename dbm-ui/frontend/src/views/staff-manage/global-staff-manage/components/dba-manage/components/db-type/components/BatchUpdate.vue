<template>
  <BkSideslider
    ...
    @hidden="handleHidden">
    <template #header>
      <div class="biz-unmanage-batch-update-header">
        <div>{{ t('批量配置业务 DBA') }}</div>
        <div class="biz-bar ml-8"></div>
        <div class="biz-count ml-8">{{ t('已选 n 个业务', { n: selected.length }) }}</div>
      </div>
    </template>
    <template #default>
      <div class="biz-unmanage-batch-update-content">
        <DbForm
          ref="form"
          form-type="vertical"
          :model="formData">
          <div class="dba-box">
            ...
            <DbFormItem
              ...
              required>
              <MemberSelector
                v-model="formData.standbyDBA"
                :multiple="false" />
            </DbFormItem>
          </div>
          <DbFormItem :label="t('二线 DBA')">
            <MemberSelector v-model="formData.level2DBA" />
          </DbFormItem>
        </DbForm>
        <DiffTable
          ref="diffTable"
...
<script setup lang="ts">
  ...
  const getDefaultData = () => ({
    level2DBA: [] as string[],
    primaryDBA: [] as string[],
    standbyDBA: [] as string[],
  });
  ...
  const isFormEmpty = computed(() => {
    return (
      formData.value.primaryDBA.length === 0 &&
      formData.value.standbyDBA.length === 0 &&
      formData.value.level2DBA.length === 0
    );
  });
  ...
  const getBeforeList = () => {
    ...
    return props.selected.flatMap((selectedItem) => {
      return [
  ...
  watch(
    ...
    () => {
      if (modelValue.value) {
        ...
        const roleMap = {
          [DBARoleTypes.BACKUP_DBA]: formData.value.standbyDBA,
          [DBARoleTypes.LEVEL2_DBA]: formData.value.level2DBA,
          [DBARoleTypes.PRIMARY_DBA]: formData.value.primaryDBA,
        };
        beforeList.forEach((item) => {
          const afterValue = roleMap[item.role] && roleMap[item.role].length > 0 ? roleMap[item.role] : item.before;
          Object.assign(item, {
            after: afterValue,
            isChanged: !_.isEqual(_.sortBy(item.before), _.sortBy(afterValue)),
          });
        });
...
<style lang="less">
  .biz-unmanage-batch-update-header {
    display: flex;
    align-items: center;
  ...
  .biz-unmanage-batch-update-content {
    padding: 24px;
