...
<script setup lang="ts">
  ...
  interface IDataRow {
    cluster: {
      ...
      redis_slave: RedisModel['redis_slave'];
    };
    current_versions: string[];
    ...
    cluster: Object.assign(
      {
        ...
        redis_slave: [] as RedisModel['redis_slave'],
      },
      values.cluster,
    ),
      ...
      editableTableRef.value!.validate().then((validateResult) => {
        if (validateResult) {
          return tableData.value.map((tableItem) => ({
            ...
            target_versions: tableItem.cluster.redis_master.concat(tableItem.cluster.redis_slave).map((proxyItem) => ({
              instance_role: '',
              ip: proxyItem.ip,
              related_clusters: [],
              slave_ip: '',
