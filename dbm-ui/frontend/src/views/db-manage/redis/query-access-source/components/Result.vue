...
<script setup lang="tsx">
  ...
  const tableColumns = [
    ...
    },
    {
      field: 'establish',
      label: t('连接数（ESTAB）'),
      render: ({ data }: { data: RowData }) => (data.remote_ip ? data.establish : '--'),
    },
    {
      field: 'all_connections',
      label: t('连接数（ALL）'),
      render: ({ data }: { data: RowData }) => (data.remote_ip ? data.all_connections : '--'),
    },
    {
      field: 'topo',
      label: t('业务模块'),
