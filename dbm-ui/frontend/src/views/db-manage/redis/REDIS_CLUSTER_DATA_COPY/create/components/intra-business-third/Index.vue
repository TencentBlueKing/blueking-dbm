...
<template>
  <EditableTable
    ...
    <EditableRow
      ...
      <AccessCodeColumn
        v-model="item.dst_cluster_password"
        data-copy-type="copy_to_other_system"
        field="dst_cluster_password"
        :label="t('访问密码')"
        :params="{
...
<script setup lang="ts">
  ...
  const createRowData = (values = {} as Partial<IDataRow>) => ({
    ...
    dst_cluster_password: values?.dst_cluster_password || '',
      ...
      editableTableRef.value!.validate().then((validateResult) => {
        if (validateResult) {
          return tableData.value.map((tableItem) => ({
            ...
            dst_cluster_password: tableItem.dst_cluster_password,
    ...
    setTableByTicketClone: (ticketDetail: TicketModel<Redis.ClusterDataCopy>) => {
      ...
      tableData.value = infos.map((infoItem) =>
        createRowData({
          ...
          dst_cluster: infoItem.dst_cluster as string,
          dst_cluster_password: infoItem.src_cluster_password,
