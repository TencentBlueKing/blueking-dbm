...
<template>
  <EditableTable
    ...
    <EditableRow
      ...
      <TargetBusinessColumn v-model="item.dst_bk_biz_id" />
      <RenderTargetCluster
        v-model="item.dst_cluster"
        :biz-id="item.dst_bk_biz_id"
...
<script setup lang="ts">
  ...
  import TargetBusinessColumn from './components/TargetBusinessColumn.vue';
  import RenderTargetCluster from './components/TargetClusterColumn.vue';
  ...
  const domainMemo = computed(() =>
    Object.fromEntries(
      Object.values(selected.value).flatMap((clusterList) =>
        clusterList
          .filter((clusterItem) => clusterItem.master_domain)
          .map((clusterItem) => [clusterItem.master_domain, true]),
      ),
    ),
  );
  ...
  defineExpose<Exposes>({
    ...
    setTableByTicketClone: (ticketDetail: TicketModel<Redis.ClusterDataCopy>) => {
      ...
      tableData.value = infos.map((infoItem) =>
        createRowData({
          ...
          dst_bk_biz_id: infoItem.dst_bk_biz_id,
    ...
    setTableByLocalStorage: (item: RedisDSTHistoryJobModel) => {
      tableData.value = [
        createRowData({
          cluster: {
            master_domain: item.src_cluster,
          } as IDataRow['cluster'],
          dst_cluster: item.dst_cluster_id,
          dst_bk_biz_id: Number(item.dst_bk_biz_id),
