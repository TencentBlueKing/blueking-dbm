...
<script setup lang="ts">
  ...
  const clusterRoleNodeGroup = computed(() => {
    ...
    return {
      ...
      Master: (data.value?.redis_master || []).map((item) => ({
        ...item,
        displayInstance: item.seg_range && `${item.instance}(${item.seg_range})`,
      })),
      Slave: (data.value?.redis_slave || []).map((item) => ({
        ...item,
        displayInstance: item.seg_range && `${item.instance}(${item.seg_range})`,
      })),
    };
  ...
  const handleSwitchDNSBindCLB = (data: RedisDetailModel) => {
   ...
   * 集群 北极星启用/禁用
   */
  const handleSwitchPolaris = (data: RedisDetailModel) => {
