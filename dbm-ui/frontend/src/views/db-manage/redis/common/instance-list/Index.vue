...
<template>
  <div class="redis-instance-list-page">
    <div class="operation-box">
      ...
      <InstanceBatchCopy
        v-db-console="
          clusterType === ClusterTypes.REDIS_CLUSTER
            ? 'redis.instanceManage.batchCopy'
            : 'redis.haInstanceManage.batchCopy'
        "
        class="ml-8"
      ...
      <DropdownExportExcel
        v-db-console="
          clusterType === ClusterTypes.REDIS_CLUSTER ? 'redis.instanceManage.export' : 'redis.haInstanceManage.export'
        "
        class="ml-8"
    ...
    <InstanceTable
      ...
      <template #shard>
        <ShardColumn :cluster-type="clusterType" />
      </template>
...
<script setup lang="tsx">
  ...
  interface Props {
    clusterType: ClusterTypes.REDIS_CLUSTER | ClusterTypes.REDIS_INSTANCE;
  }
  ...
  const { isSearching, quickSearchData, quickSearchValue } = useInstanceQuickSearch({
    cluster_type:
      props.clusterType === ClusterTypes.REDIS_CLUSTER
        ? [
  ...
  const { settings, updateTableSettings } = useTableSettings(
    props.clusterType === ClusterTypes.REDIS_CLUSTER
      ? UserPersonalSettings.REDIS_INSTANCE_SETTINGS
      : UserPersonalSettings.REDIS_HA_INSTANCE_SETTINGS,
    {
  ...
  const handleApply = () => {
    router.push({
      name: props.clusterType === ClusterTypes.REDIS_CLUSTER ? 'SelfServiceApplyRedis' : 'SelfServiceApplyRedisHa',
      query: {
