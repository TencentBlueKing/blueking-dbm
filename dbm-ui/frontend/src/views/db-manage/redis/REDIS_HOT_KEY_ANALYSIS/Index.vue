...
<script lang="ts" setup>
  ...
  const handleSubmit = async () => {
    ...
    formRef.value!.validate().then(() => {
      const clusterMap = formData.tableData.reduce<Record<number, RowData[]>>((prev, item) => {
        if (prev[item.instance.cluster_id]) {
          return Object.assign(prev, {
            [item.instance.cluster_id]: prev[item.instance.cluster_id].concat(item),
          });
        }
        return Object.assign(prev, {
          [item.instance.cluster_id]: [item],
        });
      }, {});
      ...
      } = formData.tableData[0].instance;

      createTicketRun({
        details: {
          analysis_time: formData.analysis_time,
          bk_cloud_id: bkCloudId,
          infos: Object.values(clusterMap).map((item) => {
            return {
              cluster_id: clusterId,
              cluster_type: clusterType,
              immute_domain: masterDomain,
              ins: item.map((item) => item.instance.instance_address),
            };
          }),
        },
        ...formData.payload,
