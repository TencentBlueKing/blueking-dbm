...
<template>
  <div
    ...
    <template v-if="data">
      <DisplayBox
        cluster-detail-router-name="OracleHaDetail"
        :data="data">
...
<script setup lang="ts">
  ...
  const clusterRoleNodeGroup = computed(() => {
    return {
      ...
      Standby: data.value?.standbys || [],
    };
  });

  const { loading: isLoading, run: fetchClusterDetail } = useRequest(getOracleHaClusterDetail, {
    manual: true,
    onSuccess(result) {
      data.value = result;
    },
