...
<script setup lang="ts">
  ...
  const dstClusterIdList = computed(() => props.clusterData.dstCluster.map((item) => item.id));
  const dstClusterMap = computed(() => Object.fromEntries(props.clusterData.dstCluster.map((cur) => [cur.id, cur])));
    ...
    },
    {
      validator: (value: string) => {
        ...
        return checkClusterDatabase<{
          ...
          cluster_id: props.clusterData.srcCluster.id,
