...
<script setup lang="ts">
  ...
  const clusterIds = defineModel<number[]>('clusterIds', {
    required: true,
  });

  const clusterMap = defineModel<Record<string, string>>('clusterMap', {
    required: true,
  });
    ...
    },
    {
      ...
      validator: (value: string) => {
        ...
        return filterClusters<SqlServerHaModel>({
          ...
          clusterMap.value = Object.fromEntries(data.map((item) => [item.id, item.master_domain]));
          const notExists = domains.filter((domain) => !clusterIsExist[domain]);
          if (notExists.length > 0) {
            return t('目标集群xx不存在', [notExists.join(',')]);
          }
          clusterIds.value = data.map((item) => item.id);
          return true;
        });
      },
    },
  ...
  const handelClusterChange = (selected: { [key: string]: Array<ClusterModelMap[Props['clusterType']]> }) => {
    ...
    clusterIds.value = selected[props.clusterType].map((item) => item.id);
    clusterMap.value = Object.fromEntries(selected[props.clusterType].map((item) => [item.id, item.master_domain]));
  };
</script>
