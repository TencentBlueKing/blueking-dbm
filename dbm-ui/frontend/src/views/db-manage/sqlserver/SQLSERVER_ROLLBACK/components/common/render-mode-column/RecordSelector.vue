...
<template>
  <div ref="rootRef">
    <EditableBlock :placeholder="t('请选择文件')">
      {{ logNameDisplay }}
      <template #prepend>
        <DbIcon
          class="file-flag"
...
<script setup lang="ts">
  ...
  type Emits = (e: 'datetime-confirm') => void;

  interface Expose {
    validateManual: () => boolean;
    validateMatchLog: () => Promise<boolean>;
  }
  ...
  const logNameDisplay = computed(() => {
    return formatLogName(modelValue.value);
  });
  ...
  const fetchLogData = () => {
    ...
    queryBackupLogs({
      ...
      if (modelValue.value) {
        modelValue.value = _.find(
          backupLogListMemo.value,
          (logItem) => logItem.backup_id === modelValue.value?.backup_id,
        );
      }
    });
  };
  ...
  defineExpose<Expose>({
    validateManual() {
      return recordType.value === OperateType.MANUAL ? Boolean(modelValue.value) : true;
    },
    validateMatchLog() {
