...
<template>
  <div class="qdrant-ha-list-page">
    <div class="operation-box">
      <AuthButton
        v-db-console="'qdrant.haClusterList.instanceApply'"
        action-id="k8s_qdrant_apply"
        theme="primary"
        @click="handleApply">
        {{ t('申请实例') }}
      </AuthButton>
      <DropdownExportExcel
        v-db-console="'qdrant.haClusterList.export'"
    ...
    <ClusterTable
      ...
      <template #operation>
        <OperationColumn
          ...
          :cluster-type="ClusterTypes.K8S_QDRANT_HA">
          <template #default="{ data }: { data: QdrantHaModel }">
            <div
              v-if="data.isOnline"
              v-db-console="'qdrant.haClusterList.disable'">
            ...
            <div
              v-if="data.isOnline"
              v-db-console="'qdrant.haClusterList.restart'">
              <OperationBtnStatusTips :data="data">
                <AuthButton
            ...
            <div
              v-if="data.isOffline"
              v-db-console="'qdrant.haClusterList.enable'">
              <OperationBtnStatusTips
                :data="data"
                style="width: 100%">
                <AuthButton
            ...
            <div v-db-console="'qdrant.haClusterList.delete'">
              <OperationBtnStatusTips :data="data">
                <AuthButton
                  v-bk-tooltips="{
                  ...
                  :permission="data.permission.k8s_qdrant_destroy"
                  :resource="data.id"
                  text
...
<script setup lang="ts">
  ...
  import { getQdrantHaList } from '@services/source/qdrantHa';
  ...
  import ClusterDetail from '@views/db-manage/qdrant/common/ha-cluster-detail/Index.vue';

  const route = useRoute();
  const router = useRouter();
  const { t } = useI18n();
  const { isSearching, quickSearchData, searchValue } = useClusterQuickSearch(ClusterTypes.K8S_QDRANT_HA);
  ...
  const { handleClusterRestart } = useK8sClusterRestart(ClusterTypes.K8S_QDRANT_HA, {
    onSuccess: () => fetchData(),
  });
  ...
  } = useGoClusterDetail('QdrantHaDetail');

  const { handleSelection, isSelected, selectedIdList, selectedList } = useClusterTableSelect<QdrantHaModel>();
  ...
  // 设置用户个人表头信息
  const { settings, updateTableSettings } = useTableSettings(UserPersonalSettings.QDRANT_HA_TABLE_SETTINGS, {
    disabled: ['domain'],
  });
  ...
  const handleApply = () => {
    router.push({
      name: TicketTypes.K8S_QDRANT_HA_APPLY,
      query: {
