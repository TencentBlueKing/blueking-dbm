      ...
      <DbForm
        ...
        <DbCard :title="t('资源配置')">
          <K8SVersion
            ...
            addon-type="qdrant" />
          <QdrantSpecConfig
            v-model:qdrant="formData.details.qdrant"
            addon-type="qdrant"
...
<script setup lang="ts">
  ...
  import type { Qdrant } from '@services/model/ticket/ticket';
  ...
  import QdrantSpecConfig, { getDefaultQdrantConfig } from './components/QdrantSpecConfig.vue';
  ...
  const initData = () => ({
    ...
    details: {
      ...
      cluster_alias: '',
      cluster_name: '',
      cluster_type: ClusterTypes.K8S_QDRANT_HA,
      creator: '',
      db_app_abbr: '',
      db_version: '',
      k8s_cluster_name: '',
      major_version: '',
      qdrant: [getDefaultQdrantConfig()],
      remark: '',
    },
    remark: '',
    ticket_type: TicketTypes.K8S_QDRANT_HA_APPLY,
  });
  ...
  useTicketDetail<Qdrant.HaApply>(TicketTypes.K8S_QDRANT_HA_APPLY, {
    onSuccess(ticketDetail) {
      const { details } = ticketDetail;

      const [qdrantItem] = details.component_list;
        ...
        qdrant: [
          {
            ...
            replicas: qdrantItem.replicas,
            request_cpu: Number(qdrantItem.request_cpu),
            request_memory: Number(qdrantItem.request_memory.replace('Gi', '')),
            storage: Number(qdrantItem.storage.replace('Gi', '')),
          },
        ],
      });
    },
  ...
  const handleSubmit = async () => {
    ...
    const getDetails = () => {
      ...
      const { qdrant } = details;
      const qdrantItem = qdrant![0];
      ...
      Object.assign(details, {
        ...
        component_list: [
          {
            ...qdrantItem,
            request_cpu: `${qdrantItem.request_cpu}`,
            request_memory: `${qdrantItem.request_memory}Gi`,
            storage: `${qdrantItem.storage}Gi`,
          },
        ],
