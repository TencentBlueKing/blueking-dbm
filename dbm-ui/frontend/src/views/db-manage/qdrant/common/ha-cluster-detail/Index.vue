...
<template>
  <div
    ...
    <template v-if="data">
      <DisplayBox
        cluster-detail-router-name="QdrantHaDetail"
        :data="data">
        <div
          v-if="data.isOnline"
...
<script setup lang="ts">
  ...
  import { getQdrantHaDetail } from '@services/source/qdrantHa';
  ...
  const data = ref<QdrantHaDetailModel>();

  const { loading: isLoading, run: fetchClusterDetail } = useRequest(getQdrantHaDetail, {
    manual: true,
    onSuccess(result) {
      data.value = result;
    },
  ...
  const { handleDisableCluster } = useOperateClusterBasic(ClusterTypes.K8S_QDRANT_HA, {
    onSuccess: () => {
      fetchDetailData();
      emits('change');
    },
