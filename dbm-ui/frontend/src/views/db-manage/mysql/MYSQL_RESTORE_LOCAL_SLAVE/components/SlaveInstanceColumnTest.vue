...
<template>
  ...
  <InstanceSelector
    ...
    :source-data-params="sourceDataParams"
...
<script lang="ts" setup>
  ...
  const uniquePanelSettings = {
    enable: false,
    // tip: 'tip',
  };

  const disableSelectMethod = (data: TendbhaInstanceModel) => {
    if (data.cluster_type === ClusterTypes.TENDBHA) {
      return data.role === 'proxy' ? t('实例已禁用') : false;
    }
    // return data.role === 'orphan' ? t('实例已禁用') : false;
    return false;
  };
  ...
  const selectedInstances = computed(() => ({
    [ClusterTypes.TENDBHA]: props.selected
      .filter((item) => item.cluster_type === ClusterTypes.TENDBHA)
      .map((item) => ({
        instance_address: item.instance_address,
      })) as TendbhaInstanceModel[],
    [ClusterTypes.TENDBSINGLE]: props.selected
      .filter((item) => item.cluster_type === ClusterTypes.TENDBSINGLE)
      .map((item) => ({
        instance_address: item.instance_address,
      })) as TendbhaInstanceModel[],
  }));
