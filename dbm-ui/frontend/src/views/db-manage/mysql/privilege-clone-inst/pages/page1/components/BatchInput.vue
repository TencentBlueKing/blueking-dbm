...
<script setup lang="ts">
  ...
  type Emits = (e: 'change', value: Array<{ source: string; target: string }>) => void;
   ...
   */
  function handleCopy() {
    copy('127.0.0.1:8080    127.0.0.2:8080');
  }
  ...
  function handleConfirm() {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (ipPort.test(`${ip}:${port}`) === false || ipPort.test(target) === false) {
        const remove = lines.splice(i, 1);
        newLines.push(...remove);
      }
    }
    ...
    if (state.ipError.show || state.formatError.show) {
      return;
    }
