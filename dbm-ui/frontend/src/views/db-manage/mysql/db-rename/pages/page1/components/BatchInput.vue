<!--
 * TencentBlueKing is pleased to support the open source community by making 蓝鲸智云-DB管理系统(BlueKing-BK-DBM) available.
 *
 * Copyright (C) 2017-2023 THL A29 Limited, a Tencent company. All rights reserved.
 *
 * Licensed under the MIT License (the "License"); you may not use this file except in compliance with the License.
 * You may obtain a copy of the License athttps://opensource.org/licenses/MIT
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License is distributed
 * on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the License for
 * the specific language governing permissions and limitations under the License.
-->

<template>
  <BkDialog
    :is-show="isShow"
    :quick-close="false"
    :title="t('DB重命名_批量录入')"
    :width="700"
    @closed="handleClose">
    <div class="batch-input">
      <div class="batch-input-format">
        <div class="batch-input-format-item">
          <strong>{{ t('集群域名') }}</strong>
          <p class="pt-8">test.mysql.db</p>
        </div>
        <div class="batch-input-format-item">
          <strong>{{ t('源DB名') }}</strong>
          <p class="pt-8">origin-db</p>
        </div>
        <div class="batch-input-format-item">
          <strong>{{ t('新DB名') }}</strong>
          <p class="pt-8">
            rename-db
            <DbIcon
              v-bk-tooltips="t('复制格式')"
              class="batch-input-copy"
              type="copy"
              @click="handleCopy" />
          </p>
        </div>
      </div>
      <BkInput
        ref="inputRef"
        v-model="state.values"
        class="batch-input-textarea"
        :placeholder="placeholder"
        :rows="20"
        style="height: 320px; margin: 12px 0 30px"
        type="textarea"
        @input="handleInput" />
      <div class="batch-input-errors">
        <span
          v-if="state.formatError.show"
          class="mr-8">
          <I18nT
            keypath="n处格式错误"
            tag="span">
            <strong>{{ state.formatError.count }}</strong>
          </I18nT>
          <DbIcon
            v-bk-tooltips="t('标记错误')"
            class="batch-input-errors-icon"
            type="audit"
            @click="handleSelectionError('formatError')" />
        </span>
        <!-- <span v-if="state.nameError.show">
          <strong>{{ state.nameError.count }}</strong>
          处 DB 名格式错误
          <DbIcon
            v-bk-tooltips="'标记错误'"
            class="batch-input-errors-icon"
            type="audit"
            @click="handleSelectionError('nameError')" />
        </span> -->
      </div>
    </div>
    <template #footer>
      <BkButton
        class="mr-8 w-88"
        theme="primary"
        @click="handleConfirm">
        {{ t('确定') }}
      </BkButton>
      <BkButton
        class="w-88"
        @click="handleClose">
        {{ t('取消') }}
      </BkButton>
    </template>
  </BkDialog>
</template>

<script setup lang="ts">
  import { useI18n } from 'vue-i18n';

  import { useCopy } from '@hooks';

  type Emits = (e: 'change', value: Array<{ domain: string; origin: string; rename: string }>) => void;

  const emits = defineEmits<Emits>();

  const isShow = defineModel<boolean>('isShow', {
    default: false,
  });

  const { t } = useI18n();
  const copy = useCopy();

  const inputRef = ref();

  const placeholder = t('请分别输入集群域名_单个_源DB名_单个_新DB名_单个_多个对象_换行分隔');

  const state = reactive({
    formatError: {
      count: 0,
      selectionEnd: 0,
      selectionStart: 0,
      show: false,
    },
    nameError: {
      count: 0,
      selectionEnd: 0,
      selectionStart: 0,
      show: false,
    },
    values: '',
  });

  /**
   * 复制格式
   */
  function handleCopy() {
    copy('test.mysql.db    origin-db    rename-db');
  }

  /**
   * 标记错误信息
   */
  function handleSelectionError(key: 'nameError' | 'formatError') {
    const { selectionEnd, selectionStart } = state[key];
    const textarea = inputRef.value?.$el?.getElementsByTagName?.('textarea')?.[0];
    if (textarea) {
      (textarea as HTMLInputElement).focus();
      (textarea as HTMLInputElement).setSelectionRange(selectionStart, selectionEnd);
    }
  }

  function handleInput() {
    state.formatError.show = false;
    state.nameError.show = false;
  }

  function handleClose() {
    const init = {
      count: 0,
      selectionEnd: 0,
      selectionStart: 0,
      show: false,
    };
    state.formatError = { ...init };
    state.nameError = { ...init };
    state.values = '';
    isShow.value = false;
  }

  function handleConfirm() {
    if (state.values === '') {
      handleClose();
      return;
    }

    const newLines: string[] = [];
    const lines = state.values.split('\n').filter((text) => text);
    const getContents = (value: string) => {
      const contents = value
        .trim() // 清除前后空格
        .replace(/\s+/g, ' ') // 替换多余空格
        .split(' '); // 通过空格分割
      return contents;
    };

    // 格式错误
    for (let i = lines.length - 1; i >= 0; i--) {
      const contents = getContents(lines[i]);
      if (contents.length !== 3 || contents.some((text) => !text)) {
        const remove = lines.splice(i, 1);
        newLines.push(...remove);
      }
    }
    const count = newLines.length;
    state.formatError.count = count;
    state.formatError.selectionStart = 0;
    state.formatError.selectionEnd = newLines.join('\n').length;
    state.formatError.show = count > 0;

    // // db name 不符合正则错误
    // for (let i = lines.length - 1; i >= 0; i--) {
    //   const regex = /\W+/;
    //   const contents = getContents(lines[i]);
    //   if (!contents.slice(1).every(name => regex.test(name))) {
    //     const remove = lines.splice(i, 1);
    //     newLines.push(...remove);
    //   }
    // }
    // state.nameError.count = newLines.length - count;
    // state.nameError.selectionStart = state.formatError.selectionEnd === 0 ? 0 : state.formatError.selectionEnd + 1;
    // state.nameError.selectionEnd = newLines.join('\n').length;
    // state.nameError.show = newLines.slice(count).length > 0;

    // 将调整好的内容回填显示
    newLines.push(...lines); // 没有错误内容回填
    state.values = newLines.join('\n');

    // if (state.nameError.show || state.formatError.show) return;

    // if (state.nameError.show || state.formatError.show) return;
    if (state.formatError.show) {
      return;
    }

    const res = newLines.map((item) => {
      const [domain, origin, rename] = getContents(item);
      return {
        domain,
        origin,
        rename,
      };
    });
    emits('change', res);
    handleClose();
  }
</script>

<style lang="less" scoped>
  .batch-input {
    position: relative;

    .batch-input-format {
      display: flex;
      padding: 16px;
      background-color: #f5f7fa;
      border-radius: 2px;

      .batch-input-format-item {
        margin-right: 24px;
        font-size: @font-size-mini;
      }
    }

    .batch-input-copy {
      color: @primary-color;
      cursor: pointer;
    }

    .batch-input-textarea {
      height: 310px;
      margin: 16px 0 30px;

      :deep(textarea) {
        &::selection {
          background-color: #fdd;
        }
      }
    }

    .batch-input-errors {
      position: absolute;
      bottom: 8px;
      font-size: @font-size-mini;
      color: @danger-color;

      .batch-input-errors-icon {
        font-size: @font-size-large;
        color: @gray-color;
        cursor: pointer;

        &:hover {
          color: @default-color;
        }
      }
    }
  }
</style>
