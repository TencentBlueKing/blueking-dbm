...
<script setup lang="tsx">
  ...
  const columns = computed(() => [
    ...
    },
    {
      ...
      label: t('类型'),
      render: ({ data }: { data: KafkaNodeModel }) => <RenderClusterRole data={[data.role]} />,
      width: 200,
    },
    {
      field: 'status',
  ...
  const handleShrinkOne = (data: KafkaNodeModel) => {
    operationNodeList.value = [data];
    isShowShrink.value = true;
  };

  const handleReplaceOne = (data: KafkaNodeModel) => {
    operationNodeList.value = [data];
    isShowReplace.value = true;
  };

  const handleShowDetail = (data: KafkaNodeModel) => {
