...
<template>
  <div class="surrealdb-single-list-page">
    <div class="operation-box">
      <AuthButton
        v-db-console="'surrealdb.singleClusterList.instanceApply'"
      ...
      <DropdownExportExcel
        v-db-console="'surrealdb.singleClusterList.export'"
    ...
    <ClusterTable
      ...
      <template #operation>
        <OperationColumn
          ...
          :cluster-type="ClusterTypes.K8S_SURREALDB_SINGLE">
          <template #default="{ data }: { data: SurrealdbSingleModel }">
            <div
              v-if="data.isOnline"
            ...
            <div v-db-console="'surrealdb.singleClusterList.delete'">
              <OperationBtnStatusTips :data="data">
                <AuthButton
                  v-bk-tooltips="{
...
<script setup lang="ts">
  ...
  import ClusterDetail from '@views/db-manage/surrealdb/common/single-cluster-detail/Index.vue';

  const route = useRoute();
  const router = useRouter();
  const { t } = useI18n();
  const { isSearching, quickSearchData, searchValue } = useClusterQuickSearch(ClusterTypes.K8S_SURREALDB_SINGLE);
  ...
  } = useGoClusterDetail('SurrealdbSingleDetail');

  const { handleSelection, isSelected, selectedIdList, selectedList } = useClusterTableSelect<SurrealdbSingleModel>();
  ...
  // 设置用户个人表头信息
  const { settings, updateTableSettings } = useTableSettings(UserPersonalSettings.SURREALDB_SINGLE_TABLE_SETTINGS, {
    disabled: ['domain'],
  });
  ...
  const handleApply = () => {
    router.push({
      name: TicketTypes.K8S_SURREALDB_SINGLE_APPLY,
      query: {
