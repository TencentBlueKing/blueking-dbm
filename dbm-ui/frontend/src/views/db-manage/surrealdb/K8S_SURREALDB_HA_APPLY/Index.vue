        ...
        <DbCard :title="t('资源配置')">
          ...
          <DbFormItem :label="t('部署模式')">
            <BkRadioGroup
              ...
              style="width: 270px"
              type="card">
              <BkRadioButton label="cluster">
                {{ t('集群') }}
              </BkRadioButton>
              <span class="input-desc ml-12">{{ t('surreal + tikv + pd 分布式架构') }}</span>
            </BkRadioGroup>
          </DbFormItem>
          <AddonSpecPlan
            v-model:pd="formData.details.pd"
...
<script setup lang="ts">
  ...
  import AddonSpecPlan, {
    getDefaultPdConfig,
    getDefaultSurrealConfig,
    getDefaultTikvConfig,
  ...
  const initData = () => ({
    ...
    details: {
      ...
      cluster_alias: '',
      cluster_name: '',
      cluster_type: ClusterTypes.K8S_SURREALDB_HA,
      creator: '',
      db_app_abbr: '',
      db_version: '', // 小版本
      k8s_cluster_name: '',
      major_version: '', // 大版本
      pd: [getDefaultPdConfig()],
      remark: '',
      surreal: [getDefaultSurrealConfig()],
      tikv: [getDefaultTikvConfig()],
    },
    remark: '',
    ticket_type: TicketTypes.K8S_SURREALDB_HA_APPLY,
  });
  ...
  useTicketDetail<SurrealDB.HaApply>(TicketTypes.K8S_SURREALDB_HA_APPLY, {
    onSuccess(ticketDetail) {
      const { details } = ticketDetail;

      const [surrealItem, tikvItem, pdItem] = details.component_list;
      ...
      Object.assign(formData.details, {
        ...
        pd: [
          {
            component_name: 'pd',
            replicas: pdItem.replicas,
            request_cpu: Number(pdItem.request_cpu),
            request_memory: Number(pdItem.request_cpu.replace('Gi', '')),
            storage: Number(pdItem.storage.replace('Gi', '')),
          },
        ],
        surreal: [
          {
        ...
        tikv: [
          {
            component_name: 'tikv',
            replicas: tikvItem.replicas,
            request_cpu: Number(tikvItem.request_cpu),
            request_memory: Number(tikvItem.request_cpu.replace('Gi', '')),
            storage: Number(tikvItem.storage.replace('Gi', '')),
          },
        ],
      });
    },
  ...
  const handleSubmit = async () => {
    ...
    const getDetails = () => {
      ...
      const { pd, surreal, tikv } = details;
      const surrealItem = surreal![0];
      const pdItem = pd![0];
      const tikvItem = tikv![0];
      ...
      Object.assign(details, {
        ...
        component_list: [
          ...
          {
            ...pdItem,
            request_cpu: `${pdItem.request_cpu}`,
            request_memory: `${pdItem.request_memory}Gi`,
            storage: `${pdItem.storage}Gi`,
          },
          {
            ...tikvItem,
            request_cpu: `${tikvItem.request_cpu}`,
            request_memory: `${tikvItem.request_memory}Gi`,
            storage: `${tikvItem.storage}Gi`,
          },
        ],
