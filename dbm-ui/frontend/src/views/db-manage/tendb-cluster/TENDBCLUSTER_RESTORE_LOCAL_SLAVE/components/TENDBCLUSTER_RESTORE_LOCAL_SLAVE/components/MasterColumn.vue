  ...
  const { loading, run: queryHost } = useRequest(checkInstance, {
    ...
    onSuccess: (data) => {
      if (data.length) {
        ...
        modelValue.value = {
          bk_biz_id: window.PROJECT_CONFIG.BIZ_ID,
<<<<<<<< HEAD:dbm-ui/frontend/src/views/db-manage/tendb-cluster/TENDBCLUSTER_RESTORE_LOCAL_SLAVE/components/TENDBCLUSTER_RESTORE_LOCAL_SLAVE/components/SlaveInstanceColumn.vue
...
========
          ...
          role: item.role,
>>>>>>>> 2b66d8680 (fix(frontend): 主库故障切换重构_验收问题处理 #9859):dbm-ui/frontend/src/views/db-manage/tendb-cluster/TENDBCLUSTER_RESTORE_LOCAL_SLAVE/components/TENDBCLUSTER_RESTORE_LOCAL_SLAVE/components/MasterColumn.vue
        };
      }
    },
  });
