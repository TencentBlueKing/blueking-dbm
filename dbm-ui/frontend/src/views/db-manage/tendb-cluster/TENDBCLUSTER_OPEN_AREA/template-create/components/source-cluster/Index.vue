...
<script lang="ts" setup>
  ...
  const selectedCluster = shallowRef<Record<string, TendbclusterModel[]>>({
    [ClusterTypes.TENDBCLUSTER]: [],
  });
  ...
  const handelChange = (selected: Record<string, TendbclusterModel[]>) => {
    const selectList = Object.keys(selected).reduce(
      (list: TendbclusterModel[], key) => list.concat(...selected[key]),
      [],
    );
