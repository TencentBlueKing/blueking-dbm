...
<script lang="ts" setup>
  ...
  const selected = computed<Record<string, TendbclusterModel[]>>(() => ({
    [ClusterTypes.TENDBCLUSTER]: props.selectedIds.map(
      (id) =>
        ({
          id,
  ...
  const handleSelectorChange = (selected: Record<string, TendbclusterModel[]>) => {
    const dataList = selected[ClusterTypes.TENDBCLUSTER];
