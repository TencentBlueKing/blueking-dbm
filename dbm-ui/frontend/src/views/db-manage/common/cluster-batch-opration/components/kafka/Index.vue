<template>
  <BkDropdownItem v-db-console="'kafka.clusterManage.batchAddTag'">
  ...
  <BkDropdownItem v-db-console="'kafka.clusterManage.batchRemoveTag'">
    <BatchOperationButton
      :action-id="TAG_ACTION_ID"
      :disabled="!tagEditable && !tagNoPermission"
      :no-permission="tagNoPermission"
  ...
  <BkDropdownItem
    ...
    v-db-console="'kafka.clusterManage.configAlarmSubscription'">
    <BatchOperationButton
  ...
  <BkDropdownItem
    v-if="isClusterTypeAlarmSupported"
    v-db-console="'kafka.clusterManage.deleteAlarmSubscription'">
    <BatchOperationButton
      :action-id="SUBSCRIBE_ACTION_ID"
      :disabled="!subscriptionEditable && !subscriptionNoPermission"
      :no-permission="subscriptionNoPermission"
  ...
  <ClusterBatchAddTag
    ...
    :get-editable="(item) => item.permission?.kafka_edit !== false"
    :selected="selected"
...
<script setup lang="ts">
  ...
  const { t } = useI18n();
  const { isClusterTypeAlarmSupported } = useAlarmSubscribe([ClusterTypes.KAFKA]);
  ...
  const TAG_ACTION_ID = 'kafka_edit';
  /** 设置/删除告警订阅鉴权 action-id（与单行一致） */
  const SUBSCRIBE_ACTION_ID = 'kafka_subscribe_monitor';
  /** 是否具备告警订阅权限 */
  const hasSubscribePermission = (data: KafkaModel) =>
  ...
  const tagNoPermission = computed(
    () => props.selected.length > 0 && props.selected.every((data) => data.permission.kafka_edit === false),
  );
  /** 添加/移除标签：至少 1 个有权限则亮起 */
  const tagEditable = computed(() => props.selected.some((data) => data.permission.kafka_edit !== false));
