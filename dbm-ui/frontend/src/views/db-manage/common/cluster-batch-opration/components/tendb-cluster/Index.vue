<template>
  ...
  <BkDropdownItem
    ...
    v-db-console="'tendbCluster.clusterManage.batchAddTag'">
    <BatchOperationButton
  ...
  <BkDropdownItem
    ...
    v-db-console="'tendbCluster.clusterManage.batchRemoveTag'">
    <BatchOperationButton
      :action-id="TAG_ACTION_ID"
      :disabled="tagDisabled"
      :no-permission="tagNoPermission"
...
<script setup lang="ts">
  ...
  const AUTHORIZE_ACTION_ID = 'tendbcluster_authorize';
  /** 批量授权：全部无权限时置灰可点击，点击弹权限申请 */
  const authorizeNoPermission = computed(
    () => props.selected.length > 0 && props.selected.every((data) => data.permission.tendbcluster_authorize === false),
  );
  ...
  const batchAuthorizeDisabled = computed(
    () =>
      ...
      props.selected.every((data) => data.permission.tendbcluster_authorize !== false && data.isOffline),
  );
  ...
  const tagDisabled = computed(
    () =>
      ...
      !props.selected.some((data) => data.permission.tendbcluster_edit !== false && !data.isOffline),
  );
