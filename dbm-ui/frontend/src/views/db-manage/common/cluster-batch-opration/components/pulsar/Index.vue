<template>
  <BkDropdownItem
    ...
    v-db-console="'pulsar.clusterManage.batchAddTag'">
  ...
  <BkDropdownItem
    ...
    v-db-console="'pulsar.clusterManage.batchRemoveTag'">
    <BatchOperationButton
      :action-id="TAG_ACTION_ID"
      :disabled="tagDisabled"
      :no-permission="tagNoPermission"
...
<script setup lang="ts">
  ...
  const TAG_ACTION_ID = 'pulsar_edit';
  /** 设置/删除告警订阅鉴权 action-id（与单行一致） */
  const SUBSCRIBE_ACTION_ID = 'pulsar_subscribe_monitor';
  /** 是否具备告警订阅权限 */
  const hasSubscribePermission = (data: PulsarModel) =>
  ...
  const tagNoPermission = computed(
    () => props.selected.length > 0 && props.selected.every((data) => data.permission.pulsar_edit === false),
  );
  ...
  const tagDisabled = computed(
    () =>
      ...
      !props.selected.some((data) => data.permission.pulsar_edit !== false && !data.isOffline),
  );
