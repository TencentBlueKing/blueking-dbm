<template>
  <BkDropdownItem
    ...
    v-db-console="'hdfs.clusterManage.batchAddTag'">
  ...
  <BkDropdownItem
    ...
    v-db-console="'hdfs.clusterManage.batchRemoveTag'">
    <BatchOperationButton
      :action-id="TAG_ACTION_ID"
      :disabled="tagDisabled"
      :no-permission="tagNoPermission"
...
<script setup lang="ts">
  ...
  const tagDisabled = computed(
    () =>
      !tagNoPermission.value && !props.selected.some((data) => data.permission.hdfs_edit !== false && !data.isOffline),
  );
