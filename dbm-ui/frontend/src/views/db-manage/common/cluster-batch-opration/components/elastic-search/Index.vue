<template>
  ...
  <ClusterBatchAddTag
    ...
    :get-editable="(item) => item.permission?.es_edit !== false"
    :selected="selected"
...
<script setup lang="ts">
  ...
  const TAG_ACTION_ID = 'es_edit';
  /** 设置/删除告警订阅鉴权 action-id（与单行一致） */
  const SUBSCRIBE_ACTION_ID = 'es_subscribe_monitor';
  /** 是否具备告警订阅权限 */
  const hasSubscribePermission = (data: EsModel) =>
  ...
  const tagEditable = computed(() => props.selected.some((data) => data.permission.es_edit !== false));
