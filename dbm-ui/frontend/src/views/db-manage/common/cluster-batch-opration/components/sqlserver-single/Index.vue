<template>
  ...
  <BkDropdownItem v-db-console="'sqlserver.singleClusterList.enable'">
    <BkButton
      class="opration-button"
...
<script setup lang="ts">
  ...
  const { handleConfirmDialog, handleDeleteCluster, handleDisableCluster, handleEnableCluster, isShow, operateDialog } =
    useOperateClusterBatch<SqlserverSingleModel>(ClusterTypes.SQLSERVER, {
      ...
      hasPermission: (data) =>
        data.permission.sqlserver_enable_disable !== false || data.permission.sqlserver_destroy !== false,
  ...
  const isClusterTagEditable = computed(() => props.selected.some((data) => data.permission.sqlserver_edit !== false));
  /** 是否具备禁用/启用/删除权限 */
  const hasOperatePermission = (data: SqlserverSingleModel) =>
    data.permission.sqlserver_enable_disable !== false || data.permission.sqlserver_destroy !== false;
