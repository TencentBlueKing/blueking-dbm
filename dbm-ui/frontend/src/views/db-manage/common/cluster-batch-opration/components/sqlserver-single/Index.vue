<template>
  <BkDropdownItem
    ...
    v-db-console="'sqlserver.singleClusterList.batchAuthorize'">
  ...
  <BkDropdownItem
    ...
    v-db-console="'sqlserver.singleClusterList.batchAddTag'">
    <BatchOperationButton
  ...
  <BkDropdownItem
    ...
    v-db-console="'sqlserver.singleClusterList.batchRemoveTag'">
    <BatchOperationButton
      :action-id="TAG_ACTION_ID"
      :disabled="tagDisabled"
      :no-permission="tagNoPermission"
  ...
  <BkDropdownItem
    ...
    v-db-console="'sqlserver.singleClusterList.configAlarmSubscription'">
    <BatchOperationButton
  ...
  <BkDropdownItem
    ...
    v-db-console="'sqlserver.singleClusterList.deleteAlarmSubscription'">
    <BatchOperationButton
      :action-id="SUBSCRIBE_ACTION_ID"
      :disabled="subscriptionDisabled"
      :no-permission="subscriptionNoPermission"
...
<script setup lang="ts">
  ...
  const { handleConfirmDialog, handleDeleteCluster, handleDisableCluster, handleEnableCluster, isShow, operateDialog } =
    useOperateClusterBatch<SqlserverSingleModel>(ClusterTypes.SQLSERVER, {
  ...
  const { isClusterTypeAlarmSupported } = useAlarmSubscribe([ClusterTypes.SQLSERVER_SINGLE]);
  ...
  /** 是否具备禁用/启用权限 */
  const hasDisablePermission = (data: SqlserverSingleModel) => data.permission.sqlserver_enable_disable !== false;
  /** 是否具备删除权限 */
  const hasDeletePermission = (data: SqlserverSingleModel) => data.permission.sqlserver_destroy !== false;
  /**
   * 禁用/启用/删除三态：
  ...
  /** 是否具备告警订阅权限 */
  const hasSubscribePermission = (data: SqlserverSingleModel) =>
