<template>
  <BkDropdownItem
    ...
    v-db-console="'doris.clusterManage.batchAddTag'">
  ...
  <BkDropdownItem
    ...
    v-db-console="'doris.clusterManage.batchRemoveTag'">
    <BatchOperationButton
      :action-id="TAG_ACTION_ID"
      :disabled="tagDisabled"
      :no-permission="tagNoPermission"
...
<script setup lang="ts">
  ...
  const { t } = useI18n();
  const { isClusterTypeAlarmSupported } = useAlarmSubscribe([ClusterTypes.DORIS]);
  ...
  const tagDisabled = computed(
    () =>
      !tagNoPermission.value && !props.selected.some((data) => data.permission.doris_edit !== false && !data.isOffline),
  );
