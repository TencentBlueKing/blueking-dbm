...
<script setup lang="ts">
  ...
  watch(isShow, () => {
    if (isShow.value && props.data.rules.length === 1) {
      ...
      const privileges = props.data.rules[0].reduce<string[]>(
        (acc, item) => [...acc, ...item.privilege.split(',')],
        [],
      );
