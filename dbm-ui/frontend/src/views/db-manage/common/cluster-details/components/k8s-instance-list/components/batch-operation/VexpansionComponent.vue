...
<template>
  <BkDialog
    ...
    quick-close>
    <template #header>
      <div class="k8s-instance-list-vexpansion-component-header">
        {{ t('磁盘扩容') }}
        <div class="header-divider ml-8 mr-8" />
        <div class="header-role">{{ role }}</div>
      </div>
    </template>
    <div class="k8s-instance-list-vexpansion-component mt-24">
      <DbForm
        ref="form"
        label-width="0"
        :model="formData">
        <DbFormItem
          ...
          required>
          <div class="form-box">
            <div class="box-label">{{ t('磁盘容量') }}</div>
            <div class="box-count">{{ beforeData.storage }} Gi</div>
...
<script setup lang="tsx">
  ...
  import { vexpansionComponent } from '@services/source/kubernetesToolbox';
  ...
  const handleConfirm = async () => {
    ...
    const { storage } = formData.value;

    runVexpansionComponent({
...
<style lang="less">
  .k8s-instance-list-vexpansion-component-header {
    display: flex;
    align-items: center;
  ...
  .k8s-instance-list-vexpansion-component {
    .form-box {
      display: flex;
      padding: 8px;
