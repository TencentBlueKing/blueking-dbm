...
<template>
  <div class="hdfs-detail-host-list">
...
<script setup lang="tsx">
  ...
  const { initQuickSearchValue, quickSearchData, quickSearchValue } = useHostQuickSearch(ClusterTypes.HDFS, {
  ...
  const checkNodeReplaceDisable = (node: HdfsMachineModel) => {
    const options = {
      disabled: false,
      tooltips: {
        content: '',
  ...
  const batchShrinkDisabledInfo = computed(() => {
    ...
    if (_.find(selectedMachineList.value, (item) => !item.isDataNode)) {
      options.disabled = true;
      options.tooltips.disabled = false;
