...
<template>
  <div
    ...
    class="hdfs-detail-page">
    <BkTab
      v-model:active="activePanelKey"
      class="detail-tab"
      type="card-tab">
      <BkTabPanel
        v-if="checkDbConsole('hdfs.clusterManage.clusterTopo')"
        :label="t('集群拓扑')"
        name="topo" />
      <BkTabPanel
        :label="t('节点列表')"
      ...
      <BkTabPanel
        ...
        name="baseInfo" />
      <BkTabPanel
        v-if="checkDbConsole('hdfs.clusterManage.changeLog')"
        :label="t('变更记录')"
        name="record" />
      <BkTabPanel
    ...
    () => {
      if (props.clusterId) {
        ...
        runGetMonitorUrls({
          ...
          cluster_type: ClusterTypes.HDFS,
