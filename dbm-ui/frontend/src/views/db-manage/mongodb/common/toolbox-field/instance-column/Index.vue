...
<script lang="ts" setup>
  ...
  type Emits = (e: 'batch-edit', list: MongodbInstanceModel[]) => void;
  ...
  const dataSourceMap = {
    [ClusterTypes.MONGO_REPLICA_SET]: (params: ServiceParameters<typeof getMongoInstancesList>) =>
      getMongoInstancesList({
        ...params,
        cluster_type: ClusterTypes.MONGO_REPLICA_SET,
      }),
    [ClusterTypes.MONGO_SHARED_CLUSTER]: (params: ServiceParameters<typeof getMongoInstancesList>) =>
      getMongoInstancesList({
        ...params,
  ...
  const selectedInstances = computed(() => ({
    [ClusterTypes.MONGO_REPLICA_SET]: props.selected.map((item) => ({
      instance_address: item.instance_address,
    })) as MongodbInstanceModel[],
    [ClusterTypes.MONGO_SHARED_CLUSTER]: props.selected.map((item) => ({
      instance_address: item.instance_address,
    })) as MongodbInstanceModel[],
  }));
  ...
  const { loading, run: queryInstance } = useRequest(checkInstance, {
    ...
    onSuccess: (data) => {
      if (data.length) {
        ...
        const [{ region }] = instanceInfo.related_clusters;
        modelValue.value = {
