        ...
        <EditableTable
          ...
          <EditableTableRow
            ...
            <EditSpecColumn
              ...
              :current-spec-ids="item.spec_id ? [item.spec_id] : []"
              field="spec_id"
              :label="t('新机规格')"
    ...
    const newList = list.map((item) =>
      createRowData({
        host: {
          ...
          related_clusters: item.related_clusters.map((item) => item.master_domain),
        },
        spec_id: item.spec_config?.id,
      }),
    );

    tableData.value = sortTableByCluster([...(tableData.value[0].host.ip ? tableData.value : []), ...newList]);
    window.changeConfirm = true;
  };

  // 表格排序，方便合并集群显示
  const sortTableByCluster = (dataList: IDataRow[]) => {
