...
<script setup lang="ts">
  ...
  const rules = [
    {
      validator: (list: string[]) => {
        if (props.cluster.disaster_tolerance_level !== 'CROS_SUBZONE') {
          return true;
        }
  ...
  const dataList = computed(() => props.cluster.mongos || []);
  const optionDisabled = computed(() => dataList.value.length - modelValue.value.length < 3);
</script>
