  ...
  const rules = [
    ...
    },
    {
      validator: () => {
        ...
        const otherHostMap = Object.values(otheHostMemoMap).reduce(
          (result, item) => ({
            ...result,
            ...item,
          }),
    ...
    },
    {
      validator: () => {
        ...
        if (props.rowData.clusterType === ClusterTypes.MONGO_REPLICA_SET) {
          if (clusterNodeCount[props.rowData.clusterId].mongodb.some((mongodbItem) => mongodbItem > 1)) {
            return t('同一个副本集，一次只能替换一个节点');
          }
          return true;
        }
        if (clusterNodeCount[props.rowData.clusterId].mongo_config.some((mongoConfigItem) => mongoConfigItem > 1)) {
          return t('config一次只能替换一个节点');
        }
        if (clusterNodeCount[props.rowData.clusterId].mongodb.some((mongoConfigItem) => mongoConfigItem > 1)) {
