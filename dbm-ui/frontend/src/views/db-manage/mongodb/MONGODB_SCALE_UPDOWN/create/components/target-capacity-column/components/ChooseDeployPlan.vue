      ...
      <div class="capacity-panel">
        <div class="panel-row">
          <div class="column">
            ...
            <div class="content">
              {{ cluster.mongodb[0].spec_config.name }}
            </div>
          </div>
          <div class="column">
      ...
      render: ({ index, data }: { index: number, data: ClusterSpec }) => (
        <TextOverflowLayout>
          {{
            ...
            append: () => {
              if (props.cluster.mongodb[0].spec_config.id === data.spec_id) {
                return (
                  <bk-tag
                    size="small"
                    class='ml-2'
  ...
  const handleSearchClusterSpec = async () => {
    ...
    if (capacityNeed.value > 0) {
      ...
      const params = {
        ...
        shard_num: props.cluster.shard_num,
      };
      ...
      const list = retArr.map(item => ({
        ...item,
        shard_num: props.cluster.shard_num,
        shard_node_count: props.cluster.shard_node_count,
        machine_need_num: item.machine_pair * props.cluster.shard_node_count,
