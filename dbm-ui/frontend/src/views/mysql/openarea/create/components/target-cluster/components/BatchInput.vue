...
<script setup lang="ts">
  ...
  type TendbhaModel = ServiceReturnType<typeof getTendbhaList>['results'][number];
  ...
  const placeholder = t('请分别输入目标集群（单个），一个或多个变量值，授权IP（可为多个英文逗号分隔）\n多个对象 ，换行分隔');
  ...
  const handleConfirm = async () => {
    ...
    const clusterInfoMap = clusterInfoResults.results.reduce((results, item) => {
      Object.assign(results, {
        [item.master_domain]: item,
      });
      return results;
    }, {} as Record<string, TendbhaModel>);
