...
<template>
  <span
    v-bk-tooltips="{
      content: t('暂不支持'),
      disabled: isResourcePool || isSupported,
    }">
    <BkButton
      :disabled="!isResourcePool || !isSupported"
      text
...
<script setup lang="ts">
  ...
  const isResourcePool = computed(() => {
    ...
    return ticketTypeRouteNameMap[props.data.ticket_type] === props.data.ticket_type && isResourcePoolDetails;
  });
