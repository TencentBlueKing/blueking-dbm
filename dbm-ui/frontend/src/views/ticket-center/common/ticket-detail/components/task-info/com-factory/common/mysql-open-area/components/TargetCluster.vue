...
<template>
  <DbOriginalTable
    ...
    :data="tableData"
    :rowspan-and-colspan="tableMerge">
    <TableColumn
...
<script setup lang="tsx">
  ...
  const tableMerge = ({ col, row }: { col: any; row: RowData }) => {
    if (col.colKey === 'targetCluster') {
      const rowSpan = tableData.value.filter((item) => item.targetCluster === row.targetCluster).length;
