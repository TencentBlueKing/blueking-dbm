...
<script setup lang="ts">
  import { Select as BkSelect, Tag as BkTag } from 'bkui-vue';
  ...
  interface Exposes {
    /** 获取标签条件与匹配类型；校验失败返回 null */
    getValue: () => Promise<{ clusterTags: ClusterTagItem[]; matchType: TagMatchType } | null>;
    /** 重置 */
    reset: () => void;
  }
  ...
  defineExpose<Exposes>({
    async getValue() {
      ...
      // 校验：非任意值时必须选择至少一个具体值
      if (!isAnyValue.value && selectedValues.value.length === 0) {
        return null;
      }

      let clusterTags: ClusterTagItem[];
      let matchType: TagMatchType;

      if (isAnyValue.value) {
        // 任意值：单条，tag_value 固定为 '任意值'
        clusterTags = [
          {
            id: props.getTagId(selectedKey.value, TAG_ANY_VALUE),
            tag_key: selectedKey.value,
            tag_value: TAG_ANY_VALUE,
          },
        ];
        matchType = 'exists';
        ...
        clusterTags = [
          {
            id: props.getTagId(selectedKey.value, selectedValues.value[0]),
            tag_key: selectedKey.value,
            tag_value: selectedValues.value[0],
          },
        ];
        matchType = 'single';
      } else {
        // 多值 in：同 tag_key 多条
        clusterTags = selectedValues.value.map((value) => ({
