...
<script setup lang="ts">
  ...
  watch([
    ...
    if (specHotRef.value && specColdRef.value) {
      ...
      const hotDisk = hotStorageSpec.reduce((total, item) => (
        total + Number(item.size || 0)
      ), 0);
      const coldDisk = coldStorageSpec.reduce((total, item) => (
        total + Number(item.size || 0)
      ), 0);
  ...
  const followerDisableDialogSubmitMethod = (hostList: HostDetails[]) => (hostList.length === 3 ? false : t('需要n台', { n: 3 }));
    ...
    formRef.value!.validate()
      .then(() => {
        if (tipTheme.value === 'danger' && formData.details.ip_source === 'resource_pool') {
          return Promise.reject(t('请保证冷/热节点至少存在一台'));
        }
        baseState.isSubmitting = true;

        const nodeList = (ipList: Array<HostDetails>) => ipList.map(item => ({
        ...
        const getDetails = () => {
          const { details }: { details: Record<string, any> } = formData;
          ...
          return {
            ...details,
            nodes: {
              follower: nodeList(formData.details.nodes.follower),
              observer: nodeList(formData.details.nodes.observer),
              hot: nodeList(formData.details.nodes.hot),
              cold: nodeList(formData.details.nodes.cold),
            },
          };
        };
