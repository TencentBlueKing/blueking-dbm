  ...
  <BkSideslider
    ...
    <div class="main-box">
      ...
      <BkInput
        disabled
        size="default"
        :value="data.targets.map(item => item.rule.key).join(',')" />
      ...
      <BkSelect
        ...
        disabled
        multiple-mode="tag">
        <BkOption>{{ data.notify_groups }}</BkOption>
        <template #tag>
          <div class="notify-tag-box">
            <DbIcon
              style="font-size: 16px"
              type="auth" />
            <span class="dba">{{ data.notify_groups }}</span>
            <DbIcon
              class="close-icon"
              type="close" />
          </div>
...
<script setup lang="tsx">
  ...
  const monitorTarget = computed(() => props.data.targets);
  const nofityTarget = computed(() => props.data.notify_groups);
