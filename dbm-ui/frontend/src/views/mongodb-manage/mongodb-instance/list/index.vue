  ...
  <div class="instance-list-page">
    <div class="header-action">
      <BkDropdown
        ...
        <template #content>
          <BkDropdownMenu>
            <BkDropdownItem @click="handleGoApply('mongoReplicaSet')">
              {{ t('副本集集群') }}
            </BkDropdownItem>
            <BkDropdownItem @click="handleGoApply('mongoShardedCluster')">
              {{ t('分片集群实例') }}
            </BkDropdownItem>
          </BkDropdownMenu>
        </template>
...
<script setup lang="tsx">
  ...
  interface Emits {
    (e: 'getPayload', value: MongodbInstanceModel): void
  }
  ...
  const columns = computed(() => {
    const list = [
      ...
      },
      {
        ...
        render: ({ data }: { data: MongodbInstanceModel }) => (
          <div class="text-overflow" v-overflow-tips>
            <router-link
              to={{
                name: MongodbInstanceModel.staticClusterType === 'replicaset'
      ...
      },
      {
        ...
        filter: {
          list: Object.keys(clusterMap).map(id => ({
            text: t(clusterMap[id]), value: id,
          })),
        },
      },
      {
          ...
          <>
            <OperationStatusTips class="mr8">
              <bk-button
                text
                theme='primary'
                onClick={ () => changesState(data, true) }>
                  { t('重启') }
              </bk-button>
            </OperationStatusTips>
            <OperationStatusTips>
              <bk-button
                text
                theme='primary'
                onClick={ () => changesState(data, false) }>
                  { t('禁用') }
              </bk-button>
            </OperationStatusTips>
          </>
  ...
  const handleGoApply = (type: string) => {
    router.push({
      name: type === 'mongoReplicaSet' ? 'mongoDBReplicaSetApply' : 'mongoDBSharedClusterApply',
    });
  };
