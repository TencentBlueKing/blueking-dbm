...
<script setup lang="tsx" generic="T extends SpiderModel">
  ...
  interface Emits {
    (e: 'change', value: Record<string, Record<string, RowData>>): void,
  }

  type RowData = ValueOf<Props['selected']>[0];
  ...
  const selectedMap = shallowRef<Record<string, Record<string, RowData>>>({});
  ...
  const {
    ...
    handleChangePage,
    handeChangeLimit,
  } = useClusterData<RowData>(activeTab, searchSelectValue);
  ...
  const checkSelectedAll = () => {
    if (tableData.value.filter(data => !data.isClusterNormal).length > 0) {
      nextTick(() => {
   ...
   * 选择当行数据
   */
  const handleSelecteRow = (data: RowData, value: boolean) => {
  ...
  const handleRowClick = (row:any, data: RowData) => {
