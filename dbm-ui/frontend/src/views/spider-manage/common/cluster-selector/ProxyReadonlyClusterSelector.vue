...
<script setup lang="tsx">
  ...
  const props = withDefaults(defineProps<Props>(), {
    isShow: false,
    selected: () => ({ [ClusterTypes.SPIDER]: [] }),
    tabList: () => [ClusterTypes.SPIDER],
  });
  ...
  let rawTableData: SpiderModel[] = [];
  /** tabs 功能 */
  const tabState = reactive({
    showTips: false,
  });
  const tabTextMap: Record<string, string> = {
    [ClusterTypes.SPIDER]: t('集群选择'),
  };
  const tabs = computed(() => {
    const { tabList } = props;
  ...
  function initData() {
    const clusterType = ClusterTypes.SPIDER;
    return {
  ...
  const handleClickSearch = async () => {
    ...
    if (keyword) {
      ...
      const filterList = rawTableData.filter(item => regex.test(item.master_domain));
   ...
   * 选择当前行数据
   */
  function handleSelected(data: SpiderModel, value: boolean) {
  ...
  function handleRowClick(_:number, data: SpiderModel) {
    if (data.spider_slave.length > 0) {
      return;
    }
