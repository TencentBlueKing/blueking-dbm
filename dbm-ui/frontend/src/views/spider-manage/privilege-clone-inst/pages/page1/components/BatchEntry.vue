    ...
    <div class="proxy-replace-batch-entry">
      ...
      <div class="error-box">
        ...
        <span v-if="inputHostErrorStack.length > 0">
          ...
          <span>{{ $t('n处目标Proxy不存在', [inputHostErrorStack.length]) }}</span>
          <DbIcon
            class="action-btn"
            type="audit"
...
<script setup lang="ts">
  ...
  const placeholder = t('请分别输入目标Proxy_新Proxy主机_多个对象_换行分隔');
  ...
  const handleCopy = () => {
    execCopy('127.0.0.1:8000    127.0.0.2\n');
  };
  ...
  const handleSubmit = () => {
    ...
    inputRecordList.forEach((recordItem) => {
      ...
      const [originIp, targetIp] = ipList;
      const payload = {
        originIp,
        targetIp,
      };

      if (!ipPort.test(originIp) || !ipv4.test(targetIp)) {
