...
<script setup lang="ts">
  ...
  defineOptions({
    name: 'CloneModuleLevelConfigTable',
  });
  ...
  // ====== 计算属性 ======/** 判断一行是否为自定义（level_name 匹配） */
  const isCustomRow = (row: CloneConfItem): boolean => row.level_name === 'module';
  /** 各类数量统计 */
  const counts = computed(() => ({
    custom: allItems.value.filter((i) => i.level_name === 'module').length,
  }));
