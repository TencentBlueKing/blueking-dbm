      ...
      <BkSelect
        ...
        <template #trigger>
          <div
            ...
            :class="{ 'is-active': isShowVersionPanel, 'is-disabled': versionSelectDisabled }">
...
<script setup lang="ts">
  ...
  const versionSelectDisabled = ref(false);
  ...
  watch(
    ...
    () => {
      ...
      if (props.data) {
        localData.value.permit_os = !props.data.permit_os?.length ? ['all'] : props.data.permit_os;
        localData.value.permit_os_type = props.data.permit_os_type || '';
        if (localData.value.permit_os_type) {
          setTimeout(() => {
            handleSystemChange(localData.value.permit_os_type);
          });
        }
      }
    },
  ...
  const handleSystemChange = (value: string) => {
    versionSelectDisabled.value = value === 'all';
    if (versionSelectDisabled.value) {
      versionList.value = [];
      localData.value.permit_os = [];
      return;
    }
