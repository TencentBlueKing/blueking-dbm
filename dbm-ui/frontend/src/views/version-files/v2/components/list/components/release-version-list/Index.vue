    ...
    <div
      ...
      <ScrollFaker ref="scrollFakerRef">
        <div
          ...
          <div class="count">{{ item.version_series_count }}</div>
...
<script setup lang="ts">
  ...
  type ReleaseItem = Awaited<ReturnType<typeof getReleaseVersionList>>[number];
  ...
  const releaseListRef = ref<HTMLElement>();
  ...
  const handleSuccess = () => {
    ...
    if (!isEditRelease.value) {
      setTimeout(() => {
        const scrollHeight = releaseListRef.value!.scrollHeight;
        scrollFakerRef.value?.scrollTo(0, scrollHeight + 100);
      }, 500);
    }
  };
