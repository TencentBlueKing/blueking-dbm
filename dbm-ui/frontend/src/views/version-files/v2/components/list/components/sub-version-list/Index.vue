    ...
    <template v-else>
      <div class="title-operate">
        <div class="title-operate-left">
          <template v-if="dbType === DBTypes.MYSQL">
            <div class="main-title">{{ releaseVersion?.name }}（{{ versionSeriesList?.length }}）</div>
      ...
      <div
        ...
        <ScrollFaker ref="scrollFakerRef">
          <SubVersion
            v-for="versionSeries in versionSeriesList"
            :key="versionSeries.id"
            ref="subVersionRefs"
            :data="versionSeries"
            @add-new-version="() => handleAddNewDbVersion(versionSeries)"
            @delete-version-series="fetchVersionSeriesList"
...
<script setup lang="ts">
  ...
  const scrollFakerRef = ref<InstanceType<typeof ScrollFaker>>();
  const subVersionRefs = ref<InstanceType<typeof SubVersion>[]>([]);
  const versionsListRef = ref<HTMLElement>();
  ...
  const handleSearchChange = (value: Record<string, string>) => {
    subVersionRefs.value.forEach((item) => {
      item.filterSearch({
        filter: value,
      });
    });
  ...
  const handleEditVersionSuccess = (versionSeriesId: number) => {
    ...
    if (index !== -1) {
      subVersionRefs.value[index]?.refresh();
    } else {
      emits('refreshReleaseList');
      fetchVersionSeriesList();
      setTimeout(() => {
        const scrollHeight = versionsListRef.value!.scrollHeight;
        scrollFakerRef.value?.scrollTo(0, scrollHeight + 500);
      }, 500);
    }
  };
