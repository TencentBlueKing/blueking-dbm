...
<script setup lang="tsx">
      ...
      render: ({ data }: { data: PermissionRuleExtend }) => (
        <div
          ...
          onClick={ () => handleToggleExpand(data) }>
            {
              data.rules.length > 1 && (
              <i class={ ['db-icon-down-shape user-icon', { 'user-icon-expand': data.isExpand }] } />
              )
            }
            <div class="user-name">
              <a
  ...
  } = useRequest(getAccountRulesList, {
    manual: true,
    onSuccess() {
      data.value = data.value && data.value.map(item => (Object.assign(item, { isExpand: item.rules.length > 1 })));
      accountMapList.value = data.value?.map(item => item.account);
    },
  });

  const { run: runDeleteAccount } = useRequest(deleteAccount, {
    manual: true,
    onSuccess() {
  ...
  const handleDeleteAccount = (data: PermissionRuleExtend) => {
    useInfoWithIcon({
      ...
      onConfirm: async () => {
        runDeleteAccount(data.account.account_id);
        return true;
      },
    });
  };
