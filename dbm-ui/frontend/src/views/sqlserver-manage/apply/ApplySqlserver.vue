<template>
        ...
        <DbCard
          ...
          <BkFormItem
            ...
            <BkInput
              ...
              :min="1025"
              type="number" />
            <span class="apply-form__tips ml-10">{{ t('默认从起始端口开始分配') }}</span>
          </BkFormItem>
        </DbCard>
        <DbCard :title="t('需求信息')">
          <BkFormItem
  ...
  <BkDialog
    ...
    <template #header>
      {{ t('实例预览') }}
      <span class="apply-dialog__quantity">{{ t('共n条') }}</span>
    </template>
    <template #footer>
      <BkButton>
        {{ t('关闭') }}
...
<script setup lang="tsx">
  ...
  import DomainTable from './components/SqlserverDomainTable.vue';

  type ModuleType = ServiceReturnType<typeof getModules>
  ...
  const moduleName = computed(() => {
    const module = (moduleList as unknown as ModuleType)
      .find(item => item.db_module_id === formData.details.db_module_id);
  ...
  const {
    ...
    manual: true,
    onAfter: () => {
      levelConfigList.value = (levelConfigData as unknown as ConfigBaseDetails).conf_items ;
    },
  });
