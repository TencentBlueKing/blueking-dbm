...
export function getDorisList(params: {
  ...
  return (
    http
      .get<ListBase<DorisModel[]>>('http://127.0.0.1:8083/mock/20/apis/bigdata/bizs/3/doris/doris_resources', params)
      // return http.get<ListBase<DorisModel[]>>(`${path}/`, params)
      .then((data) => ({
        ...data,
...
export function getDorisTableFields() {
  return http.get<ListBase<DorisModel[]>>(`${path}/get_table_fields/`);
}
...
export function getDorisInstanceList(params: {
  ...
  return (
    http
      .get<ListBase<DorisInstanceModel[]>>(
        'http://127.0.0.1:8083/mock/20/apis/bigdata/bizs/3/doris/doris_resources/1/list_instances',
        params,
      )
      // return http.get<ListBase<DorisInstanceModel[]>>(`${path}/list_instances/`, params)
      .then((data) => ({
        ...data,
...
export function retrieveDorisInstance(params: { bk_biz_id: number }) {
  return http.get<ListBase<DorisModel[]>>(`${path}/retrieve_instance/`, params);
}
...
export function getDorisDetail(params: { id: number }) {
  return (
    http
      .get<DorisModel>('http://127.0.0.1:8083/mock/20/apis/bigdata/bizs/3/es/es_resources/91/')
      // return http.get<DorisModel>(`${path}/${params.id}/`)
      .then((data) => new DorisModel(data))
  );
}
...
export function getDorisNodes(params: { cluster_id: number }) {
  return http.get<ListBase<DorisModel[]>>(`${path}/${params.cluster_id}/get_nodes/`);
}
...
export function getDorisTopoGraph(params: { cluster_id: number }) {
  return http.get<ListBase<DorisModel[]>>(`${path}/${params.cluster_id}/get_topo_graph/`);
}
...
export function getDorisPassword(params: { cluster_id: number }) {
  return http
    .get<DorisPasswordModel>(`${path}/${params.cluster_id}/get_password/`)
...
export function getDorisNodeList(params: { bk_biz_id: number; cluster_id: number; no_limit: number }) {
  // return http.get<ListBase<Array<DorisNodeModel>>>(`${path}/${params.cluster_id}/list_nodes/`, params)
  return http
    .get<
      ListBase<Array<DorisNodeModel>>
    >('http://127.0.0.1:8083/mock/20/apis/bigdata/bizs/3/doris/doris_resources/1/list_nodes', params)
    .then((data) => ({
      ...data,
      results: data.results.map(
        (item) =>
