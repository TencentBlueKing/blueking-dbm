...
export const findRelatedClustersByInstances = (
  params: {
    instances: Array<{
      bk_cloud_id: number;
      ip: string;
...
) => http.post(`/apis/mysql/bizs/${params.bk_biz_id}/cluster/find_related_clusters_by_instances/`, params);

export const getIntersectedSlaveMachinesFromClusters = (params: { bk_biz_id: number; cluster_ids: number[] }) =>
  http.post<
    Array<{
      bk_biz_id: number;
      bk_cloud_id: number;
...
) => http.post<MySQLClusterInfos[]>(`/apis/mysql/bizs/${params.bk_biz_id}/cluster/query_clusters/`, params);

// 批量下载文件
export const batchFetchFile = (params: { file_path_list: string[] }) =>
  http.post<
    Array<{
      content: string;
      path: string;
...
export const getRemoteParis = function (params: { cluster_ids: number[] }) {
...
export const getRemoteMachineInstancePair = function (params: { instances?: string[]; machines?: string[] }) {
  const { currentBizId } = useGlobalBizs();
  return http.post<{
