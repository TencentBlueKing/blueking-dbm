...
export default class SurrealdbHa extends ClusterBase {
  ...
  constructor(payload = {} as SurrealdbHa) {
    super(payload);
  ...
  get operationStatusIcon() {
    return SurrealdbHa.operationIconMap[this.operationRunningStatus];
  }

  get operationStatusText() {
    return SurrealdbHa.operationTextMap[this.operationRunningStatus];
  }

  get operationTagTips() {
    return this.operations.map((item) => ({
      icon: SurrealdbHa.operationIconMap[item.ticket_type],
      ticketId: item.ticket_id,
      tip: SurrealdbHa.operationTextMap[item.ticket_type],
    }));
  }
  ...
  get runningOperation() {
    const operateTicketTypes = Object.keys(SurrealdbHa.operationTextMap);
