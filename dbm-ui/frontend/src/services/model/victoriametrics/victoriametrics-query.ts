...
export default class VictoriametricsQuery extends ClusterBase {
  static operationIconMap: Record<string, string> = {
    ...
    [TicketTypes.K8S_VICTORIAMETRICS_ENABLE]: t('启用中'),
    [TicketTypes.K8S_VICTORIAMETRICS_RESTART]: t('重启中'),
  };

  static operationTextMap: Record<string, string> = {
    [TicketTypes.K8S_VICTORIAMETRICS_DESTROY]: t('删除任务进行中'),
    [TicketTypes.K8S_VICTORIAMETRICS_DISABLE]: t('禁用任务进行中'),
    [TicketTypes.K8S_VICTORIAMETRICS_ENABLE]: t('启用任务进行中'),
    [TicketTypes.K8S_VICTORIAMETRICS_RESTART]: t('重启任务进行中'),
  };
  ...
  permission: {
    k8s_victoriametrics_destroy: boolean;
    k8s_victoriametrics_edit: boolean;
    k8s_victoriametrics_manage: boolean;
    k8s_victoriametrics_view: boolean;
    k8s_vm_enable_disable: boolean;
  };
  ...
  get isOfflineOperationRunning() {
    return ([TicketTypes.K8S_VICTORIAMETRICS_ENABLE, TicketTypes.K8S_VICTORIAMETRICS_DESTROY] as string[]).includes(
