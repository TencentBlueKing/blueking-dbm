...
export default class QdrantHa extends ClusterBase {
  static operationIconMap: Record<string, string> = {
    [TicketTypes.K8S_QDRANT_DELETE]: t('删除中'),
    [TicketTypes.K8S_QDRANT_DISABLE]: t('禁用中'),
    [TicketTypes.K8S_QDRANT_ENABLE]: t('启用中'),
    [TicketTypes.K8S_QDRANT_RESTART]: t('重启中'),
  };

  static operationTextMap: Record<string, string> = {
    [TicketTypes.K8S_QDRANT_DELETE]: t('删除任务进行中'),
    [TicketTypes.K8S_QDRANT_DISABLE]: t('禁用任务进行中'),
    [TicketTypes.K8S_QDRANT_ENABLE]: t('启用任务进行中'),
    [TicketTypes.K8S_QDRANT_RESTART]: t('重启任务进行中'),
  };
  ...
  constructor(payload = {} as QdrantHa) {
    super(payload);
  ...
  get isOfflineOperationRunning() {
    return ([TicketTypes.K8S_QDRANT_ENABLE, TicketTypes.K8S_QDRANT_DELETE] as string[]).includes(
  ...
  get isStarting() {
    return Boolean(this.operations.find((item) => item.ticket_type === TicketTypes.K8S_QDRANT_ENABLE));
  }
  ...
  get operationStatusIcon() {
    return QdrantHa.operationIconMap[this.operationRunningStatus];
  }

  get operationStatusText() {
    return QdrantHa.operationTextMap[this.operationRunningStatus];
  }

  get operationTagTips() {
    return this.operations.map((item) => ({
      icon: QdrantHa.operationIconMap[item.ticket_type],
      ticketId: item.ticket_id,
      tip: QdrantHa.operationTextMap[item.ticket_type],
    }));
  }
  ...
  get runningOperation() {
    const operateTicketTypes = Object.keys(QdrantHa.operationTextMap);
