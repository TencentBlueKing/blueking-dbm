...
export default class Doris {
  ...
  static operationIconMap = {
    [Doris.ES_SCALE_UP]: t('扩容中'),
    [Doris.ES_SHRINK]: t('缩容中'),
    [Doris.ES_REPLACE]: t('替换中'),
    [Doris.ES_ENABLE]: t('启用中'),
    [Doris.ES_DISABLE]: t('禁用中'),
    [Doris.ES_DESTROY]: t('删除中'),
    [Doris.ES_REBOOT]: t('重启中'),
  };

  static operationTextMap = {
    [Doris.ES_SCALE_UP]: t('扩容任务进行中'),
    [Doris.ES_SHRINK]: t('缩容任务进行中'),
    [Doris.ES_REPLACE]: t('替换任务进行中'),
    [Doris.ES_ENABLE]: t('启用任务进行中'),
    [Doris.ES_DISABLE]: t('禁用任务进行中'),
    [Doris.ES_DESTROY]: t('删除任务进行中'),
    [Doris.ES_REBOOT]: t('实例重启任务进行中'),
  };
  ...
  creator: string;
  domain: string;
  doris_datanode_cold: Array<Node>;
  doris_datanode_hot: Array<Node>;
  ...
  constructor(payload = {} as Doris) {
    ...
    this.doris_datanode_cold = payload.doris_datanode_cold;
    this.doris_datanode_hot = payload.doris_datanode_hot;
  ...
  get isStarting() {
    return Boolean(this.operations.find((item) => item.ticket_type === Doris.ES_ENABLE));
  }
  ...
  get domainDisplayName() {
    const port = this.doris_datanode_hot[0]?.port;
