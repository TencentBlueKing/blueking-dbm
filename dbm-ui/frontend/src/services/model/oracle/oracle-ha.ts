...
export default class OracleHaCluster extends ClusterBase {
  ...
  constructor(payload: OracleHaCluster) {
    super(payload);
    ...
    this.standbys = payload.standbys || [];
  ...
  get allInstanceList() {
    return [...this.standbys, ...this.primaries];
  }
  ...
  get dbStatusConfigureObj() {
    const text = OracleHaCluster.statusMap[this.status] || '--';
    const theme = OracleHaCluster.themes[this.status] || 'danger';
    return {
      text,
      theme,
    };
  ...
  // get isStarting() {
  //   return Boolean(this.operations.find((item) => item.ticket_type === OracleHaCluster.SQLSERVER_ENABLE));
  // }
  ...
  get roleFailedInstanceInfo() {
    return {
      ...
      Slave: ClusterBase.getRoleFaildInstanceList(this.standbys),
    };
  }
