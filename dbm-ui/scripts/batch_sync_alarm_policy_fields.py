        ...
        for policy in all_policies:
            ...
            if policy.target_level == "platform":
                ...
                if policy.parent_id:
                    ...
                    if test_rules_is_eq and notify_rules_is_eq:
                        policy_tag = "inner"
                    else:
                        policy_tag = "custom"
            ...
            update_data["notify_config"] = {
                ...
                "voice_notice": policy.details["notice"]["config"].get("voice_notice") or "parallel",
            }
            ...
            if policy.custom_conditions:
                for condition in policy.custom_conditions:
                    if condition.get("value"):
                        custom_key[condition["key"]] = {
                            "rule": {
                                "key": condition["key"],
            ...
            new_targets = [target for target in old_targets if target.get("rule", {}).get("key") not in custom_key]
            new_targets.extend(custom_key.values())
            update_data["targets"] = new_targets
            if update_data:
