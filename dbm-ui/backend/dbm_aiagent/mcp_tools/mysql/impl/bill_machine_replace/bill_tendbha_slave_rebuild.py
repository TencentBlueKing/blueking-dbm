...
from backend.ticket.builders.mysql.mysql_restore_local_slave import MysqlRestoreLocalSlaveDetailSerializer
from backend.ticket.constants import TicketType
from backend.ticket.models import Ticket


@bill_response_wrapper
def bill_tendbha_slave_rebuild(username: str, cluster_domains: List[str], ips: List[str]):
    """
    创建 TenDBHA 存储 slave 原地重建单据。

    - 每个 slave 实例（ip:port）一行，一行对应一个 slave 实例 + 所属集群；
    - 原地重建不申请新资源（无 resource_spec / ip_source），在原机重建实例；
    ...
    # 找到所有输入 IP 对应的 slave 实例（TenDBHA 存储层 slave）
    slave_objs = list(
    ...
    if related_cluster_ids != input_cluster_ids:
        ...
        raise DBMMcpBaseException(
            msg=_("输入集群与 slave 实例承载集群不一致, 缺少={}, 多余={}").format(sorted(missing_clusters), sorted(extra))
        )

    # 每个 slave 实例（ip:port）一行，一行对应一个实例 + 所属集群
    ...
    ticket_param = {
        "ticket_type": TicketType.MYSQL_RESTORE_LOCAL_SLAVE,
        "creator": username,
        "helpers": [],
        "bk_biz_id": bk_biz_id,
        "remark": TicketType.MYSQL_RESTORE_LOCAL_SLAVE,
        "details": {
            "force": False,
    ...
    slz = MysqlRestoreLocalSlaveDetailSerializer(data=ticket_param["details"])
    slz.context["ticket_type"] = TicketType.MYSQL_RESTORE_LOCAL_SLAVE
