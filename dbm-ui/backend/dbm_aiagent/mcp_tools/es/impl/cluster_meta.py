...
def es_list_clusters(bk_biz_id: int) -> List:
    """查询业务下的 ES 集群列表"""
    clusters = Cluster.objects.filter(bk_biz_id=bk_biz_id, cluster_type=ClusterType.Es,).annotate(
        master_count=Count("storageinstance", filter=Q(storageinstance__instance_role=InstanceRole.ES_MASTER.value)),
        hot_count=Count(
            "storageinstance", filter=Q(storageinstance__instance_role=InstanceRole.ES_DATANODE_HOT.value)
        ),
        cold_count=Count(
            "storageinstance", filter=Q(storageinstance__instance_role=InstanceRole.ES_DATANODE_COLD.value)
        ),
        client_count=Count("storageinstance", filter=Q(storageinstance__instance_role=InstanceRole.ES_CLIENT.value)),
    )
...
def _get_spec_details(spec_keys: List, specs: Dict = None) -> Dict:
    """获取规格详细信息；可传入已查询的 specs 缓存以避免重复查询"""
    if not spec_keys:
        return {}
    ...
    return {
        spec_id: {
            "spec_id": spec_id,
            "spec_name": specs[spec_id].spec_name,
            "cpu": specs[spec_id].cpu,
            "memory": specs[spec_id].mem,
            "storage_spec": specs[spec_id].storage_spec,
            "device_class": specs[spec_id].device_class,
            "desc": specs[spec_id].desc,
        }
...
def _build_role_stats(instances, role_label: str) -> Dict:
    ...
    return {
        ...
        "spec_details": _get_spec_details(list(machine_stats["spec_summary"].keys()), specs),
        "nodes": nodes,
    }
