...
class RedisClusterReport:
    """Redis集群报告, 用sub_type区分检查子项"""

    cluster: Cluster
    report_day: int
    subtype: str
    records: dict[str, list[RedisCheckReport]]

    def __init__(self, cluster: Cluster, report_day: int, subtype: str):
    ...
    def make_skip_record(self, reason: str) -> list[RedisCheckReport]:
        """生成跳过记录"""
        return [
            RedisCheckReport(
                creator="",
...
class RedisCheckReportBatchOps:
    """记录批量操作类 用于批量操作记录, 用于处理failed_days字段"""

    records: list[RedisCheckReport] = []
    sub_type: str
    report_day: int
    ...
    def append(self, record: RedisCheckReport):
    ...
    def bulk_create(self):
        ...
        RedisCheckReport.objects.bulk_create(self.records)
        self.records = []
    ...
    def get_continuous_key(self, row: RedisCheckReport) -> str:
        return f"{row.cluster_id}:{row.shard}:{row.instance}:{row.state}"

    def get_continuous_days(self):
        rows = RedisCheckReport.objects.filter(
