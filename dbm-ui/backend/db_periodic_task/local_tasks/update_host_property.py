...
@register_periodic_task(run_every=crontab(hour=1, minute=0))
def migrate_hosts_to_pool():
    """
    补录主机池信息，主要是填充在machine / 资源池但没记录到主机池的主机
    这些漏网的主机无法进入主机管控 TODO: 暂不注册到定时任务
    """
    exist_hosts_ids = list(DirtyMachine.objects.all().values_list("bk_host_id", flat=True))

    # 补录machine表中所有主机
    bk_host_ids = list(Machine.objects.all().values_list("bk_host_id", flat=True))
    new_host_ids = list(set(bk_host_ids) - set(exist_hosts_ids))
    new_hosts = [{"bk_host_id": host_id} for host_id in new_host_ids]

    logger.info("machine not in host pool ids: %s", new_hosts)

    size = 200
    batch = math.ceil(len(new_hosts) / size)

    for page in range(0, batch):
        hosts = new_hosts[page * size : (page + 1) * size]
    ...
    resource_data = DBResourceApi.resource_list(params=params)["details"]
    resource_host_ids = [resource["bk_host_id"] for resource in resource_data]
    new_host_ids = list(set(resource_host_ids) - set(exist_hosts_ids))
    new_hosts = [{"bk_host_id": host_id} for host_id in new_host_ids]

    logger.info("resource not in host pool ids: %s", new_hosts)
