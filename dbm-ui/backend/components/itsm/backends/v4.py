...
class ItsmV4Backend(BaseItsmBackend):
    ...
    def normalize_ticket_logs(cls, logs):
        ...
        if (
            len(log_list) >= 3
            and log_list[1].get("action") == "submit"
            and log_list[2].get("action") == "revoked"
            and log_list[2].get("operator") == ItsmV4Api.get_system_id()
        ):
            log_list[2]["operator"] = log_list[1].get("operator")
    ...
    def ticket_approval_result(self, params, *args, **kwargs):
        ...
        ticket_id_params = self.format_ticket_id_params(params)
        detail = ItsmV4Api.get_ticket_detail(ticket_id_params, *args, **kwargs)
        for _ in range(3):
            if detail.get("status", "").lower() != "draft":
                break
            # 提单后 ITSM 状态可能有延迟，这里等待后最多重试三次。
            time.sleep(1)
            detail = ItsmV4Api.get_ticket_detail(ticket_id_params, *args, **kwargs)
