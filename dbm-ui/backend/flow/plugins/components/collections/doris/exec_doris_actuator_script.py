...
from backend.flow.utils.doris.doris_script_template import actuator_template, fast_execute_script_common_kwargs

logger = logging.getLogger("json")


class ExecuteDorisActuatorScriptService(BkJobService):
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        kwargs 字典传入格式：
        {
           ...
           get_doris_payload_func : 表示获取执行 es的db-actuator 参数方法名称，对应DorisActPayload类
           exec_ip: 表示执行的ip节点
        }
        """
...
class ExecuteDorisActuatorScriptComponent(Component):
    name = __name__
    code = "doris_actuator_execute"
    bound_service = ExecuteDorisActuatorScriptService
