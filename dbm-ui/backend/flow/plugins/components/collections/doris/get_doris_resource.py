...
class GetDorisResourceService(BaseService):
    """
    根据Doris单据获取DB资源,在资源池获取节点资源,并根据单据类型拼接对应的参数
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        for role in global_data["nodes"]:
            if role == DorisRoleEnum.HOT:
                ...
                trans_data.new_observer_ips = [node["ip"] for node in global_data["nodes"][role]]
...
class GetDorisResourceComponent(Component):
    name = __name__
    code = "get_doris_resource"
    bound_service = GetDorisResourceService
