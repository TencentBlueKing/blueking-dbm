...
from backend.flow.consts import OracleFlowEnum
from backend.flow.plugins.components.collections.common.base_service import BaseService

logger = logging.getLogger("flow")


class GetOracleResourceService(BaseService):
    """
    根据Oracle单据获取DB资源,在资源池获取节点资源,并根据单据类型拼接对应的参数
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        if global_data["ticket_type"] == OracleFlowEnum.KAFKA_REPLACE:
        ...
        elif global_data["ticket_type"] == OracleFlowEnum.KAFKA_SCALE_UP:
...
class GetOracleResourceComponent(Component):
    name = __name__
    code = "get_kafka_resource"
    bound_service = GetOracleResourceService
