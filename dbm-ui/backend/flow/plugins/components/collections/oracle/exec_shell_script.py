...
from backend.flow.utils.oracle.oracle_script_template import make_script_common_kwargs

logger = logging.getLogger("json")
cpl = re.compile("<ctx>(?P<context>.+?)</ctx>")  # 非贪婪模式，只匹配第一次出现的自定义tag


class OracleExecShellScriptService(BkJobService):
    """
...
class OracleExecShellScriptComponent(Component):
    name = __name__
    code = "oracle_exec_shell_script"
    bound_service = OracleExecShellScriptService
