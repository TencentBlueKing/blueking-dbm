...
from backend.flow.utils.oracle.oracle_script_template import oracle_actuator_template_v2
...
class ExecuteOracleActuatorScriptService(BkJobService):
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        kwargs 字典传入格式：
        {
           ...
           get_oracle_payload_func : 表示获取执行 oracle的db-actuator 参数方法名称，对应OracleActPayload类
           exec_ip: 表示执行的ip节点
        ...
        # 获取oracle actuator 组件所需要执行的参数
        oracle_act_payload = OracleActPayload(
        ...
        db_act_template = getattr(oracle_act_payload, kwargs["get_oracle_payload_func"])(
        ...
        template = jinja_env.from_string(oracle_actuator_template_v2)
...
class ExecuteOracleActuatorScriptComponent(Component):
    name = __name__
    code = "oracle_actuator_execute"
    bound_service = ExecuteOracleActuatorScriptService
