...
class KafkaApplySummarySerializer(BaseFlowOutputSerializer):
    """Kafka集群部署成功后的摘要信息定义，字段顺序即前端表格展示顺序，第一个字段同时作为表格行唯一标识"""

    table_name = "kafka_cluster_info"
    table_display_name = _("Kafka集群信息")
...
class KafkaApplySummaryService(BaseService):
    """
    Kafka集群部署成功后，将集群关键信息(地区/域名/端口)写入FlowSummary，供前端"执行摘要"展示。
    该节点需要在集群元数据创建之后执行，才能查询到集群信息。

    统一入参格式：kwargs = {"items": [{bk_biz_id, domain_name, region, version, port}, ...]}，
    单集群写入时items传一个元素即可(见add_kafka_apply_summary_output_act)。
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        for item in items:
            ...
            if cluster:
                summary_data["access_entry_url"] = "{}/{}/db-manage/kafka/detail/{}?open=access_entry".format(
        ...
        FlowOutputHandler(KafkaApplySummarySerializer).insert_data(root_id, summary_data_list)
        return True
...
class KafkaApplySummaryComponent(Component):
    name = __name__
    code = "kafka_apply_summary"
    bound_service = KafkaApplySummaryService


def add_kafka_apply_summary_output_act(
    kafka_pipeline,
    bk_biz_id: int,
    domain_name: str,
    region: str,
    ...
    """
    Kafka集群部署成功后，将集群关键信息(地区/域名/端口)写入FlowSummary，供前端"执行摘要"展示。
    该函数需要在"更新DBMeta元信息"之后调用。
    @param kafka_pipeline: 当前Kafka部署流程的Builder实例，节点会直接追加到该流程中
    ...
    kafka_pipeline.add_act(
        ...
        act_component_code=KafkaApplySummaryComponent.code,
        kwargs={"items": [item]},
    )
