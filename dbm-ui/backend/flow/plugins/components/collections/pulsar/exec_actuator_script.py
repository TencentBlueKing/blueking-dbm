...
class ExecutePulsarActuatorScriptService(BkJobService):
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        kwargs 字典传入格式：
        {
           ...
           get_pulsar_payload_func : 表示获取执行 pulsar的db-actuator 参数方法名称，对应PulsarActPayload类
           exec_ip: 表示执行的ip节点
        ...
        # 获取pulsar actuator 组件所需要执行的参数,
        db_act_template = getattr(trans_data.pulsar_act_payload, kwargs["get_pulsar_payload_func"])(
            ip=exec_ips[0],
            role=kwargs["role"],
            my_id=kwargs["zk_my_id"],
            # no need in prod env
            zk_host_map=kwargs["zk_host_map"],
            cur_bookie_num=kwargs["cur_bookie_num"],
...
class ExecutePulsarActuatorScriptComponent(Component):
    name = __name__
    code = "pulsar_actuator_execute"
    bound_service = ExecutePulsarActuatorScriptService
