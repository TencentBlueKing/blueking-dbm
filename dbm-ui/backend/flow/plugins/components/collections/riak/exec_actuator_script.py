...
class ExecuteRiakActuatorScriptService(BkJobService):
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        kwargs 字典传入格式：
        {
           ...
           get_riak_payload_func : 表示获取执行 riak的db-actuator 参数方法名称，对应RiakActPayload类
           exec_ip: 表示执行的ip节点
        ...
        # 获取riak actuator 组件所需要执行的参数
        riak_act_payload = RiakActPayload(
        ...
        db_act_template = getattr(riak_act_payload, kwargs["get_riak_payload_func"])(
...
class ExecuteRiakActuatorScriptComponent(Component):
    name = __name__
    code = "riak_actuator_execute"
    bound_service = ExecuteRiakActuatorScriptService
