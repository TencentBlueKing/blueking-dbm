...
class ExecuteHdfsActuatorScriptService(BkJobService):
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        # 获取hdfs actuator 组件所需要执行的参数,
        db_act_template = getattr(trans_data.hdfs_act_payload, kwargs["get_hdfs_payload_func"])(
            ip=exec_ips[0],
            transfer_target_ip=kwargs["transfer_target_ip"],
            hdfs_role=kwargs["hdfs_role"],
            trans_data=asdict(trans_data),
        )

        db_act_template["db_actor_cmd"] = "dbactuator"
...
class ExecuteHdfsActuatorScriptComponent(Component):
    name = __name__
    code = "hdfs_actuator_execute"
    bound_service = ExecuteHdfsActuatorScriptService
