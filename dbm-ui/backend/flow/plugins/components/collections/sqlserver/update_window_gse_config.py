...
class UpdateWindowGseConfig(BkSopsService):
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        param = {
            "template_id": env.UPDATE_WINDOW_GSE_CONFIG,
            "bk_biz_id": bk_biz_id,
            "template_source": "common",
            "name": _("更新window的gse配置信息"),
            "flow_type": "common",
            "constants": {
                "${biz_cc_id}": bk_biz_id,
                "${bk_biz_id}": bk_biz_id,
                "${job_ip_list}": "\n".join(ips),
                "${job_account}": WINDOW_SYSTEM_JOB_USER,
            },
        }
...
class UpdateWindowGseConfigComponent(Component):
    name = _("更新window的gse配置信息")
    code = "update_window_gse_config_for_sop"
    bound_service = UpdateWindowGseConfig
