...
from backend.flow.utils.vm.vm_script_template import actuator_template, fast_execute_script_common_kwargs

logger = logging.getLogger("json")


class ExecuteVmActuatorScriptService(BkJobService):
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        kwargs 字典传入格式：
        {
           ...
           get_vm_payload_func : 表示获取执行 vm的db-actuator 参数方法名称，对应VMActPayload类
           exec_ip: 表示执行的ip节点
        }
        """
        ...
        # 获取vm actuator 组件所需要执行的参数,
        db_act_template = getattr(trans_data.vm_act_payload, kwargs["get_vm_payload_func"])(
            ip=exec_ips[0],
            role=kwargs["vm_role"],
        )
...
class ExecuteVmActuatorScriptComponent(Component):
    name = __name__
    code = "vm_actuator_execute"
    bound_service = ExecuteVmActuatorScriptService
