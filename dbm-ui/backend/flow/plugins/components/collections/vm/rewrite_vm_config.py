...
from backend.flow.utils.vm.consts import VmConfigEnum

logger = logging.getLogger("flow")


class WriteBackVmConfigService(BaseService):
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        conf_items = [
            {
                "conf_name": VmConfigEnum.VmstorageRetentionPeriod,
                "conf_value": str(global_data["retention_period"]),
                "op_type": OpType.UPDATE,
            },
            {
                "conf_name": VmConfigEnum.VminsertReplicationFactor,
                "conf_value": str(global_data["replication_factor"]),
                "op_type": OpType.UPDATE,
            },
        ]
        DBConfigApi.upsert_conf_item(
            {
                ...
                "level_value": global_data["vminsert_domain"],
            }
        )
    ...
    def write_auth_to_prv_manager(self, global_data: dict):
        ...
        query_params = {
            "instances": [
                {"ip": global_data["vminsert_domain"], "port": 0, "bk_cloud_id": global_data["bk_cloud_id"]}
            ],
            "password": base64.b64encode(str(global_data["username"]).encode("utf-8")).decode("utf-8"),
            "username": MySQLPrivComponent.VM_FAKE_USER.value,
            "component": NameSpaceEnum.Vm,
            "operator": "admin",
        }
...
class WriteBackVmConfigComponent(Component):
    name = __name__
    code = "write_back_vm_config"
    bound_service = WriteBackVmConfigService
