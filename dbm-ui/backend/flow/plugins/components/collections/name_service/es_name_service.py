...
class EsExecNameServiceOperation(BaseService):
    """
    EsNameServiceCreate服务
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        # 主域名绑定clb ip
            ...
            res = clb.immute_domain_forward_to_clb_ip(cluster_id=cluster_id, creator=creator, bind=True)
        # 主域名解绑clb ip
        elif name_service_operation_type == "domain_unbind_clb_ip":
            res = clb.immute_domain_forward_to_clb_ip(cluster_id=cluster_id, creator=creator, bind=False)
        # clb信息写入meta
...
class EsExecNameServiceOperationComponent(Component):
    """
    EsExecNameServiceOperation组件
    """

    name = __name__
    code = "es_name_service_operation"
    bound_service = EsExecNameServiceOperation
