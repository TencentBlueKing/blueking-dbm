...
def _probe_gse_alive_once(
    ip: str, bk_cloud_id: int, log_fn: Optional[Callable[[str], None]] = None, attempt: int = 1
) -> Optional[bool]:
    """
...
def _probe_drs_login(
    ...
    if not ports:
        _probe_log(
            log_fn,
            "mongo autofix pre drs end ip={} drs_ok=False auth_err=False at={} elapsed_sec={:.1f} reason=empty_ports".format(
                ip, _now_str(), time.monotonic() - started
            ),
        )
        return False, False
    ...
    for port in ports:
        ...
        try:
            ...
            result = str(rpc_results).strip()
            try:
                ok_val = float(result)
            except (TypeError, ValueError):
                ok_val = None
            if ok_val != 1:
                logger.warning("mongo autofix pre drs isMaster.ok fail addr=%s result=%s", addr, result)
                all_ok = False
        except Exception as exc:  # noqa: BLE001
...
def _unlock_pre_for_followup(pre_ticket_id, cluster_ids: List[int], log_fn=None) -> None:
    """
    PRE Inner 仍在 RUNNING 时出后续单会撞集群互斥；先对 PRE 的操作记录解锁 RELOAD/ENSURE_START/FIX_STATUS/AUTOFIX。
    """
    if not pre_ticket_id:
        return
    unlock_types = [
        TicketType.MONGODB_INSTANCE_RELOAD.value,
        TicketType.MONGODB_INSTANCE_ENSURE_START.value,
        TicketType.MONGODB_INSTANCE_FIX_STATUS.value,
        TicketType.MONGODB_AUTOFIX.value,
        TicketType.MONGODB_AUTOFIX_MANUAL.value,
    ...
    def _handle_one(self, info: dict, global_data: dict) -> bool:
        ...
        if action == ACTION_MANUAL:
            ...
            if followup_ticket:
                ...
                write_autofix_log(MongoAutofixLogEvent.ERROR, "follow-up MANUAL ticket create failed", core=core)
                return False
            return True

        # REPLACE/RELOAD 才打 UNAVAILABLE；login_ok 修状态不要先把健康实例打挂
        if action in (ACTION_REPLACE, ACTION_RELOAD):
