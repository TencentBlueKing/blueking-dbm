...
class CtlSwitchToSlaveService(BaseService):
    ...
    def _new_master_enable_primary(self, cluster: Cluster, new_master, reduce_ctl_primary):
        ...
        reduce_primary_ip = reduce_ctl_primary.split(":")[0]
        reduce_primary_port = reduce_ctl_primary.split(":")[1]
        server_name = "test_name"
        select_sql = [
            "set tc_admin = 1",
            f"select Server_name from mysql.servers where host = '{reduce_primary_ip}' and port = {reduce_primary_port}",
        ]
