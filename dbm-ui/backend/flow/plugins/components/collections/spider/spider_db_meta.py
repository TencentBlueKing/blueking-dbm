...
class SpiderDBMetaService(BaseService):
    """
    定义spider集群单据类型来更新cmdb的活动节点
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        spider_meta = SpiderDBMeta(global_data=global_data, cluster=cluster_info)

        # 注入参数
        result = getattr(spider_meta, kwargs.get("db_meta_class_func"))(**kwargs.get("component_kwargs", {}))
...
class SpiderDBMetaComponent(Component):
    name = __name__
    code = "spider_db_meta"
    bound_service = SpiderDBMetaService
