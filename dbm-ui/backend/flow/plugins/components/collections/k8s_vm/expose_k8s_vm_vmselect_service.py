...
from backend.flow.utils.k8s_db.vm.consts import COMPONENT_VMSELECT, VMSELECT_PORT, VMSELECT_SERVICE_NAME

logger = logging.getLogger("flow")


class ExposeK8sVmVmselectServiceService(BaseService):
    """
    调用dbs接口暴露vmselect服务
    """
    ...
    def _schedule(self, data, parent_data, callback_data=None) -> bool:
        ...
        if attempt >= max_retries:
            ...
            self.log_error(f"vmselect expose {clb_id} failed after {max_retries} attempts. {error_msg}")
            self.finish_schedule()
            return False

        self.log_info(
            f"vmselect expose {clb_id} not ready yet, attempt {attempt}/{max_retries}. "
...
class ExposeK8sVmVmselectServiceComponent(Component):
    name = __name__
    code = "expose_k8s_vm_vmselect_service"
    bound_service = ExposeK8sVmVmselectServiceService
