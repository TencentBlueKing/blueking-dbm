...
class GetK8sVmVmselectClbDetailService(BaseService):
    """
    调用dbs接口查询vmselect CLB详情，使用pipeline schedule机制异步轮询
    """
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        data.outputs["clb_id"] = trans_data.vmselect_clb_id
    ...
    def _schedule(self, data, parent_data, callback_data=None) -> bool:
        ...
        if clb_item_list and clb_item_list[0].get("LoadBalancerVips"):
            trans_data.vmselect_clb_detail = clb_item_list[0]
            self.finish_schedule()
            self.log_info(f"CLB {clb_id} detail ready.")
            return True

        if attempt >= max_retries:
            self.log_error(_("获取vmselect CLB详情失败"))
            self.finish_schedule()
            return False
...
class GetK8sVmVmselectClbDetailComponent(Component):
    name = __name__
    code = "get_k8s_vm_vmselect_clb_detail"
    bound_service = GetK8sVmVmselectClbDetailService
