...
class ReleaseUnlockTicketTypeConfigService(BaseService):
    """
    添加释放解除单据互斥锁的配置动作
    """
    ...
    def _execute(self, data, parent_data):
        ...
        for cluster_id in kwargs.get("cluster_ids", []):
            ...
            # 判断是否提前释放单据互斥锁
            record.remove_unlock_ticket_type_config_operations(kwargs["release_unlock_ticket_type_list"])
...
class ReleaseUnlockTicketTypeConfigComponent(Component):
    name = _("添加解除单据互斥锁关系配置")
    code = "release_unlock_ticket_type_config"
    bound_service = ReleaseUnlockTicketTypeConfigService
