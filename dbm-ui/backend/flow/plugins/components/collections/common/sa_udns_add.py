...
from backend.flow.utils.base.base_dataclass import SAUdnsAddKwargs


class UdnsAddInMachineService(BkSopsService):
    ...
    def _execute(self, data, parent_data) -> bool:
        ...
        param = {
            ...
            "constants": {
                ...
                "${bk_biz_id}": bk_biz_id,
                "${job_ip_list}": "\n".join(ips),
                "${job_account}": kwargs["account_name"],
            },
        }
        rpdata = BkSopsApi.create_task(param)
        task_id = rpdata["task_id"]
        task_url = rpdata.get("task_url", "")
        # start task
        self.log_info(f"job url:{task_url}")
...
class UdnsAddInMachineComponent(Component):
    name = _("sa添加强制添加Udns")
    code = "udns_add_in_machine"
    bound_service = UdnsAddInMachineService
    kwargs = SAUdnsAddKwargs
