...
class SerialTransFileService(BkJobService):
    """
    串行下载介质文件包到目标机器
    {
        "root_id": "xxxx",
        "node_name": "xxxx",
        "node_id": "xxx",
        "exec_ip": ["1.1.1.1", "2.2.2.2"],  # 目标主机ip列表
        "source_ip_list": ["3.3.3.3"],
        "bk_cloud_id": 0,
        "file_list": ["/tmp/dba-toolkit.tar.gz", "/tmp/mysql-5.7.tar.gz"]
...
class SerialTransFileComponent(Component):
    name = __name__
    code = "exec_serial_trans_file"
    bound_service = SerialTransFileService
