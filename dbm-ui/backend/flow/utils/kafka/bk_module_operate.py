...
def transfer_host_in_cluster_module(
    ...
    bk_biz_id = Cluster.objects.get(id=cluster_ids[0]).bk_biz_id
    CcManage.transfer_host_module(bk_biz_id, transfer_bk_host_ids, transfer_bk_module_ids)
    ...
    for cluster in Cluster.objects.filter(id__in=cluster_ids):
        ...
        if machine_type == MachineType.BROKER.value:
        ...
        elif machine_type == MachineType.ZOOKEEPER.value:
            # 消费延迟
            ins = StorageInstance.objects.filter(cluster=cluster, machine__ip__in=ip_list)[0]
            init_instance_service(
                cluster=cluster,
                ins=ins,
                bk_module_id=bk_module_id,
