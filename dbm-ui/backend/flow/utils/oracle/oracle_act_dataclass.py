...
class ExecActuatorBaseKwargs:
    ...
    get_oracle_payload_func: str = None  # 上下文中MysqlActPayload类的获取参数方法名称。空则传入None
...
class UploadFile:
    ...
    path: str = None  # 文件路径
    content: str = None  # 文件内容(静态值)
    content_var: str = None  # 从上下文trans_data中获取content的变量名
    encoding: str = "text"  # 内容编码方式: "text" 直接按utf-8写入; "base64" 先base64解码后按二进制写入(用于密码文件等二进制文件)
