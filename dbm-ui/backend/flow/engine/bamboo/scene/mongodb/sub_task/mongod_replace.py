...
def _append_source_down_cutover(
    *,
    ...
    # get_host_replace 只给新机下发介质；sourceDown 在 peer 执行，需单独下发 actuator + 建目录
    peer_host = {
        "ip": exec_host["ip"],
        "bk_cloud_id": exec_host.get("bk_cloud_id", info["bk_cloud_id"]),
    }
    saved_hosts = list(sub_sub_get_kwargs.payload.get("hosts") or [])
    sub_sub_get_kwargs.payload["hosts"] = [peer_host]
    kwargs = sub_sub_get_kwargs.get_send_media_kwargs(media_type="actuator")
    sub_sub_pipeline.add_act(
        act_name=_("MongoDB-介质下发(peer)-{}".format(peer_host["ip"])),
        act_component_code=ExecSendMediaOperationComponent.code,
        kwargs=kwargs,
    )
    kwargs = sub_sub_get_kwargs.get_create_dir_kwargs()
    sub_sub_pipeline.add_act(
        act_name=_("MongoDB-创建原子任务执行目录(peer)-{}".format(peer_host["ip"])),
        act_component_code=ExecuteDBActuatorJobComponent.code,
        kwargs=kwargs,
    )
    sub_sub_get_kwargs.payload["hosts"] = saved_hosts
