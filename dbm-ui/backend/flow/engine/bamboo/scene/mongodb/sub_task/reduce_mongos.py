...
def reduce_mongos(root_id: str, ticket_data: Optional[Dict], sub_kwargs: ActKwargs, info: dict) -> SubBuilder:
    ...
    domain = sub_get_kwargs.payload["mongos_nodes"][0]["domain"]
    port = sub_get_kwargs.payload["mongos_nodes"][0]["port"]
    sub_get_kwargs.payload["mongos_nodes"] = [
        {"ip": mongos["ip"], "port": port, "bk_cloud_id": mongos["bk_cloud_id"], "domain": domain}
        for mongos in info["reduce_nodes"]
    ]
    sub_get_kwargs.mongos_info = {}
    sub_get_kwargs.mongos_info["nodes"] = info["reduce_nodes"]
    sub_get_kwargs.mongos_info["cluster_id"] = cluster_id
    sub_get_kwargs.payload["mongos"] = {}
    sub_get_kwargs.payload["mongos"]["domain"] = domain
    ...
    # 修改db_meta数据
    kwargs = sub_get_kwargs.get_change_meta_mongos_kwargs(increase=False)
    sub_pipeline.add_act(
    ...
    return sub_pipeline.build_sub_process(sub_name=_("MongoDB--{}减少mongos".format(sub_get_kwargs.payload["set_id"])))
