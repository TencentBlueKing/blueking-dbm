...
from backend.flow.utils.doris.consts import DORIS_FOLLOWER_MUST_COUNT, DORIS_OBSERVER_NOT_COUNT, DorisResourceTag
...
class DorisScaleUpFlow(DorisBaseFlow):
    ...
    def scale_up_doris_flow(self):
        ...
        # 单据data中enable_cold_storage为True时, 且不存在绑定独立集群资源，创建冷存储
        if self.data.get("enable_cold_storage"):
            resource_flow = DorisResourceFlow(root_id=self.root_id, data=scale_up_data)
            if resource_flow.cluster_exists_resource(res_tag=DorisResourceTag.PRIVATE):
                # 若集群已绑定存储资源，不报错，只打印warn日志
                # raise ResourceExistsException(cluster_name=scale_up_data["cluster_name"])
                logger.warning("cluster: {} exists private resource".format(scale_up_data["cluster_name"]))
            if resource_flow.data_exist_private_resource():
                # 若单据data里包含独立集群资源信息
                bind_private_pipeline = resource_flow.bind_exist_resource_sub_flow(
                    data=scale_up_data, res_info=resource_flow.data["private_res"]
                )
    ...
    def check_only_scale_up_resource(self, data: dict) -> bool:
        ...
        return not scale_ips and self.data.get("enable_cold_storage")
