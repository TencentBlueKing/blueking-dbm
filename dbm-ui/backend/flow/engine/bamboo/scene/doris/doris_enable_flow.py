...
class DorisEnableFlow(DorisBaseFlow):
    """
    构建Doris集群启用流程
    """
    ...
    def enable_doris_flow(self):
        """
        启用Doris集群
        """
        # Builder 传参 为封装好角色IP的数据结构
        enable_data = self.get_flow_base_data()
        doris_pipeline = Builder(root_id=self.root_id, data=enable_data)
        ...
        for ip in all_ips:
            # 启动节点进程
            act_kwargs.get_doris_payload_func = DorisActPayload.get_start_process_payload.__name__
            act_kwargs.exec_ip = ip
            act_kwargs.doris_role = DORIS_ROLE_ALL
            act = {
                "act_name": _("启动Doris集群进程-{}").format(ip),
