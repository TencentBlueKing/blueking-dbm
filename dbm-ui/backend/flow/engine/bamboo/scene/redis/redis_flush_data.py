...
from backend.flow.consts import DEFAULT_MONITOR_TIME, DEFAULT_REDIS_SYSTEM_CMDS, RedisBackupEnum
...
class RedisFlushDataFlow(object):
    """
    redis清档
    """
    ...
    def __get_cluster_info(bk_biz_id: int, cluster_id: int) -> dict:
        """
        flush data 需要元数据：
        1、master ip_ports 清档+备份
        2、版本
        """
        cluster = Cluster.objects.get(id=cluster_id, bk_biz_id=bk_biz_id)
        master_redis_map = defaultdict(list)
        for ins in cluster.storageinstance_set.filter(instance_role=InstanceRole.REDIS_MASTER.value):
            master_redis_map[ins.machine.ip].append(ins.port)

        return {
            "domain_name": cluster.immute_domain,
            "bk_cloud_id": cluster.bk_cloud_id,
            # "cluster_type": cluster.cluster_type,
            "master_redis_map": dict(master_redis_map),
            "db_version": cluster.major_version,
        }

    def redis_flush_data_flow(self):
        """
        主要逻辑：
            1、初始化信息
            3、for
            3.1、根据cluster_id获取slave信息
            3.2、检查请求
            3.3、执行备份
            3.4、执行清档
        """
        ...
        for rule in self.data["rules"]:
            ...
            master_ips = list(cluster_info["master_redis_map"].keys())
            ...
            sub_pipelines.append(
                sub_pipeline.build_sub_process(sub_name=_("集群[{}]清档").format(cluster_info["domain_name"]))
            )
