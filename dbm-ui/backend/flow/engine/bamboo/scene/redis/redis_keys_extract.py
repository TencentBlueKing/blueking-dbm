...
class RedisKeysExtractFlow(object):
    """
    redis提取keys流程类
    """
    ...
    def redis_keys_extract_flow(self):
        """
        主要逻辑：
            1、初始化信息
            3、for
        ...
        for rule in self.data["rules"]:
            ...
            for ip, __ in ip_ports.items():
                ...
                act_kwargs.get_redis_payload_func = RedisActPayload.keys_extract_payload.__name__
                acts_list.append(
                    {
                        "act_name": _("提取key: {}").format(ip),
            ...
            sub_pipelines.append(sub_pipeline.build_sub_process(sub_name=_("集群[{}提取keys").format(domain_name)))
