    ...
    def redis_data_structure_flow(self):
        ...
        for info in self.data["infos"]:
            ...
            if act_kwargs.cluster["cluster_type"] == ClusterType.TendisPredixyTendisplusCluster.value:
                logger.info("cluster_type is:{} 需要构建tendis cluster关系".format(act_kwargs.cluster["cluster_type"]))
                act_kwargs.cluster["all_instance"] = cluster_dst_instance
                act_kwargs.get_redis_payload_func = (
                    RedisActPayload.data_structure_clustermeet_slotsassign_payload.__name__
                )
                # 选第一台作为下发执行任务的机器
            ...
            act_kwargs.get_trans_data_ip_var = RedisDataStructureContext.get_new_install_proxy_exec_ip_var_name()
            proxy_payload = ""
        ...
        for new_temp_ip in [host["ip"] for host in info["redis_data_structure_hosts"]]:
            ...
            if len(source_ip_map) > 1:
                for source_temp_ip in source_ip_map:
                    ...
                    logger.info("数据构造下发data_params：{}".format(act_kwargs.cluster["data_params"]))
