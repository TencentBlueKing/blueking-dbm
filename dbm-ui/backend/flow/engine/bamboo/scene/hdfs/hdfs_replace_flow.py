...
from backend.flow.consts import ConfigTypeEnum, DnsOpType, HdfsRoleEnum, ManagerOpType, ManagerServiceType, UserName
...
from backend.flow.utils.hdfs.hdfs_context_dataclass import ActKwargs, DnsKwargs, HdfsReplaceContext
from backend.ticket.constants import TicketType

logger = logging.getLogger("flow")


class HdfsReplaceFlow(object):
    """
    构建hdfs集群申请流程的抽象类
    """
    ...
    def replace_hdfs_flow(self):
        ...
        add_dn_sub_pipeline = self.build_add_dn_sub_flow(act_kwargs)
        hdfs_pipeline.add_sub_pipeline(add_dn_sub_pipeline.build_sub_process(sub_name=_("集群DN替换-扩容DN")))

        del_dn_sub_pipeline = self.build_del_dn_sub_flow(act_kwargs)
        ...
        if manager_ip in self.data_with_role["del_dn_ips"]:
            ...
            manager_kwargs = BigdataManagerKwargs(
                ...
                db_type=DBType.Hdfs,
                service_type=ManagerServiceType.HA_PROXY,
                manager_ip=self.data_with_role["new_dn_ips"][0],
    ...
    def build_add_dn_sub_flow(self, act_kwargs: ActKwargs) -> SubBuilder:
        # sub_flow 添加DN
        self.__update_data_replace_dn(TicketType.HDFS_SCALE_UP)
        add_dn_sub_pipeline = SubBuilder(root_id=self.root_id, data=copy.deepcopy(self.data_with_role))
    ...
    def build_del_dn_sub_flow(self, act_kwargs: ActKwargs) -> SubBuilder:
        # sub_flow 缩容DN
        self.__update_data_replace_dn(TicketType.HDFS_SHRINK)
        del_dn_sub_pipeline = SubBuilder(root_id=self.root_id, data=copy.deepcopy(self.data_with_role))
    ...
    def __update_data_replace_dn(self, ticket_type: TicketType):
        data_with_role = copy.deepcopy(self.data_with_role)
        data_with_role["ticket_type"] = ticket_type.value
        self.data_with_role = data_with_role
