...
from backend.flow.utils.es.es_context_dataclass import EsActKwargs

logger = logging.getLogger("flow")


class ClearEsMachineFlow(object):
    """
    构建清理es机器的流程
    兼容跨云区域的执行
    """
    ...
    def run_flow(self):
        ...
        main_pipeline.add_act(
            act_name=_("清理机器cmdb元数据"), act_component_code=EsMetaComponent.code, kwargs=asdict(act_kwargs)
        )
