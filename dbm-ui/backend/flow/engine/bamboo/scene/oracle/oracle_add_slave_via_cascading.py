...
from backend.flow.engine.bamboo.scene.oracle.sub_task.add_slave_common import (
    _make_oracle_act,
    build_cascading_switch_to_master_sub_flow,
...
class OracleAddSlaveViaCascadingFlow(OracleBaseFlow):
    """
    Oracle[添加从库/重建从库]单据的流程引擎（使用级联方式，减少对主库性能的影响）
    ...
    资源池模式(ip_source=resource_pool):
    {
        ...
        "created_by": "admin",
        "bk_biz_id": 9991001,
        "ticket_type": "ORACLE_ADD_SLAVE_VIA_CASCADING",
        "infos": [
            {
                "cluster_id": 2,
    ...
    def oracle_add_slave_via_cascading_flow(self):
        ...
        for info in self.data["infos"]:
            ...
            # 2. 下发介质 (旧实例侧: old_node + old_master + cluster_master)
            sub_pipeline.add_sub_pipeline(
                sub_flow=build_media_transfer_sub_flow(
                    root_id=self.root_id,
                    data=sub_flow_data,
                    bk_cloud_id=bk_cloud_id,
                    old_hosts=[old_node, old_master, cluster_master],
                    new_slave=new_slave,
            ...
            # 3. 级联模式独有: 先获取最终同步源(old_master)配置, 再获取RMAN拷贝源(old_node)配置
            sub_pipeline.add_act(
                **_make_oracle_act(
                    act_name=_("获取最终同步源配置"),
                    exec_ip=old_master,
                    bk_cloud_id=bk_cloud_id,
                    payload_func_name=OracleActPayload.get_config_payload.__name__,
                    run_as_system_user=DBA_ORACLE_USER,
                    write_payload_var=AddSlaveContext.get_master_configs_var_name(),
                )
            )
            sub_pipeline.add_act(
                **_make_oracle_act(
                    act_name=_("获取RMAN拷贝源配置"),
                    exec_ip=old_node,
                    bk_cloud_id=bk_cloud_id,
                    payload_func_name=OracleActPayload.get_config_payload.__name__,
                    run_as_system_user=DBA_ORACLE_USER,
            ...
            # 6. 配置 DataGuard(via_statistic) + RMAN 在线复制 + 切日志/检查/实时应用 + 启动动态监听
            sub_pipeline.add_sub_pipeline(
            ...
            # 7. 级联模式独有: 暂停同步 -> 与原主配置DG -> 实时应用日志 -> 切换日志 -> 检查同步
            sub_pipeline.add_sub_pipeline(
                sub_flow=build_cascading_switch_to_master_sub_flow(
                    root_id=self.root_id,
                    data=sub_flow_data,
                    bk_cloud_id=bk_cloud_id,
            ...
            if info["replace_flag"]:
                ...
                sub_pipeline.add_sub_pipeline(
                    sub_flow=build_replace_common_sub_flow(
                        ...
                        dns_role=ClusterEntryRole.SLAVE_ENTRY.value,
                    )
                )
