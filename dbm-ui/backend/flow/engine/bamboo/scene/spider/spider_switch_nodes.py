...
class TenDBClusterSwitchNodesFlow(TenDBClusterAddNodesFlow, TenDBClusterReduceNodesFlow):
    ...
    def switch_nodes_flow_with_cluster(
        ...
        if not BaseValidator.check_disaster_tolerance_level(cluster, new_spider_hosts):
            raise CheckDisasterToleranceException(
                message=_("[{}]集群spider节点不满足容灾要求，请检查，剩余的节点信息:{}".format(cluster.immute_domain, new_spider_hosts))
            )
