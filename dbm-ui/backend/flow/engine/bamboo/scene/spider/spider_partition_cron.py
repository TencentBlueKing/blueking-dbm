...
class SpiderPartitionCronFlow(object):
    """
    分区定时任务单据的流程引擎
    {
        "uid": "xxx",
        ...
        "created_by": "xxx",
        "bk_biz_id": "xxx",
        "ticket_type": "TENDBCLUSTER_PARTITION_CRON",
        "infos": [
            {
                "bk_cloud_id": 0,
                "ip": "1.1.1.1",
    ...
    def spider_partition_cron_flow(self):
        """
        每个分区配置一个子流程：
        （1）检查表结构
        （2）获取分区变更的sql
        （3）dbactor执行分区指令
        """
        spider_partition_pipeline = Builder(root_id=self.root_id, data=self.data)
        sub_pipelines = []
        ...
        spider_partition_pipeline.add_parallel_sub_pipeline(sub_flow_list=sub_pipelines)
        sub_pipeline_callback = SubBuilder(root_id=self.root_id, data={**sub_data})
        sub_pipeline_callback.add_act(
            act_name=_("录入分区日志"),
            act_component_code=SpiderPartitionCallbackComponent.code,
            kwargs={},
        )
        spider_partition_pipeline.add_sub_pipeline(sub_pipeline_callback.build_sub_process(sub_name=_("录入分区日志")))
        logger.info(_("构建spider partition 定时任务流程成功"))
        spider_partition_pipeline.run_pipeline(init_trans_data_class=MysqlPartitionContext(), is_drop_random_user=True)
