...
class InfluxdbEnableFlow(object):
    """
    构建Influx启用流程
    """
    ...
    def enable_influxdb_flow(self):
        ...
        for instance in all_ips:
            ...
            # 启动进程
            ip = instance["ip"]
            act_kwargs.template = get_base_payload(action=InfluxdbActuatorActionEnum.StartProcess.value, host=ip)
            ...
            sub_pipelines.append(sub_pipeline.build_sub_process(sub_name=_("启用Influxdb {}子流程").format(ip)))
        # 并发执行所有子流程
