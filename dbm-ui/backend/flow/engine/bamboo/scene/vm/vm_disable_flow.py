...
class VmDisableFlow(VmBaseFlow):
    """
    构建vm集群禁用流程
    """
    ...
    def disable_vm_flow(self):
        """
        禁用vm集群
        """
        # Builder 传参 为封装好角色IP的数据结构
        disable_data = self.get_flow_base_data()
        vm_pipeline = Builder(root_id=self.root_id, data=disable_data)
        ...
        for ip in all_ips:
            # 停止进程
            act_kwargs.get_vm_payload_func = VmActPayload.get_stop_process_payload.__name__
            act_kwargs.exec_ip = ip
            act = {
                "act_name": _("停止vm集群进程-{}").format(ip),
