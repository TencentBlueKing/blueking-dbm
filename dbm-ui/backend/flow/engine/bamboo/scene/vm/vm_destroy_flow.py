...
from backend.flow.utils.vm.vm_context_dataclass import DnsKwargs, VmActKwargs, VmApplyContext

logger = logging.getLogger("flow")


class VmDestroyFlow(VmBaseFlow):
    """
    构建vm集群删除流程
    """
    ...
    def destroy_vm_flow(self):
        """
        删除vm集群
        """
        # Builder 传参 为封装好角色IP的数据结构
        destroy_data = self.get_flow_base_data()
        vm_pipeline = Builder(root_id=self.root_id, data=destroy_data)
