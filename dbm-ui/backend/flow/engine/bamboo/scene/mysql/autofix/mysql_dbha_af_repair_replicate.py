...
from backend.flow.plugins.components.collections.mysql.autofix.mysql_dbha_af_change_master import (
    MySQLDBHAAFChangeMasterComponent,
)
from backend.flow.plugins.components.collections.mysql.autofix.mysql_dbha_af_check_replicate import (
    MySQLDBHAAFCheckReplicateComponent,
)

logger = logging.getLogger("root")


class MySQLDBHAAFRepairROSlaveReplicateFlow(object):
    ...
    def do_repair(self):
        ...
        self.data = {
            "infos":[
                "bk_cloud_id": int,
                "cluster_id": int,
                "new_master_address": str 1.1.1.1:3306,
                "new_master_log_file": str,
                "new_master_log_pos": int,
                "old_master_address": str,
                "ro_slave_addresses": ["2.2.2.2:3306"],
                "check_id": int
            ]
        }
        流程分2个部分
        ...
        """
        ro_slave_subpipes = []
        # 每一个 info 就是一个独立的 cluster
        for info in self.data["infos"]:
            bk_cloud_id = info["bk_cloud_id"]
            # cluster_id = info["cluster_id"]
            new_master_address = info["new_master_address"]
            new_master_log_file = info["new_master_log_file"]
            new_master_log_pos = info["new_master_log_pos"]
            old_master_address = info["old_master_address"]
            ro_slave_addresses = info["ro_slave_addresses"]

            for ro_slave_address in ro_slave_addresses:
                ro_slave_subpipe = SubBuilder(root_id=self.root_id, data=copy.deepcopy(self.data))
                ro_slave_subpipe.add_act(
                    act_name=_("重设到 {} 的同步".format(new_master_address)),
                    act_component_code=MySQLDBHAAFChangeMasterComponent.code,
                    kwargs={
                        "bk_cloud_id": bk_cloud_id,
                        "ro_slave_address": ro_slave_address,
                        "old_master_address": old_master_address,
                        "new_master_address": new_master_address,
                        "new_master_log_file": new_master_log_file,
                        "new_master_log_pos": new_master_log_pos,
                    },
                )
                ro_slave_subpipe.add_act(
                    act_name=_("检查到 {} 的同步状态".format(new_master_address)),
                    act_component_code=MySQLDBHAAFCheckReplicateComponent.code,
                    kwargs={
                        "bk_cloud_id": bk_cloud_id,
                        "address": ro_slave_address,
                        "master_address": new_master_address,
                    },
                )
                ro_slave_subpipes.append(
                    ro_slave_subpipe.build_sub_process(sub_name=_("修复只读 slave {} 同步".format(ro_slave_address)))
                )

        repair_pipe = Builder(root_id=self.root_id, data=copy.deepcopy(self.data))
        repair_pipe.add_parallel_sub_pipeline(sub_flow_list=ro_slave_subpipes)

        # cluster_ids = self.data["cluster_ids"]
        # cluster_objs = Cluster.objects.filter(pk__in=cluster_ids)
        # # records = MySQLDBHAEvent.objects.filter(check_id=check_id)
