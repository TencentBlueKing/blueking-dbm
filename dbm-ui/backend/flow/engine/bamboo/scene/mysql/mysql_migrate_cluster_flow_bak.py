    ...
    def deploy_migrate_cluster_flow(self):
        ...
        for one_machine in self.data["infos"]:
            ...
            for one_id in one_machine["cluster_ids"]:
                ...
                if not one_cluster["running_slave_exist"]:
                    logger.error("cluster {} has not running slave".format(one_id))
                    raise TendbGetClusterInfoFailedException(message=_("集群 {} 不存在running状态的从库".format(one_id)))
