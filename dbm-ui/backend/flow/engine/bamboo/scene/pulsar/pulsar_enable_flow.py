...
class PulsarEnableFlow(PulsarBaseFlow):
    """
    构建Pulsar集群重新启用流程
    """
    ...
    def enable_pulsar_flow(self):
        ...
        for ip in all_ips:
            # 启动进程
            act_kwargs.get_pulsar_payload_func = PulsarActPayload.get_start_process_payload.__name__
        ...
        # 并发执行所有节点
        pulsar_pipeline.add_parallel_acts(acts_list=start_acts)
