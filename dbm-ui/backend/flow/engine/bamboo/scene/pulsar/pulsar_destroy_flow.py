...
class PulsarDestroyFlow(PulsarBaseFlow):
    """
    构建Pulsar集群禁用流程
    """
    ...
    def destroy_pulsar_flow(self):
        ...
        # 拼接活动节点需要的私有参数
        act_kwargs = PulsarActKwargs(self.base_flow_data["bk_cloud_id"])
        ...
        for ip in all_ips:
            ...
            act = {
                "act_name": _("Pulsar集群节点清理-{}").format(ip),
        ...
        # 清理域名
        dns_kwargs = DnsKwargs(bk_cloud_id=self.base_flow_data["bk_cloud_id"], dns_op_type=DnsOpType.CLUSTER_DELETE)
        pulsar_pipeline.add_act(
            act_name=_("删除域名"),
        ...
        zk_dns_kwargs = ZkDnsKwargs(
            ...
            zk_host_map=self.base_flow_data["zk_host_map"],
        )
        pulsar_pipeline.add_act(
            act_name=_("删除ZK域名"),
