    ...
    def replace_pulsar_flow(self):
        ...
        if self.is_role_in_ticket_replace(PulsarRoleEnum.ZooKeeper):
            ...
            common_sub_flow = new_machine_common_sub_flow(
                ...
                new_ips=self.base_flow_data["new_zk_ips"],
            )
            if common_sub_flow:
                replace_zk_sub_pipeline.add_sub_pipeline(sub_flow=common_sub_flow)
        ...
        if self.is_role_in_ticket_replace(PulsarRoleEnum.Broker) or self.is_role_in_ticket_replace(
            ...
            common_sub_flow = new_machine_common_sub_flow(
                uid=self.uid, root_id=self.root_id, bk_cloud_id=self.bk_cloud_id, new_ips=new_ips
            )
            if common_sub_flow:
                scale_up_sub_pipeline.add_sub_pipeline(sub_flow=common_sub_flow)
