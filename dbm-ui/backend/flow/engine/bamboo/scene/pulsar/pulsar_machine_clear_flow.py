...
class ClearPulsarMachineFlow(object):
    """
    构建清理pulsar机器的流程
    兼容跨云区域的执行
    """
    ...
    def run_flow(self):
        ...
        act_kwargs = PulsarActKwargs(bk_cloud_id=self.bk_cloud_id)
        main_pipeline.add_act(
            act_name=_("清理机器cmdb元数据"), act_component_code=PulsarDBMetaComponent.code, kwargs=asdict(act_kwargs)
        )
