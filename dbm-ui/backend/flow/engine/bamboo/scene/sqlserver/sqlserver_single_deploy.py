...
class SqlserverSingleApplyFlow(BaseFlow):
    """
    ...
    def run_flow(self):
        """
        定义部署单节点实例流程，支持多实例部署，支持多行并行部署，支持多云区域同时部署
        部署逻辑：
        1：机器空闲检测（可选）
        2：下发安装介质包
        3：安装实例
        ...
        for info in self.data["infos"]:
            ...
            for number, cluster in enumerate(info["clusters"]):
                ...
                cluster["master_ip"] = info["mssql_host"]["ip"]
            ...
            # 安装实例
            print(sub_flow_context["clusters"])
            sub_pipeline.add_sub_pipeline(
                sub_flow=install_sqlserver_sub_flow(
                    ...
                    target_hosts=[Host(**info["mssql_host"])],
