...
class SqlserverDTSFlow(BaseFlow):
    ...
    def full_dts_flow_v2(self):
        ...
        # 计算需要添加临时账号的集群
        src_cluster_ids = set(i["src_cluster"] for i in self.data["infos"])
        dst_cluster_ids = set(sum((i["dst_cluster_list"] for i in self.data["infos"]), []))
        total_cluster_ids = list(src_cluster_ids | dst_cluster_ids)
        ...
        main_pipeline.add_act(
            ...
            kwargs=asdict(DropRandomJobUserKwargs(cluster_ids=total_cluster_ids)),
            is_remote_rewritable=True,
        )
