...
class KafkaScaleUpFlow(object):
    ...
    def __get_dbmeta_brokers(self) -> list:
        ...
        """
        cluster = Cluster.objects.get(id=self.data["cluster_id"])
        brokers = list(set(StorageInstance.objects.filter(
            cluster=cluster, instance_role=InstanceRole.BROKER).values_list("machine__ip", flat=True)))
        return brokers
    ...
    def scale_up_kafka_flow(self):
        ...
        instance_list = [
            {"ip": ip, "port": self.data["port"], "bk_cloud_id": self.data["bk_cloud_id"]}
            for ip in all_ips
        ]
        details = {
