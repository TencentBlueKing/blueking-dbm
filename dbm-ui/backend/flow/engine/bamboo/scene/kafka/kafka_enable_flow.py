...
class KafkaEnableFlow(object):
    ...
    def enable_kafka_flow(self):
        ...
        for ip in all_ips:
            ...
            # 启动进程
            act_kwargs.template = get_base_payload(action=KafkaActuatorActionEnum.StartProcess.value, host=ip)
            ...
            sub_pipelines.append(sub_pipeline.build_sub_process(sub_name=_("启用Kafka {}子流程").format(ip)))
        # 并发执行所有子流程
