...
from backend.flow.utils.kafka.kafka_context_dataclass import ActKwargs

logger = logging.getLogger("flow")


class ClearKafkaMachineFlow(object):
    """
    构建清理kafka机器的流程
    兼容跨云区域的执行
    """
    ...
    def run_flow(self):
        ...
        main_pipeline.add_act(
            act_name=_("清理机器cmdb元数据"), act_component_code=KafkaDBMetaComponent.code, kwargs=asdict(act_kwargs)
        )
