...
from backend.db_report.views.redis.base import RedisCheckReportBaseViewSet, RedisCheckReportSerializer


@register_report(DBType.Redis)
class RedisConfCheckReportViewSet(RedisCheckReportBaseViewSet):
    """Unified Redis 配置检查报告 (role mismatch + predixy server fail / conf drift)."""

    queryset = RedisCheckReport.objects.filter(subtype=RedisCheckSubType.ConfigInconsistent.value)
    serializer_class = RedisCheckReportSerializer
    report_type = ReportType.CONF_CHECK
    report_title = [
    ...
    @common_swagger_auto_schema(
        operation_summary=_("Redis 配置检查报告"),
