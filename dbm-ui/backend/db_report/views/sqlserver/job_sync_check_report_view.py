...
from backend.db_report.models import SqlserverCheckJobSyncReport
...
class JobSyncCheckReportBaseViewSet(ReportBaseViewSet):
    queryset = SqlserverCheckJobSyncReport.objects.filter(is_job_inconsistent=True).order_by("-create_at")
    serializer_class = JobSyncCheckReportSerializer
    report_type = ReportType.SQLSERVER_JOB_SYNC_CHECK
    report_title = [
        {
            "name": "bk_biz_id",
            "display_name": _("业务"),
    ...
    @common_swagger_auto_schema(
        operation_summary=_("主从集群的作业同步检查报告"),
        responses={status.HTTP_200_OK: JobSyncCheckReportSerializer()},
        tags=[SWAGGER_TAG],
    )
