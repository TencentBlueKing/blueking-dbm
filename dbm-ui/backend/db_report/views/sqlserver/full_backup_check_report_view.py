...
from backend.db_report.models import SqlserverFullBackupCheckReport
...
class FullBackupCheckReportSerializer(serializers.ModelSerializer, ReportCommonFieldSerializerMixin):
    class Meta:
        model = SqlserverFullBackupCheckReport
        fields = (
            "bk_biz_id",
            "cluster",
            "cluster_type",
...
class FullBackupCheckReportBaseViewSet(ReportBaseViewSet):
    queryset = SqlserverFullBackupCheckReport.objects.filter(
        state__in=[ReportStateType.ABNORMAL.value, ReportStateType.WARNING.value]
    ).order_by("-create_at")
    serializer_class = FullBackupCheckReportSerializer
    report_type = ReportType.SQLSERVER_FULL_BACKUP_CHECK_BY_MODEL
    report_title = [
        {
            "name": "bk_biz_id",
            "display_name": _("业务"),
