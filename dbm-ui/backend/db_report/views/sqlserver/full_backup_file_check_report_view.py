...
from backend.db_report.models import SqlserverFullBackupInfoReport
...
class FullBackupInfoReportSerializer(serializers.ModelSerializer, ReportCommonFieldSerializerMixin):
    class Meta:
        model = SqlserverFullBackupInfoReport
...
class FullBackupInfoReportBaseViewSet(ReportBaseViewSet):
    queryset = SqlserverFullBackupInfoReport.objects.filter(status=False).order_by("-create_at")
    serializer_class = FullBackupInfoReportSerializer
    report_type = ReportType.SQLSERVER_FULL_BACKUP_CHECK
    report_title = [
        {
            "name": "bk_biz_id",
            "display_name": _("业务"),
    ...
    @common_swagger_auto_schema(
        ...
        responses={status.HTTP_200_OK: FullBackupInfoReportSerializer()},
        tags=[SWAGGER_TAG],
    )
