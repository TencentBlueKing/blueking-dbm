...
from backend.db_report.models import SqlserverLogBackupCheckReport
...
class LogBackupCheckReportSerializer(serializers.ModelSerializer, ReportCommonFieldSerializerMixin):
    class Meta:
        model = SqlserverLogBackupCheckReport
        fields = (
            "bk_biz_id",
            "cluster",
            "cluster_type",
...
class LogBackupCheckReportBaseViewSet(ReportBaseViewSet):
    queryset = SqlserverLogBackupCheckReport.objects.filter(
        state__in=[ReportStateType.ABNORMAL.value, ReportStateType.WARNING.value]
    ).order_by("-create_at")
    serializer_class = LogBackupCheckReportSerializer
    report_type = ReportType.SQLSERVER_LOG_BACKUP_CHECK_BY_MODEL
    report_title = [
        {
            "name": "bk_biz_id",
            "display_name": _("业务"),
