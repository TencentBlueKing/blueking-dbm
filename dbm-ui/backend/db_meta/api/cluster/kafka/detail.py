...
def scan_cluster(cluster: Cluster) -> Graphic:
    ...
    # 获得broker节点组，zookeeper节点组
    broker_insts, broker_group = graph.add_instance_nodes(
        cluster=cluster, roles=InstanceRole.BROKER, group_name=_("Broker 节点")
    )
    _dummy, zk_group = graph.add_instance_nodes(
        cluster=cluster, roles=InstanceRole.ZOOKEEPER, group_name=_("Zookeeper 节点")
    )
