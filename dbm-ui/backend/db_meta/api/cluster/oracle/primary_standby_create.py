...
from backend.db_meta.api import machine, storage_instance, storage_instance_tuple
...
class OracleCCTopoOperator(CCTopoOperator):
    db_type = DBType.Oracle.value
            ...
            instances.append(
                {
                    ...
                    "name": storage["service_name"],
                }
            )
            if storage["role"] == InstanceRole.PRIMARY:
                primary = storage
        for storage in storages:
            if storage["role"] != InstanceRole.PRIMARY:
                tuple.append(
...
def pkg_create_oracle(
    ...
    """
    这里打包从头开始创建一个 oracle 主备集群
    包括 machine、storage、tuple、cluster、cluster_entry等

    storages (_type_): _description_
        [{"ip":"","port":,"role":"primary","domain":""},{},{}]
    """
    ...
    # 创建machine storageInstance
    create_oracle_instances(bk_biz_id, bk_cloud_id, MachineType.ORACLE.value, storages, spec_id, spec_config)
...
def create_oracle_set(
    ...
    Args:
        ...
        storages (Optional[List], optional): => _description_
         [{"role":"primary","ip":"xx","port":20000,"domain":""},
          {"role":"standby","ip":"xx","port":20000,"domain":""}]
    """
    ...
    try:
        ...
        # 创建集群与实例的关系 db_meta_storageinstance_cluster
        ...
        logger.error(traceback.format_exc())
        raise Exception("oracleset create failed {}".format(e))
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        raise Exception("oracleset add dns entry failed {}".format(e))
    # 移动cc模块
    # OracleCCTopoOperator(cluster).transfer_instances_to_cluster_module(storage_objs, is_increment=True)
