...
def create(
    ...
    broker_instances = common.filter_out_instance_obj(
        storages, StorageInstance.objects.filter(instance_role=InstanceRole.PULSAR_BROKER)
    )
    if broker_instances.exists():
        cluster_entry.storageinstance_set.add(*broker_instances)
    cluster_entry.save()
