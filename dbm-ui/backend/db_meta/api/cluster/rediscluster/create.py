...
def create(
    ...
    for storage_obj in storage_objs:
        slave = storage_obj.as_ejector.get().receiver
        storage_bind_objs.append(storage_obj)
        storage_bind_objs.append(slave)
    # nodes注册访问入口
    try:
        ce = ClusterEntry.objects.create(
            cluster=cluster,
        ...
        ce.storageinstance_set.add(*storage_bind_objs)
        ce.save()
    except IntegrityError:
        logger.error(traceback.format_exc())
        raise ClusterEntryExistException(entry=nodes_domain)
