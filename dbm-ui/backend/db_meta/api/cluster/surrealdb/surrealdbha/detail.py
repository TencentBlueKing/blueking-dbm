...
def scan_cluster(
    ...
    # 获取集群tikv实例
    tikv_infos = get_instances(KubernetesInstanceRole.TIKV)
    tikv_group = Group(node_id=KubernetesInstanceRole.TIKV, group_name="TiKV")
    for tikv in tikv_infos:
        graph.add_node(tikv, to_group=tikv_group)

    graph.add_line(source=surreal_group, target=tikv_group, label=LineLabel.ReadWrite)

    # 获取集群PD实例
    pd_infos = get_instances(KubernetesInstanceRole.PD)
    pd_group = Group(node_id=KubernetesInstanceRole.PD, group_name="PD")
    for pd in pd_infos:
        graph.add_node(pd, to_group=pd_group)

    graph.add_line(source=surreal_group, target=pd_group, label=LineLabel.ReadWrite)
    graph.add_line(source=tikv_group, target=pd_group, label=LineLabel.Access)
    graph.add_line(source=pd_group, target=tikv_group, label=LineLabel.Access)
