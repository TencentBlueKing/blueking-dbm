...
def create(
    ...
    cluster = Cluster.objects.create(
        ...
        name=name,
        alias=alias,
        cluster_type=ClusterType.Vm.value,
    ...
    vminserts = common.filter_out_instance_obj(
        storages, StorageInstance.objects.filter(instance_role=InstanceRole.VM_INSERT)
    )
    vmselects = common.filter_out_instance_obj(
        storages, StorageInstance.objects.filter(instance_role=InstanceRole.VM_SELECT)
    )
    ...
    if vminserts.exists():
        cluster_entry.storageinstance_set.add(*vminserts)
    cluster_entry.save()
    ...
    if slave_domain:
        ...
        cluster_slave_entry.storageinstance_set.add(*vmselects)
        cluster_slave_entry.save()
