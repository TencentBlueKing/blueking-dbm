...
def replace(
    ...
    # 关联对应的域名信息
    vmauths = cluster.storageinstance_set.filter(instance_role=InstanceRole.VM_AUTH)
    for vmauth in vmauths:
        if not vmauth.bind_entry.exists():
            cluster_entry.storageinstance_set.add(vmauth)
