...
MODULE = "backend.dbm_aiagent.mcp_tools.mysql.impl.bill_machine_replace.bill_tendbha_slave_rebuild"
...
class TestTendbhaSlaveRebuildEarlyValidation:
    """TenDBHA slave 原地重建：靠前的纯逻辑校验分支（空 ips、部分 IP 未找到、跨集群不一致）"""
    ...
    def test_empty_ips_raises(self, mock_validate):
        ...
        with pytest.raises(Exception, match="ips 不能为空"):
            bill_tendbha_slave_rebuild("admin", ["test.tendbha.db"], [])
    ...
    def test_missing_ip_raises(self, mock_validate, mock_storage):
        ...
        with pytest.raises(Exception, match="部分 IP 未找到对应 slave 实例"):
            bill_tendbha_slave_rebuild("admin", ["test.tendbha.db"], ["1.1.1.1"])
...
class TestTendbhaSlaveRebuildSuccessPath:
    """TenDBHA slave 原地重建：成功提单路径（验证 ticket_param 结构组装正确）"""

    @patch(f"{MODULE}.find_duplicate_ticket")
    @patch(f"{MODULE}.MysqlRestoreLocalSlaveDetailSerializer")
...
class TestTendbhaSlaveRebuildDuplicate:
    """TenDBHA slave 原地重建：防重路径（命中已有单据时直接复用，不再创建）"""
