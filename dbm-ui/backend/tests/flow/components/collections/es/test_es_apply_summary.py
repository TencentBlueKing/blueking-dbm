...
"""
# EsApplySummaryService继承了BigDataApplySummaryService，公共编排逻辑(查Cluster、拼access_entry_url、
# 跳过写入)已经在 tests/flow/components/collections/common/test_bigdata_apply_summary.py 里参数化覆盖，
# 这里只测ES专属的CLB/北极星补充逻辑、entry_remark拼装，以及schema稳定性。
# Cluster/Flow/FlowOutputHandler由基类bigdata_apply_summary模块引用，所以mock要patch在base_mod上；
# ClusterEntry/ClusterEntryType是ES自己import的，patch在es_apply_summary模块上。
...
class TestEsApplySummaryServiceExecute:
    ...
    def test_writes_access_entry_url_when_cluster_exists(self, mock_cluster_cls, mock_flow_cls, mock_handler_cls):
        ...
        assert summary_list[0]["entry_remark"] == ""
        assert (
    ...
    @patch.object(base_mod, "Flow")
    @patch.object(base_mod, "Cluster")
    def test_cluster_not_found_row_is_skipped(self, mock_cluster_cls, mock_flow_cls, mock_handler_cls):
        mock_flow_cls.objects.filter.return_value.exists.return_value = True
    ...
    @patch.object(mod, "ClusterEntry")
    @patch.object(base_mod, "Cluster")
    def test_apply_clb_true_but_entry_missing_sets_remark_and_logs_error(
        self, mock_cluster_cls, mock_entry_cls, mock_flow_cls, mock_handler_cls
    ):
        mock_flow_cls.objects.filter.return_value.exists.return_value = True
        mock_cluster_cls.objects.get.return_value = MagicMock(bk_biz_id=3, id=100)
        ...
        assert summary_list[0]["entry_remark"] == "CLB信息未查询到"
    ...
    @patch.object(mod, "ClusterEntry")
    @patch.object(base_mod, "Cluster")
    def test_apply_polaris_true_but_entry_missing_sets_remark_and_logs_error(
        self, mock_cluster_cls, mock_entry_cls, mock_flow_cls, mock_handler_cls
    ):
        mock_flow_cls.objects.filter.return_value.exists.return_value = True
        mock_cluster_cls.objects.get.return_value = MagicMock(bk_biz_id=3, id=100)
        ...
        svc = _make_service()
        result = svc._execute(FakeData([_item(apply_polaris=True)]), None)

        assert result is True
        summary_list = mock_handler_cls.return_value.insert_data.call_args.args[1]
        assert summary_list[0]["polaris_name"] == ""
        assert summary_list[0]["entry_remark"] == "北极星信息未查询到"
        svc.log_error.assert_any_call("集群[es.test.db]未查询到北极星信息")

    @patch.object(base_mod, "FlowOutputHandler")
    @patch.object(base_mod, "Flow")
    @patch.object(mod, "ClusterEntry")
    @patch.object(base_mod, "Cluster")
    def test_apply_clb_and_polaris_both_missing_joins_remarks(
        self, mock_cluster_cls, mock_entry_cls, mock_flow_cls, mock_handler_cls
    ):
        mock_flow_cls.objects.filter.return_value.exists.return_value = True
        mock_cluster_cls.objects.get.return_value = MagicMock(bk_biz_id=3, id=100)
        ...
        assert result is True
        summary_list = mock_handler_cls.return_value.insert_data.call_args.args[1]
        assert summary_list[0]["entry_remark"] == "CLB信息未查询到；北极星信息未查询到"
    ...
    @patch.object(base_mod, "Flow")
    @patch.object(base_mod, "Cluster")
    def test_skips_write_when_no_flow_record(self, mock_cluster_cls, mock_flow_cls, mock_handler_cls):
