...
"""
# 只测 PulsarApplySummaryService._execute() 的编排逻辑（查Cluster、拼access_entry_url、跳过写入），
# 以及 add_pulsar_apply_summary_output_act() 的参数拼装，全部通过mock隔离ORM/FlowOutputHandler，无需django_db标记。
...
def _make_service(root_id="root123"):
    svc = mod.PulsarApplySummaryService()
...
class TestPulsarApplySummaryServiceExecute:
    ...
    def test_writes_access_entry_url_when_cluster_exists(self, mock_cluster_cls, mock_flow_cls, mock_handler_cls):
        ...
        mock_handler_cls.assert_called_once_with(mod.PulsarApplySummarySerializer)
        root_id, summary_list = mock_handler_cls.return_value.insert_data.call_args.args
        assert root_id == "root123"
        assert summary_list[0]["domain_name"] == "pulsar.test.db"
        assert summary_list[0]["port"] == 6650
        assert (
            summary_list[0]["access_entry_url"]
            == "https://dbm.example.com/3/db-manage/pulsar/detail/100?open=access_entry"
        )
    ...
    def test_cluster_not_found_leaves_access_entry_url_blank(self, mock_cluster_cls, mock_flow_cls, mock_handler_cls):
        ...
        svc.log_error.assert_any_call("写入集群信息摘要失败，集群[pulsar.test.db]不存在")
