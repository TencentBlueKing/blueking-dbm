...
"""
# 只测 DorisApplySummaryService._execute() 的编排逻辑（查Cluster、拼access_entry_url、跳过写入），
# 以及 add_doris_apply_summary_output_act() 的参数拼装，全部通过mock隔离ORM/FlowOutputHandler，无需django_db标记。
from unittest.mock import MagicMock, patch

from backend.flow.plugins.components.collections.doris import doris_apply_summary as mod
...
def _make_service(root_id="root123"):
    svc = mod.DorisApplySummaryService()
...
class TestDorisApplySummaryServiceExecute:
    ...
    def test_writes_access_entry_url_when_cluster_exists(self, mock_cluster_cls, mock_flow_cls, mock_handler_cls):
        ...
        mock_handler_cls.assert_called_once_with(mod.DorisApplySummarySerializer)
        root_id, summary_list = mock_handler_cls.return_value.insert_data.call_args.args
        assert root_id == "root123"
        assert summary_list[0]["domain_name"] == "doris.test.db"
        assert summary_list[0]["query_port"] == 9030
        assert (
            summary_list[0]["access_entry_url"]
            == "https://dbm.example.com/3/db-manage/doris/detail/100?open=access_entry"
        )
    ...
    def test_cluster_not_found_leaves_access_entry_url_blank(self, mock_cluster_cls, mock_flow_cls, mock_handler_cls):
        ...
        svc.log_error.assert_any_call("写入集群信息摘要失败，集群[doris.test.db]不存在")
...
class TestDorisApplySummarySchema:
    def test_field_names_and_primary_key_are_stable(self):
        assert list(mod.DorisApplySummarySerializer().fields) == [
            "domain_name",
            "region",
            "version",
            "query_port",
        ...
        assert mod.DorisApplySummarySerializer.table_primary_key == "domain_name"


class TestAddDorisApplySummaryOutputAct:
    def test_add_act_receives_expected_kwargs(self):
        pipeline = MagicMock()

        mod.add_doris_apply_summary_output_act(
            doris_pipeline=pipeline,
            bk_biz_id=3,
            domain_name="doris.test.db",
            region="default",
            version="2.0.3",
        ...
        assert call_kwargs["act_component_code"] == mod.DorisApplySummaryComponent.code
