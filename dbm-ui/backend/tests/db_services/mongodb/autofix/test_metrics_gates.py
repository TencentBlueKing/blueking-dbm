...
def test_same_rs_limit_does_not_block_other_shard():
    """shard A 已达上限时，不得用粗域名键误伤 shard B。"""
    c_s1 = HostCandidate(ip="127.0.0.2", rs_keys={"d|s1"}, names={"127.0.0.2:27017"})
    passed, reasons = same_rs_limit(
        [c_s1],
        active_names_by_rs={"d|s0": {"127.0.0.1:27017", "127.0.0.9:27017"}},
        max_abnormal=2,
    )
    assert [c.ip for c in passed] == ["127.0.0.2"]
    assert reasons == {}


def test_same_rs_limit_ignores_empty_domain_key():
    """磁盘-only 空维度 rs_key='|' 不得把无关主机并成同一 RS。"""
    c1 = HostCandidate(ip="127.0.0.1", rs_keys={"|"}, names={"127.0.0.1:27017"})
    c2 = HostCandidate(ip="127.0.0.2", rs_keys={"|"}, names={"127.0.0.2:27017"})
    passed, reasons = same_rs_limit([c1, c2], max_abnormal=2)
    assert {c.ip for c in passed} == {"127.0.0.1", "127.0.0.2"}
    assert reasons == {}


def test_make_rs_key_and_usable():
    assert make_rs_key("m1.db", "s0") == "m1.db|s0"
    assert make_rs_key("", "") == "|"
    assert is_usable_rs_key("|") is False
    assert is_usable_rs_key("m1.db|") is True
    assert is_usable_rs_key("m1.db|s0") is True
