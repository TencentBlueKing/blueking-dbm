...
class MySQLProxyRescueDetailSerializer(MySQLBaseOperateDetailSerializer):
    """
    MySQL Proxy 救援工单参数序列化器

    支持两种救援场景：
    1. 有旧 Proxy 元数据：所有原 Proxy 必须不可用
    2. 没有旧 Proxy 元数据：需要提供 proxy_port 参数
    """
    ...
    # ✅ 新增：Proxy 端口（如果没有旧 Proxy 元数据，则必填）
    proxy_port = serializers.IntegerField(
        help_text=_("Proxy 端口（如果集群没有旧 Proxy 元数据，则此参数必填）"), required=False, min_value=3306, max_value=65535
    )
    # ✅ 新增：Proxy 版本（可选，指定则使用指定版本，否则使用旧 Proxy 版本或最新版本）
    proxy_version = serializers.CharField(
        help_text=_("Proxy 版本（可选，指定则使用该版本，否则自动从旧 Proxy 获取或使用最新版本）"), required=False, allow_blank=True, max_length=64
    )
    # 是否在人工确认后自动下架旧 Proxy
...
class MySQLProxyRescueFlowBuilder(BaseMySQLHATicketFlowBuilder):
    ...
    # 使用校验器
    # validator 会在控制器的 @validates_with 装饰器中应用
