...
class MysqlAddSlaveDetailSerializer(MySQLBaseOperateDetailSerializer):
    class AddSlaveInfoSerializer(serializers.Serializer):
        new_slaves = serializers.ListField(help_text=_("新从库机器信息列表"), child=HostInfoSerializer(), required=False)
    ...
    def validate(self, attrs):
        ...
        # 校验集群与新增slave云区域是否相同
        super().validate_hosts_clusters_in_same_cloud_area(attrs, host_key=["new_slaves"], cluster_key=["cluster_ids"])
...
class MysqlAddSlaveParamBuilder(builders.FlowParamBuilder):
    ...
    def format_ticket_data(self):
        ...
        for info in self.ticket_data["infos"]:
            info["new_slave_ips"] = [slave["ip"] for slave in info.get("new_slaves", [])]
...
class MysqlAddSlaveResourceParamBuilder(BaseOperateResourceParamBuilder):
    ...
    def patch_slave_subzone(cls, ticket_data):
        ...
        for info in ticket_data["infos"]:
            ...
            # 为每个new_slave设置亲和性
            for i, _ in enumerate(info.get("new_slaves", [])):
    ...
    def post_callback(self):
        ...
        for info in ticket_data["infos"]:
            info["new_slaves"] = info.pop("new_slaves")
            info["new_slave_ips"] = [slave["ip"] for slave in info["new_slaves"]]
