...
class SQLServerDataExportDetailSerializer(SQLServerBaseOperateDetailSerializer):
    class DataExportDetailSerializer(serializers.Serializer):
        ignore_dbnames = serializers.ListField(help_text=_("忽略数据库列表"),child=serializers.CharField(),)
    ...
    def validate(self, data):
        ...
        if cluster_type == 'sqlserver_ha' and not select_role:
            raise serializers.ValidationError({
                'select_role': '当集群类型为 sqlserver_ha（主从）时，必须指定查询角色（master 或 slave）'
            })
        return data
...
class SQLServerDataExportFlowParamBuilder(builders.FlowParamBuilder):
    ...
    def format_ticket_data(self):
        ...
        dump_file_name = f"{cluster.immute_domain}_{int(time.time())}_dbm_console_dump.sql"# 后端的flow把这个dump_file_name写入到制品库
