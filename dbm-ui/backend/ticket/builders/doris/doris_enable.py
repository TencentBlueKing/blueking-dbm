...
class DorisEnableDetailSerializer(BigDataTakeDownDetailSerializer):
    pass


class DorisEnableFlowParamBuilder(builders.FlowParamBuilder):
    controller = DorisController.doris_enable_scene


@builders.BuilderFactory.register(
    TicketType.DORIS_ENABLE, phase=ClusterPhase.ONLINE, iam=ActionEnum.DORIS_ENABLE_DISABLE
)
class DorisEnableFlowBuilder(BaseDorisTicketFlowBuilder):
    serializer = DorisEnableDetailSerializer
    inner_flow_builder = DorisEnableFlowParamBuilder
    inner_flow_name = _("Doris集群启用")
