...
class RedisDataCheckRepairDetailSerializer(serializers.Serializer):
    ...
    specified_execution_time = serializers.CharField(
        help_text=_("执行模式为定时执行时，需要设置执行时间"), required=False, allow_blank=True
    )

    keep_check_and_repair = serializers.BooleanField(help_text=_("是否保持校验"))
    check_stop_time = serializers.CharField(help_text=_("校验终止时间，当不保持校验时，需要设置该时间"), required=False, allow_blank=True)
