...
class TendbChecksumParamBuilder(MySQLChecksumFlowParamBuilder):
    ...
    def fetch_cluster_shard_infos(self, cluster, backup_table_info):
        ...
        # 构建 ejector 到 slave list 的字典映射
        ejector_to_slaves_map = {}
        for slave in all_slaves:
            ejector = slave.ejector
            if ejector not in ejector_to_slaves_map:
                ejector_to_slaves_map[ejector] = []
            ejector_to_slaves_map[ejector].append(slave)
        ...
        for shard in storage_set:
            ...
            # 使用映射获取从节点相关信息
            slaves = ejector_to_slaves_map.get(ejector, [])
            slave_info = [self._get_instance_related_info(slave.receiver) for slave in slaves]
