...
class TendbChecksumCronDetailSerializer(TendbChecksumDetailSerializer):
    ...
    class TicketModeSerializer(serializers.Serializer):

        mode = serializers.CharField(help_text=_("单据执行类型"))
        trigger_time = DBTimezoneField(help_text=_("定时触发时间"))

    ticket_mode = TicketModeSerializer(help_text=_("单据类型信息"))
