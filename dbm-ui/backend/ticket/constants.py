...
class TicketType(str, StructuredEnum):
    ...
    TENDBCLUSTER_STANDARDIZE = EnumField("TENDBCLUSTER_STANDARDIZE", _("TenDB Cluster 集群标准化"))
    TENDBCLUSTER_METADATA_IMPORT = EnumField("TENDBCLUSTER_METADATA_IMPORT", _("TenDB Cluster 元数据导入"))
    ...
    # SQLServer
    SQLSERVER_SINGLE_APPLY = EnumField("SQLSERVER_SINGLE_APPLY", _("SQLServer 单节点部署"))
    SQLSERVER_HA_APPLY = EnumField("SQLSERVER_HA_APPLY", _("SQLServer 高可用部署"))
    SQLSERVER_IMPORT_SQLFILE = EnumField("SQLSERVER_IMPORT_SQLFILE", _("SQLServer SQL导入执行"))
    SQLSERVER_BACKUP_DBS = EnumField("SQLSERVER_BACKUP_DBS", _("SQLServer 库表备份"))
    SQLSERVER_CLEAR_DBS = EnumField("SQLSERVER_CLEAR_DBS", _("SQLServer 清档"))
    SQLSERVER_DESTROY = EnumField("SQLSERVER_DESTROY", _("SQLServer 高可用删除"))
    SQLSERVER_DISABLE = EnumField("SQLSERVER_DISABLE", _("SQLServer 高可用禁用"))
    SQLSERVER_ENABLE = EnumField("SQLSERVER_ENABLE", _("SQLServer 高可用启用"))
    SQLSERVER_DBRENAME = EnumField("SQLSERVER_DBRENAME", _("SQLServer DB重命名"))
    SQLSERVER_MASTER_SLAVE_SWITCH = EnumField("SQLSERVER_MASTER_SLAVE_SWITCH", _("SQLServer 主从互切"))
    SQLSERVER_MASTER_FAIL_OVER = EnumField("SQLSERVER_MASTER_FAIL_OVER", _("SQLServer 主故障切换"))
    SQLSERVER_RESTORE_LOCAL_SLAVE = EnumField("SQLSERVER_RESTORE_LOCAL_SLAVE", _("SQLServer 原地重建"))
    SQLSERVER_RESTORE_SLAVE = EnumField("SQLSERVER_RESTORE_SLAVE", _("SQLServer 新机重建"))
    ...
    RIAK_CLUSTER_ENABLE = EnumField("RIAK_CLUSTER_ENABLE", _("Riak 集群启用"))
    RIAK_CLUSTER_DISABLE = EnumField("RIAK_CLUSTER_DISABLE", _("Riak 集群禁用"))
    RIAK_CLUSTER_REBOOT = EnumField("RIAK_CLUSTER_REBOOT", _("Riak 集群节点重启"))
    RIAK_CLUSTER_MIGRATE = EnumField("RIAK_CLUSTER_MIGRATE", _("Riak 集群迁移"))
