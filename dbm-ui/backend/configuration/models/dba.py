...
class DBAdministrator(models.Model):
    ...
    @classmethod
    def list_biz_admins(cls, params) -> List[Dict[str, Union[str, List[str]]]]:
        """获取业务DBA人员"""
        # DBA 人员获取优先级： 业务 > 平台 > 默认空值
        # 没类型就是获取全部的
        if "db_type" not in params:
            valid_db_types = DBType.get_values()
            params["db_type__in"] = valid_db_types
        # else:
        #   valid_db_types = [params["db_type"]]

        db_admins = []
        for biz_dba in cls.objects.filter(**params):
            db_admins.append(
                {
        ...
        #         "users": users,
        #         "is_show": True if db_type != DBType.Cloud.value else False,
        #     }
    ...
    def get_biz_db_type_admins(cls, bk_biz_id: int, db_type: str) -> List[str]:
        biz_admins = cls.list_biz_admins({"bk_biz_id": bk_biz_id})
    ...
    def get_dba_for_db_type(cls, bk_biz_id: int, db_type: str) -> Tuple[List[str], List[str], List[str]]:
        """获取主dba、备dba、二线dba人员"""
        dba_list = cls.list_biz_admins({"bk_biz_id": bk_biz_id})
