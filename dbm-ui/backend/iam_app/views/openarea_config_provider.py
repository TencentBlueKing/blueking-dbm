...
class OpenareaConfigResourceProvider(BaseModelResourceProvider):
    """
    flow资源的反向拉取类
    """

    model: models.Model = TendbOpenAreaConfig
    resource_meta: ResourceMeta = ResourceEnum.OPENAREA_CONFIG

    def list_instance(self, filter, page, **options):
        filter.data_source = self.model
        filter.value_list = [self.resource_meta.lookup_field, *self.resource_meta.display_fields]
        filter.keyword_field = "config_name__icontains"
        bk_biz_ids = self.get_bk_biz_id_from_tenant()
        if bk_biz_ids:
            filter.conditions["bk_biz_id"] = bk_biz_ids
    ...
    def list_instance_by_policy(self, filter, page, **options):
        ...
        return self._list_instance_by_policy(
            ...
            # id 同时作为查询id，cluster_type， config_name作为展示
            value_list=["id", "cluster_type", "config_name"],
