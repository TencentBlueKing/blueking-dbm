...
def try_shadow(method: str, v3_result: Any, args: List, kwargs: Dict) -> None:
    ...
    try:
        ...
        action_ids = shadow.extract_action_ids(method, args)
        if v3_norm == v4_norm:
            shadow.logger.info(
                "[iam_v4_shadow] consistent method=%s action_ids=%s result=%s", method, action_ids, v3_norm
            )
        else:
            diff = shadow.diff_action_ids(v3_norm, v4_norm)
            shadow.logger.warning(
                "[iam_v4_shadow] MISMATCH method=%s action_ids=%s diff=%s v3=%s v4=%s",
                method,
                action_ids,
                diff,
                v3_norm,
