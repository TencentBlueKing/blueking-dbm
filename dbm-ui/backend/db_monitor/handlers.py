...
def get_policy_threshold(params):
    db_type = params["db_type"]
    policy_code = params["policy_code"]
    bk_biz_id = params["bk_biz_id"]
    cluster_ids = [int(cluster_id) for cluster_id in params.get("cluster_id", "").split(",") if cluster_id]
    ...
    # 集群id必传, topic和消费组选传
    if not cluster_ids:
        raise ApiRequestError(_("集群id必传"))
    ...
    for policy in sub_policy:
        ...
        # 查询普通类型的策略传参 集群必传, Topic 消费组选传, 可组合; 正常策略的targets里面只有集群维度,
        # 如果出现topic和消费组维度 则会出现集群 topic 消费组三个可以都有值, 最少只有一个有值, 其余没有值代表匹配全部
