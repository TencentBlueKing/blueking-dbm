...
class PeerAbnormalSummary:
    ...
    @property
    def rs_key(self) -> str:
        """同副本集键：域名 + shard/set。无域名时不要退化成 '|'."""
        return make_rs_key(self.cluster_domain, self.shard or self.set_name or "")


def make_rs_key(cluster_domain: str, group: str) -> str:
    """同副本集键：域名|shard 或 域名|set。必须带域名，避免无关主机撞成 '|'."""
    return f"{(cluster_domain or '').strip()}|{(group or '').strip()}"


def is_usable_rs_key(rs_key: str) -> bool:
    """空域名的键（如 '|'）不能参与同 RS 限流，否则会把无关主机并成一组。"""
    if not rs_key or "|" not in rs_key:
        return False
    domain, _group = rs_key.split("|", 1)
