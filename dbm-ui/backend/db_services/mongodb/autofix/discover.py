...
def _set_group_from_storage(storage: StorageInstance, cluster) -> str:
    """副本集用集群名；分片用 seg_range / shard_name；都拿不到则用 host:ip 隔离，避免空 group 撞车。"""
    dtl = next((row for row in storage.nosqlstoragesetdtl_set.all() if row.cluster_id == cluster.id), None)
    if dtl and dtl.seg_range:
        return dtl.seg_range
    try:
        ext = storage.mongodbstorageinstanceext
        if ext and ext.shard_name:
            return ext.shard_name
    except Exception:  # noqa: BLE001
        pass
    if cluster.cluster_type == ClusterType.MongoReplicaSet.value:
        return cluster.name or ""
    return f"host:{storage.machine.ip}"


def _rs_key_for_storage(storage: StorageInstance, summary: Optional[PeerAbnormalSummary] = None) -> Optional[str]:
    cluster = storage.cluster.first()
    if cluster is None:
        if summary and summary.cluster_domain:
            group = summary.shard or summary.set_name or f"host:{storage.machine.ip}"
            return make_rs_key(summary.cluster_domain, group)
        return None
    domain = cluster.immute_domain or (summary.cluster_domain if summary else "")
    if cluster.cluster_type == ClusterType.MongoShardedCluster.value:
        group = (summary.shard or summary.set_name if summary else "") or _set_group_from_storage(storage, cluster)
    else:
        group = cluster.name or (summary.set_name if summary else "") or ""
    if not domain:
        return None
    return make_rs_key(domain, group)
...
def _aggregate_by_ip(
    ...
    for name, summary in sustained.items():
        ...
        cand.names.add(name)
        rs_key = _rs_key_for_storage(storage, summary)
        if rs_key:
            cand.rs_keys.add(rs_key)
            # 回填 summary，避免后续仍用空维度 rs_key
            if not summary.cluster_domain:
                summary.cluster_domain = cluster.immute_domain
            if not summary.shard and not summary.set_name:
                summary.set_name = rs_key.split("|", 1)[1]
...
def _active_core_names_by_rs() -> Dict[str, Set[str]]:
    """进行中 Core 按创建时写入的 rs_keys 登记异常 name；无 rs_keys 时分片用 host 隔离，避免跨 shard 误伤。"""
    ...
    for core in MongoAutofixCore.objects.filter(deal_status__in=MONGO_AUTOFIX_ACTIVE_STATUSES).iterator():
        ...
        for fm in core.fault_machines or []:
            if isinstance(fm, dict):
                rs_keys.extend(fm.get("rs_keys") or [])
        if not rs_keys:
            if core.cluster_type == ClusterType.MongoReplicaSet.value:
                cluster = Cluster.objects.filter(id=core.cluster_id).only("immute_domain", "name").first()
                group = (cluster.name if cluster else "") or ""
                domain = (cluster.immute_domain if cluster else "") or core.immute_domain
                rs_keys = [make_rs_key(domain, group)]
            else:
                rs_keys = [make_rs_key(core.immute_domain, f"host:{core.ip}")]
        for rs_key in rs_keys:
            result[rs_key].update(names)
    return result
...
def discover_mongod_backend_faults() -> List[MongoAutofixCore]:
    ...
    candidates = _apply_same_rs_gates(candidates, by_ip)
