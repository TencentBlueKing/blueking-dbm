...
from backend.db_services.mongodb.autofix.mongodb_autofix_ticket import (
    create_mongod_ensure_start_ticket as _create_mongod_ensure_start_ticket,
)
from backend.db_services.mongodb.autofix.mongodb_autofix_ticket import (
    create_mongod_fix_status_ticket as _create_mongod_fix_status_ticket,
)
from backend.db_services.mongodb.autofix.mongodb_autofix_ticket import (
    create_mongod_manual_ticket as _create_mongod_manual_ticket,
)
from backend.db_services.mongodb.autofix.mongodb_autofix_ticket import (
...
def create_mongod_ensure_start_ticket(core: MongoAutofixCore) -> Optional[Ticket]:
    """PRE process_bad 后出进程拉起单；委托 mongodb_autofix_ticket 实现并回写 Core."""
    try:
        creator = _mongodb_dba(core.bk_biz_id)
        ticket = _create_mongod_ensure_start_ticket(core, creator=creator)
        if ticket is None:
            return None
        core.ticket_id = ticket.id
        core.deal_status = MongoAutofixStatus.TICKETED.value
        ...
        logger.exception("create_mongod_ensure_start_ticket fail core=%s err=%s", core.id, exc)
        core.deal_status = MongoAutofixStatus.FAIL.value
        core.status_version = str(exc)[:64]
        core.save(update_fields=["deal_status", "status_version", "update_at"])
...
def create_mongod_manual_ticket(core: MongoAutofixCore) -> Optional[Ticket]:
    """PRE auth_error / gse_inconclusive 后出人工处理单；委托 mongodb_autofix_ticket 并回写 Core."""
    try:
        creator = _mongodb_dba(core.bk_biz_id)
        ticket = _create_mongod_manual_ticket(core, creator=creator)
        if ticket is None:
            return None
        core.ticket_id = ticket.id
        core.deal_status = MongoAutofixStatus.TICKETED.value
        ...
        logger.exception("create_mongod_manual_ticket fail core=%s err=%s", core.id, exc)
        core.deal_status = MongoAutofixStatus.FAIL.value
        core.status_version = str(exc)[:64]
        core.save(update_fields=["deal_status", "status_version", "update_at"])
