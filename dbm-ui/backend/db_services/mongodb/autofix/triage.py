...
# 兼容旧名：原 IGNORE 已改为显式人工跟单
ACTION_IGNORE = ACTION_MANUAL
...
CONFIRM_GSE_INCONCLUSIVE = "gse_inconclusive"


def decide_autofix_action(
    disk_rw_ok: Optional[int],
    gse_alive: Optional[bool],
    ...
    2. GSE 明确不可达 → REPLACE / machine_dead（探测失败/超时不算死机）
    3. GSE ok but datadir not writable → REPLACE / disk_dead
    4. DRS auth error → MANUAL / auth_error（显式人工跟单单）
    5. GSE 探测不确定且 DRS 失败 → MANUAL / gse_inconclusive（显式人工跟单，避免误替换）
    6. DRS fail (non-auth) → RELOAD / process_bad
    7. DRS success → FIX_STATUS / login_ok（探测成功只修元数据状态，不重启）

    gse_alive: True 可达, False 作业已结束且失败, None 探测不确定
    ...
    if drs_auth_error:
        return ACTION_MANUAL, CONFIRM_AUTH_ERROR

    if gse_alive is None and not drs_ok:
        return ACTION_MANUAL, CONFIRM_GSE_INCONCLUSIVE
