...
class MongoScriptSyntaxCheckSerializer(serializers.Serializer):
    ...
    script_content = serializers.ListField(
        help_text=_("脚本内容列表"), child=serializers.CharField(help_text=_("脚本内容")), required=False
    )
    ...
    script_filenames = serializers.ListField(
        help_text=_("脚本文件名列表(已在制品库中的文件)"), child=serializers.CharField(), required=False
    )
