...
REDIS_DELETE_RATE = {
    ...
    ClusterType.TendisTendisSSDInstance.value: {"default": 5000, "rate_list": [5000, 10000, 20000]},
...
SQL_QUERY_STORAGE_INSTANCES = (
    ...
    "LEFT JOIN db_meta_cluster cluster ON cluster.id = c.cluster_id "
    "{where} "
...
SQL_QUERY_COUNT_INSTANCES = (
    f"SELECT COUNT(*) FROM (({SQL_QUERY_STORAGE_INSTANCES}) UNION ({SQL_QUERY_PROXY_INSTANCES})) as combined "
)
