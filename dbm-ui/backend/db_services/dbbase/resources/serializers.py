...
class ListMachineSLZ(serializers.Serializer):
    ...
    region = serializers.CharField(help_text=_("地域(多个过滤以逗号分隔)"), required=False)
    bk_sub_zone = serializers.CharField(help_text=_("园区(多个过滤以逗号分隔)"), required=False)
    bk_svr_device_cls_name = serializers.CharField(help_text=_("机型(多个过滤以逗号分隔)"), required=False)
