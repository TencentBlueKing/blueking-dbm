...
@dataclass
class SQLServerDBAuthorizeMeta(BaseAuthorizeMeta):
    """授权元信息的数据模型"""

    sqlserver_users: List[Dict] = None  # 多个账号规则

    @classmethod
    def from_excel_data(cls, excel_data: Dict, cluster_type: str) -> "SQLServerDBAuthorizeMeta":
...
@dataclass
class SQLServerExcelAuthorizeMeta(BaseExcelAuthorizeMeta):
    """excel授权信息的数据模型"""
