...
import (
	"dbm-services/oracle/db-tools/dbactuator/pkg/buildinfo"
	"encoding/base64"
	"fmt"
	"log"
	"os"

	"dbm-services/oracle/db-tools/dbactuator/pkg/jobmanager"
...
func exitWithError(err error) {
	log.Println("err:", err)
	fmt.Println(buildinfo.VersionInfo())
	os.Exit(-1)
}
...
var RootCmd = &cobra.Command{
	Use:   "oracle-dbactuator",
	Short: "oracle原子任务合集",
	Long:  `oracle原子任务合集,包含oracle执行脚本等原子任务。`,
...
func init() {
	...
	RootCmd.PersistentFlags().StringVarP(&dataDir, "data_dir", "D", "",
		"数据保存路径,亦可通过环境变量 ORACLE_DATA_DIR 指定")
	RootCmd.PersistentFlags().StringVarP(&backupDir, "backup_dir", "B", "",
		"备份保存路径,亦可通过环境变量ORACLE_BACKUP_DIR指定")
	...
	RootCmd.PersistentFlags().StringVarP(&atomJobList, "atom-job-list", "A", "",
		"多个原子任务名用','分割,如 oracle_install,oracle_replicaof")
