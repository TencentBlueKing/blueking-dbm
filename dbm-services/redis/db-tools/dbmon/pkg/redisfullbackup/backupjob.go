...
func (job *Job) getSqlDB() {
	...
	job.Err = job.sqdb.AutoMigrate(&models.RedisFullbackupHistorySchema{})
	if job.Err != nil {
	...
	for _, svrItem := range job.Conf.Servers {
		...
		for _, port := range svrItem.ServerPorts {
			...
			task, job.Err = NewFullBackupTask(svrItem.BkBizID, svrItem.BkCloudID,
				...
				job.Conf.BackupClientStrorageType, job.Conf.RedisFullBackup.FullBackupTag,
				job.sqdb)
			if job.Err != nil {
				return
			}
...
func (job *Job) DeleteTooOldFullBackup(port int) {
	var toDoRows []models.RedisFullbackupHistorySchema
	var err error
	var removeOK bool
	...
	job.Err = job.sqdb.Where("start_time<=? and local_file_removed=?", Days15Ago, 1).
		Delete(&models.RedisFullbackupHistorySchema{}).Error
	if job.Err != nil {
		job.Err = fmt.Errorf(
