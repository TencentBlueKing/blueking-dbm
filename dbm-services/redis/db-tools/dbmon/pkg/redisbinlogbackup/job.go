...
func (job *Job) Run() {
	...
	// 设置默认值，兼容老配置文件
	if job.Conf.RedisBinlogBackup.BinlogBackupTag == "" {
		job.Conf.RedisBinlogBackup.BinlogBackupTag = consts.RedisBinlogTAG
	}
	...
	job.backupClient, job.Err = backupsys.NewCosBackupClient(consts.COSBackupClient,
		consts.COSInfoFile, job.Conf.RedisBinlogBackup.BinlogBackupTag, job.Conf.BackupClientStrorageType)
	if job.Err != nil {
	...
	for _, svrItem := range job.Conf.Servers {
		...
		for _, port := range svrItem.ServerPorts {
			...
			task, job.Err = NewBinlogBackupTask(svrItem.BkBizID, svrItem.BkCloudID,
				...
				job.Reporter,
				job.Conf.BackupClientStrorageType, job.Conf.RedisBinlogBackup.BinlogBackupTag,
				job.sqdb)
			if job.Err != nil {
				return
			}
