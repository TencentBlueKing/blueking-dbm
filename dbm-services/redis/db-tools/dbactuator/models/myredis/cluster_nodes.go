...
type ClusterNodeData struct {
	...
	balance    int `json:"-"` // NOCC:vet/vet(设计如此) (扩缩容)迁移slot时使用
	endSlotIdx int `json:"-"` // NOCC:vet/vet(设计如此)
}
...
// Balance TODO
// balance:the expected number of slots for each tendis during scale up or scale down.
func (n *ClusterNodeData) Balance() int {
	return n.balance
}

// SetBalance TODO
// set
func (n *ClusterNodeData) SetBalance(balance int) {
	n.balance = balance
}

// EndSlotIdx TODO
// slot idx
func (n *ClusterNodeData) EndSlotIdx() int {
	return n.endSlotIdx
}

// SetEndSlotIdx TODO
// slot idx
func (n *ClusterNodeData) SetEndSlotIdx(idx int) {
	n.endSlotIdx = idx
}
