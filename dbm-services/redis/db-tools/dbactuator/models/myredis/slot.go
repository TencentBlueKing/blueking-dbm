...
func DecodeSlotRange(str string) ([]Slot, *ImportingSlot, *MigratingSlot, error) {
	...
	if len(val) == 3 {
		...
		if err != nil {
			mylog.Logger.Error(fmt.Sprintf("DecodeSlotRange fail,err:%v,str:%s", err, str))
		...
		if separator == importingSeparator {
			...
			err = fmt.Errorf("impossible to decode slot %s", str)
...
// SlotSliceInter TODO
func SlotSliceInter(slots01, slots02 []Slot) []Slot {
	m01 := make(map[Slot]bool)
	m02 := make(map[Slot]bool)
	for _, item01 := range slots01 {
		m01[item01] = true
	}
	for _, item02 := range slots02 {
		m02[item02] = true
	}
	ret := []Slot{}
