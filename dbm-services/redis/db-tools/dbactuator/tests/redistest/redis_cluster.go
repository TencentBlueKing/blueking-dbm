...
// ClusterForgetTest ClusterForgetTest
type ClusterForgetTest struct {
	atomredis.ClusterForgetParam
	Err error `json:"-"`
}

// SetClusterMeta SetClusterMeta
func (test *ClusterForgetTest) SetClusterMeta(
	serverIP string, startPort, instNum int) {
	test.ClusterMeta.ClusterType = consts.TendisTypePredixyTendisplusCluster
	test.ClusterMeta.StoragePassword = consts.RedisTestPasswd
	test.ClusterMeta.ImmuteDomain = "hi.kipper.vi.db"
	test.ClusterMeta.RedisMasterSet = []string{}

	for i := 0; i < instNum; i++ {
		test.ClusterMeta.RedisMasterSet = append(test.ClusterMeta.RedisMasterSet,
			fmt.Sprintf("%s:%d", serverIP, startPort+i))
	}
}

// SetForgetList SetForgetList
func (test *ClusterForgetTest) SetForgetList(serverIP string, startPort, instNum int) {
	test.ForgetList = []atomredis.InstanceParam{}
	for i := 0; i < instNum; i++ {
		test.ForgetList = append(
			test.ForgetList, atomredis.InstanceParam{
				IP:   serverIP,
				Port: startPort + i,
			})
...
// RunClusterForget 运行集群Forget节点
func (test *ClusterForgetTest) RunClusterForget() {
	fmt.Println("=========RunClusterForget test start============")

	defer func() {
		if test.Err != nil {
			fmt.Println("=========RunClusterForget test fail============")
		} else {
			fmt.Println("=========RunClusterForget test success============")
		}
	}()
	...
	// fmt.Printf("-------payload(raw)--------\n%s\n\n", string(paramBytes))
	// encodeStr := base64.StdEncoding.EncodeToString(paramBytes)
	instllCmd := fmt.Sprintf(consts.ActuatorTestCmd,
		atomredis.NewRedisClusterForget().Name(), string(paramBytes))
	fmt.Println(instllCmd)
	_, test.Err = util.RunBashCmd(instllCmd, "", nil, 1*time.Hour)
	if test.Err != nil {
		return
...
// ClusterMeetSingleNode 创建redis cluster
func ClusterMeetSingleNode(serverIP string, startPort, instNum int) (err error) {
	// 建立cluster 关系
	replicaPairs := make([]atomredis.ClusterReplicaItem, 0, instNum)
	replicaPairs = append(replicaPairs,
		atomredis.ClusterReplicaItem{MasterIP: serverIP, MasterPort: consts.TestTendisPlusMasterStartPort})
	for i := 0; i < instNum; i++ {
		replicaPairs = append(replicaPairs, atomredis.ClusterReplicaItem{
			MasterIP:   serverIP,
			MasterPort: startPort + i,
		})
	}
