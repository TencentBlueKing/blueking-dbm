...
func CreateConfFileAndKeyFileAndDbTypeFileAndChown(runtime *jobruntime.JobGenericRuntime, authConfFilePath string,
	...
	keyFilePath string, app string, areaId string, dbTypeFilePath string, instanceType string,
	...
	key := GetMd5(strings.Join([]string{app, areaId}, "-"))
...
func AuthGetPrimaryInfo(user string, mongoBin string, username string, password string, ip string, port int) (string,
	error) {
	// 超时时间
	...
	for {
		...
		default:
			cmd := fmt.Sprintf(
				"su %s -c \"%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"rs.isMaster().primary\\\"\"",
				user, mongoBin, username, password, ip, port)
...
func NoAuthGetPrimaryInfo(user string, mongoBin string, ip string, port int) (string, error) {
	// 超时时间
	timeout := time.After(20 * time.Second)
	for {
		select {
		...
		default:
			cmd := fmt.Sprintf(
				"su %s -c \"%s --host %s --port %d --quiet --eval \\\"rs.isMaster().primary\\\"\"",
				user, mongoBin, ip, port)
			result, err := util.RunBashCmd(
				cmd,
				"", nil,
				10*time.Second)
...
func InitiateReplicasetGetPrimaryInfo(user string, mongoBin string, ip string, port int) (string, error) {
	cmd := fmt.Sprintf(
		"su %s -c \"%s --host %s --port %d --quiet --eval \\\"rs.isMaster().primary\\\"\"",
		user, mongoBin, ip, port)
	result, err := util.RunBashCmd(
...
func AuthCheckUser(user string, mongoBin string, username string, password string, ip string, port int, authDb string,
	checkUsername string) (bool, error) {
	cmd := fmt.Sprintf(
		"su %s -c \"%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"db.getMongo().getDB('%s').getUser('%s')\\\"\"",
		user, mongoBin, username, password, ip, port, authDb, checkUsername)
	result, err := util.RunBashCmd(
		cmd,
		"", nil,
		10*time.Second)
...
// NoAuthCheckUser 检查user是否存在
// func NoAuthCheckUser(user string, mongoBin string, ip string, port int, authDb string,
//	checkUsername string) (bool, error) {
//	cmd := fmt.Sprintf(
//		"su %s -c \"%s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"db.getMongo().getDB('%s').getUser('%s')\\\"\"",
//		user, mongoBin, ip, port, authDb, checkUsername)
//	result, err := util.RunBashCmd(
//		cmd,
//		"", nil,
//		10*time.Second)
...
func GetNodeInfo(user string, mongoBin string, ip string, port int, username string, password string,
	...
	cmdStatus := fmt.Sprintf(
		"su %s -c '%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \"rs.status().members\"'",
		user, mongoBin, username, password, ip, port)
	cmdConf := fmt.Sprintf(
		"su %s -c '%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \"rs.conf().members\"'",
		user, mongoBin, username, password, ip, port)
...
// AuthRsStepDown 主备切换
func AuthRsStepDown(user string, mongoBin string, ip string, port int, username string, password string) (bool, error) {
	cmd := fmt.Sprintf(
		"su %s -c \"%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"rs.stepDown()\\\"\"",
		user, mongoBin, username, password, ip, port)
	_, _ = util.RunBashCmd(
		cmd,
		"", nil,
		10*time.Second)
	primaryInfo, err := AuthGetPrimaryInfo(user, mongoBin, username, password, ip, port)
	if err != nil {
		return false, err
	}
...
func NoAuthRsStepDown(user string, mongoBin string, ip string, port int) (bool, error) {
	cmd := fmt.Sprintf(
		"su %s -c \"%s  --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"rs.stepDown()\\\"\"",
		user, mongoBin, ip, port)
	_, _ = util.RunBashCmd(
		cmd,
		"", nil,
		10*time.Second)
	primaryInfo, err := NoAuthGetPrimaryInfo(user, mongoBin, ip, port)
	if err != nil {
		return false, err
	}
	if primaryInfo == strings.Join([]string{ip, strconv.Itoa(port)}, ":") {
...
func CheckBalancer(user string, mongoBin string, ip string, port int, username string, password string) (string,
	error) {
	cmd := fmt.Sprintf(
		"su %s -c \"%s  -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"sh.getBalancerState()\\\"\"",
		user, mongoBin, username, password, ip, port)
	result, err := util.RunBashCmd(
		cmd,
		"", nil,
...
func GetProfilingLevel(user string, mongoBin string, ip string, port int, username string, password string,
	dbName string) (int, error) {
	cmd := fmt.Sprintf(
		"su %s -c \"%s  -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"db.getMongo().getDB('%s').getProfilingLevel()\\\"\"",
		user, mongoBin, username, password, ip, port, dbName)
	result, err := util.RunBashCmd(
		cmd,
		"", nil,
		10*time.Second)
...
func SetProfilingLevel(user string, mongoBin string, ip string, port int, username string, password string,
	dbName string, level int) error {
	cmd := fmt.Sprintf(
		"su %s -c \"%s  -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"db.getMongo().getDB('%s').setProfilingLevel(%d)\\\"\"",
		user, mongoBin, username, password, ip, port, dbName, level)
