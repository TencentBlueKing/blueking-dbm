...
func (m *JobGenericManager) atomjobsMapperLoading() {
	m.once.Do(func() {
		...
		m.atomJobMapper[atomsys.NewRedisCleanResidualExporter().Name()] = atomsys.NewRedisCleanResidualExporter
		m.atomJobMapper[atomsys.NewReverseAPIConfig().Name()] = atomsys.NewReverseAPIConfig
		// 迁移
