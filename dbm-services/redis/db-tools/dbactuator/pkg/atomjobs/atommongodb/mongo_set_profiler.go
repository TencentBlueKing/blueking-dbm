...
func (s *SetProfiler) Init(runtime *jobruntime.JobGenericRuntime) error {
	...
	// 获取primary信息
	info, err := common.AuthGetPrimaryInfo(s.OsUser, s.Mongo, s.ConfParams.AdminUsername, s.ConfParams.AdminPassword,
...
func (s *SetProfiler) setProfileSize() error {
	// 获取profile级别
	status, err := common.GetProfilingLevel(s.OsUser, s.Mongo, s.ConfParams.IP, s.ConfParams.Port,
	...
	if status != 0 {
		if err = common.SetProfilingLevel(s.OsUser, s.Mongo, s.ConfParams.IP, s.ConfParams.Port, s.ConfParams.AdminUsername,
	...
	// 删除profile.system
	cmd := fmt.Sprintf(
		"su %s \"%s  -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"db.getMongo().getDB('%s').system.profile.drop()\\\"\"",
		s.OsUser, s.Mongo, s.ConfParams.AdminUsername, s.ConfParams.AdminPassword, s.ConfParams.IP, s.ConfParams.Port,
		s.ConfParams.DbName)
	...
	cmd = fmt.Sprintf(
		"su %s \"%s  -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \\\"db.getMongo().getDB('%s').createCollection('system.profile',{ capped: true, size:%d })\\\"\"",
		s.OsUser, s.Mongo, s.ConfParams.AdminUsername, s.ConfParams.AdminPassword, s.ConfParams.IP, s.ConfParams.Port,
