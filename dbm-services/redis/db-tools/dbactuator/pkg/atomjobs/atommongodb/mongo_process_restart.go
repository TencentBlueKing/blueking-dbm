...
func (r *MongoRestart) checkPrimary() (bool, error) {
	...
	if r.ConfParams.AdminUsername != "" && r.ConfParams.AdminPassword != "" {
		info, err = common.AuthGetPrimaryInfo(r.OsUser, r.Mongo, r.ConfParams.AdminUsername,
			r.ConfParams.AdminPassword, r.ConfParams.IP, r.ConfParams.Port)
	} else {
		info, err = common.NoAuthGetPrimaryInfo(r.OsUser, r.Mongo,
...
func (r *MongoRestart) RsStepDown() error {
	if r.ConfParams.InstanceType != "mongos" {
		...
		if flag1 == true {
			...
			var flag2 bool
			if r.ConfParams.AdminUsername != "" && r.ConfParams.AdminPassword != "" {
				flag2, err = common.AuthRsStepDown(r.OsUser, r.Mongo, r.ConfParams.IP, r.ConfParams.Port,
					r.ConfParams.AdminUsername, r.ConfParams.AdminPassword)
			} else {
				flag2, err = common.NoAuthRsStepDown(r.OsUser, r.Mongo, r.ConfParams.IP, r.ConfParams.Port)
			}
			if err != nil {
