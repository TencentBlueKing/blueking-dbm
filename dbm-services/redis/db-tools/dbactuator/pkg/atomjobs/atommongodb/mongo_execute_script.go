...
func (e *ExecScript) Init(runtime *jobruntime.JobGenericRuntime) error {
	...
	if e.ConfParams.Type == "replicaset" {
		primaryInfo, err := common.AuthGetPrimaryInfo(e.OsUser, e.Mongo, e.ConfParams.AdminUsername,
		...
		if e.ConfParams.Secondary == true {
			_, _, _, _, _, memberInfo, err := common.GetNodeInfo(e.OsUser, e.Mongo, e.ConfParams.IP, e.ConfParams.Port,
...
func (e *ExecScript) creatScriptFile() error {
	...
	if _, err = util.RunBashCmd(
		...
		e.runtime.Logger.Error(fmt.Sprintf("chown auth config file fail, error:%s", err))
		return fmt.Errorf("chown auth config file fail, error:%s", err)
	}
	e.runtime.Logger.Info("start to execute chown command for script file successfully")
	return nil
}
...
func (e *ExecScript) execScript() error {
	...
	cmd := fmt.Sprintf(
		"su %s -c \"%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet  %s > %s\"",
		e.OsUser, e.Mongo, e.ConfParams.AdminUsername, e.ConfParams.AdminPassword, e.execIP, e.execPort,
		e.ScriptFilePath, e.ResultFilePath)
	cmdX := fmt.Sprintf(
		"su %s -c \"%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet  %s > %s\"",
		e.OsUser, e.Mongo, e.ConfParams.AdminUsername, "xxx", e.execIP, e.execPort,
		e.ScriptFilePath, e.ResultFilePath)
	if _, err := util.RunBashCmd(
		cmd,
		"", nil,
