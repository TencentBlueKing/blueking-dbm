...
func (u *AddUser) Init(runtime *jobruntime.JobGenericRuntime) error {
	...
	if u.ConfParams.InstanceType == "mongos" {
		...
		if u.ConfParams.AdminUsername != "" && u.ConfParams.AdminPassword != "" {
			info, err = common.AuthGetPrimaryInfo(u.OsUser, u.Mongo, u.ConfParams.AdminUsername,
...
func (u *AddUser) checkUser() (bool, error) {
	...
	if u.ConfParams.AdminUsername != "" && u.ConfParams.AdminPassword != "" {
		flag, err = common.AuthCheckUser(u.OsUser, u.Mongo, u.ConfParams.AdminUsername, u.ConfParams.AdminPassword,
			u.PrimaryIP, u.PrimaryPort, u.ConfParams.AuthDb, u.ConfParams.Username)
	} else {
		flag, err = common.AuthCheckUser(u.OsUser, u.Mongo, u.ConfParams.Username, u.ConfParams.Password,
...
func (u *AddUser) execScript() error {
	...
	if u.ConfParams.AdminUsername != "" && u.ConfParams.AdminPassword != "" {
		...
		cmd = fmt.Sprintf(
			"su %s -c '%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \"%s\" %s'",
			u.OsUser, u.Mongo, u.ConfParams.AdminUsername, u.ConfParams.AdminPassword, u.PrimaryIP, u.PrimaryPort,
		...
		cmd = fmt.Sprintf(
			"su %s -c '%s  --host %s --port %d  --quiet --eval \"%s\" %s'",
			u.OsUser, u.Mongo, "127.0.0.1", u.ConfParams.Port, u.ScriptContent, u.ConfParams.AuthDb)
		if u.ConfParams.AdminUsername != "" && u.ConfParams.AdminPassword != "" {
