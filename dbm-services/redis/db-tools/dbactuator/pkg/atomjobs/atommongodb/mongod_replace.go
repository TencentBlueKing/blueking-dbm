...
// MongoDReplaceConfParams 参数  // cluster 关闭stopBalancer
type MongoDReplaceConfParams struct {
	IP             string `json:"ip" validate:"required"` // 执行节点
	Port           int    `json:"port" validate:"required"`
	SourceIP       string `json:"sourceIP" validate:"required"` // 源节点
	SourcePort     int    `json:"sourcePort" validate:"required"`
	SourceDown     bool   `json:"sourceDown"` // 源端已down机 true：已down false：未down
	AdminUsername  string `json:"adminUsername" validate:"required"`
	AdminPassword  string `json:"adminPassword" validate:"required"`
	TargetIP       string `json:"targetIP" validate:"required"` // 目标节点
...
func (r *MongoDReplace) Init(runtime *jobruntime.JobGenericRuntime) error {
	...
	// 获取primary信息
	info, err := common.AuthGetPrimaryInfo(r.OsUser, r.Mongo, r.ConfParams.AdminUsername, r.ConfParams.AdminPassword,
	...
	// 获取源端的配置信息
	_, _, _, hidden, priority, _, err := common.GetNodeInfo(r.OsUser, r.Mongo, r.PrimaryIP, r.PrimaryPort,
...
func (r *MongoDReplace) execAddTargetScript() error {
	// 检查target是否已经存在
	flag, _, _, _, _, _, _ := common.GetNodeInfo(r.OsUser, r.Mongo, r.PrimaryIP, r.PrimaryPort,
	...
	cmd := fmt.Sprintf(
		"su %s -c '%s -u %s -p %s --host %s --port %d --authenticationDatabase=admin --quiet --eval \"%s\"'",
		r.OsUser, r.Mongo, r.ConfParams.AdminUsername, r.ConfParams.AdminPassword, r.PrimaryIP,
...
func (r *MongoDReplace) checkTargetStatus() {
	...
	for {
		select {
		case <-timeout:
			r.runtime.Logger.Error("get target status timeout")
		default:
			_, _, status, _, _, _, err := common.GetNodeInfo(r.OsUser, r.Mongo, r.PrimaryIP, r.PrimaryPort,
...
func (r *MongoDReplace) primaryStepDown() error {
	if r.ConfParams.SourceIP == r.PrimaryIP && r.ConfParams.SourcePort == r.PrimaryPort {
		...
		flag, err := common.AuthRsStepDown(r.OsUser, r.Mongo, r.PrimaryIP, r.PrimaryPort, r.ConfParams.AdminUsername,
