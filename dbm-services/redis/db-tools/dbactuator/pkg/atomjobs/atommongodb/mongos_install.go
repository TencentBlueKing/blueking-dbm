...
func (s *MongoSInstall) Run() error {
	...
	// 创建配置文件，key文件并修改属主
	if err = s.createConfFileAndKeyFileAndDbTypeFile(); err != nil {
		return err
	}
...
func (s *MongoSInstall) makeConfContent() error {
	...
	clusterId := strings.Join([]string{s.ConfParams.App, s.ConfParams.AreaId, "conf"}, "-")
...
func (s *MongoSInstall) createConfFileAndKeyFileAndDbTypeFile() error {
	...
	if err := common.CreateConfFileAndKeyFileAndDbTypeFileAndChown(
		...
		s.NoAuthConfFileContent, s.KeyFilePath, s.ConfParams.App, s.ConfParams.AreaId, s.DbTypeFilePath,
