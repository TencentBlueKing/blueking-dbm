...
func (incr *TredisRocksDBIncrBack) GetTredisIncrbacks() (backs []*TredisRocksDBIncrBackItem) {
	mylog.Logger.Info("GetTredisIncrbacks start ...")
	layout := "2006-01-02 15:04:05"
	// incr.StartTime 往前24小时
	h, _ := time.ParseDuration("-24h")
	startTime := incr.StartTime.Add(h)
	beginData := startTime.Add(h).Format(layout)
	// // incr.EndTime 往后8小时
	h1, _ := time.ParseDuration("24h")
	endTime := incr.EndTime.Add(h1)
	...
	for _, str01 := range resp.Detail {
		...
		mylog.Logger.Debug("BackupSize:%d", back01.BackupSize)
...
func (incr *TredisRocksDBIncrBack) PullFilesFromBS() {
	if len(incr.ResultSortBinlog) == 0 {
		incr.Err = fmt.Errorf(
			"filename:%s  binlog信息为空(len:%d),无法拉取,请先获取binlog信息再拉取",
...
// - 继续拉取文件;
func (incr *TredisRocksDBIncrBack) PullAllFiles() {
