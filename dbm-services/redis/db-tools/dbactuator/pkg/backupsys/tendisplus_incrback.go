...
// GetTplusIncrbacks 查询特定端口的binlog备份文件
// NOCC:golint/fnsize(设计如此)
func (incr *TplusRocksDBIncrBack) GetTplusIncrbacks() (backs []*TplusRocksDBIncrBackItem) {
	layout := "2006-01-02 15:04:05"
	// incr.StartTime 往前24小时
	h, _ := time.ParseDuration("-24h")
	// incr.StartTime = incr.StartTime.Add(h)
	beginData := incr.StartTime.Add(h).Format(layout)
	// incr.EndTime 往后8小时
	h1, _ := time.ParseDuration("24h")
	endDate := incr.EndTime.Add(h1).Format(layout)
	...
	resp, err := incr.query.QueryFile(params)
	if err != nil {
		incr.Err = fmt.Errorf("QueryFile fail,err:%v,params:%s", err, params)
	...
	mylog.Logger.Info("resp num :%v", resp.Num)
	mylog.Logger.Info("ip:%v,beginDate:%v,endDate:%v", incr.SourceIP, beginData, endDate)
...
// NOCC:golint/fnsize(设计如此)
func (incr *TplusRocksDBIncrBack) GetTplusIncrbacksSpecRocks() {
...
func (incr *TplusIncrBackPull) GetAllIncrBacksInfo() {
	...
	for _, rock01 := range incr.RocksMap {
		...
		go func(rock001 *TplusRocksDBIncrBack) {
			defer wg.Done()
			rock001.GetTplusIncrbacksSpecRocks()
		}(rockItem)
	}
	wg.Wait()
...
func (incr *TplusRocksDBIncrBack) PullFilesFromBS() {
	if len(incr.ResultSortBinlog) == 0 {
		incr.Err = fmt.Errorf(
			"filename:%s rocksdbIdx:%d binlog信息为空(len:%d),无法拉取,请先获取binlog信息再拉取",
	...
	mylog.Logger.Info("PullFilesFromBS TaskidList:%v", taskIds)
	...
	err := incr.recover.WaitForRecoverFinish(recoverParams, incr.SaveHost)
	if err != nil {
		incr.Err = fmt.Errorf("等待备份文件下载失败:%v", err)
		return
	}
...
// PullFilesIfNotExists ..
// - 如果所有 增备(已解压)文件 全部存在,则直接return;
// - 如果所有 增备(未解压)文件 全部存在,且大小校验ok,则直接 return;
// - 否则删除 本地存在的增备(已解压)文件、增备(未解压)文件;
// - 继续拉取文件;
func (incr *TplusRocksDBIncrBack) PullFilesIfNotExists() {
	mylog.Logger.Info("PullFilesIfNotExists start ... ")
	var msg string
...
// PullAllFiles 拉取所有rocksb增备信息
func (pull *TplusIncrBackPull) PullAllFiles() {
	mylog.Logger.Info("PullAllFiles start ... ")
