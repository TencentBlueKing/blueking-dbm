...
func QueryConfItemChanges(req *api.ConfItemChangesQueryReq) (resp []*api.ConfItemChangesQueryRowResp, err error) {
	changes, err := model.QueryConfItemChanges(model.DB.Self, req)
	if err != nil {
		return nil, err
	}
	resp = make([]*api.ConfItemChangesQueryRowResp, 0)
	...
	for _, change := range changes {
		...
		var rowChange = api.ConfItemChangesQueryRowResp{
			ConfigFileDesc: baseFileDesc,
			//ConfNameLc:     change.ConfName,
		}
