...
func (c *MysqlBackupResult) ConsumeClaim2(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	for {
		select {
		case message := <-claim.Messages():
			...
			for _, item := range msg.Items {
				...
				err = c.Db.Table(*c.Sinker.RuntimeConfig.Dsn.Table).Create(&modelObj).Error
				if err != nil {
					if strings.Contains(err.Error(), "Duplicate entry") {
						slog.Warn("insert message", slog.Any("msg", err), slog.String("value", string(message.Value)))
						//err = c.Db.Table(*c.Sinker.RuntimeConfig.Dsn.Table).FirstOrCreate(&modelObj).Error
						sqlBuilder := sq.Replace(*c.Sinker.RuntimeConfig.Dsn.Table).Columns("cluster_address",
							"backup_host",
							"backup_port",
							"mysql_role",
							"shard_value",
						...
						tx := c.Db.Exec(sqlStr, sqlArgs...)
						if tx.Error != nil {
							slog.Error("replace message",
								slog.Any("msg", err), slog.String("value", string(message.Value)))
							continue
							//return err
						}
					} else {
						slog.Error("insert message",
							slog.Any("msg", err), slog.String("value", string(message.Value)))
						continue
						//return err
					}
				} else {
					slog.Info("insert message", slog.String("key", modelObj.Key()))
				}
			}
...
func (c *MysqlBackupResult) HandleMessageBatch(msgs []*sarama.ConsumerMessage, s *Sinker, db *gorm.DB) error {
	if len(msgs) == 0 {
		return nil
	}
