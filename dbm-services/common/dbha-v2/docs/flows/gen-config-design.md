...
`config.Load` 会改写 viper 的全局状态和 `config.Cfg`，污染同进程后续逻辑与单元测试。现有的 `ReloadCmdRunE` 本身也不做 Load，保持一致。
...
probe 收到 reload 后**只写一条日志 `reloading configuration...`，不会重新读取配置文件**（见 [run.go](../../internal/probe/run.go)，admin / analysis / receiver 同样是 stub）。

也就是说 `--reload` 与单独执行 `reload` 完全等价，都不会让运行中的进程用上新配置。要让新 YAML 真正生效，仍然需要 `restart` 或 `ensure`。flag 的帮助文案因此写作 "signal the running probe to reload it"，而不是「应用新配置」，避免误导。
...
- **`--reload` 目前不会让新配置生效**（见 §5.5），需要真正生效请用 `restart` 或 `ensure`。
- **`--reload` 依赖当前工作目录**定位 pid 文件，建议在安装根目录下执行。
