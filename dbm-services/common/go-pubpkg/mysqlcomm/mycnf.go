...
func (m *CnfFile) ParseLogBinBasename(val string) (binlogDir, namePrefix string, err error) {
	...
	if cmutil.IsDirectory(binlogDir) && !cmutil.IsDirectory(val) {
		...
		return "", "", errors.Errorf("expect %s is a dir and %s is not dir", binlogDir, val)
	}
}
...
func GetMyCnfFileName(port int) string {
	return fmt.Sprintf("%s.%d", DefaultMyCnfName, port)
}
...
// GetMysqldKeyValue 增加基础方法，获取myconf上面的某个配置参数值，用于做前置校验的对比
func (m *CnfFile) GetMysqldKeyValue(keyName string) (value string, err error) {
	mysqld, err := m.Cfg.GetSection(MysqldSec)
	if err != nil {
		return "", err
	}
