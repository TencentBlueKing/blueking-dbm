...
// Upload TODO
func (b *BkRepoClient) Upload(filepath, filname, repopath string) (*BkRepoRespone, error) {
	return UploadDirectToBkRepo(filepath, b.getBaseUrl()+path.Join("/", repopath, filname), b.BkRepoUser, b.BkRepoPwd)
}
...
func UploadDirectToBkRepo(filepath string, targetURL string, username string, password string) (*BkRepoRespone, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("返回码非200 http code: %d,body:%s\n", resp.StatusCode, string(b))
	}
	var baseResp BkRepoRespone
	if err = json.NewDecoder(resp.Body).Decode(&baseResp); err != nil {
		return nil, err
