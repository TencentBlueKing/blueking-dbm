...
func NewMysqlRawWriter(dsn *InstanceDsn) (*MysqlRawWriter, error) {
	if dsn == nil {
	...
	return &MysqlRawWriter{db: db, dbGorm: dbForMigrate}, nil
}
...
func (w *MysqlRawWriter) Type() string {
	return "mysql_raw"
}
func (w *MysqlRawWriter) AutoMigrate(m interface{}) error {
	slog.Info("MysqlRawWriter run common migrate for", slog.Any("model", m))
...
func (w *MysqlRawWriter) OnDuplicate(objs interface{}) error {
	return nil
}

func (w *MysqlRawWriter) SetWriteMode(mode string) {
	w.writeMode = mode
}
func (w *MysqlRawWriter) GetWriteMode() string {
	return w.writeMode
}

func (w *MysqlRawWriter) GormDB() *gorm.DB {
	return w.dbGorm
}
func (w *MysqlRawWriter) CloseGormDB() error {
	db, _ := w.dbGorm.DB()
	return db.Close()
}
...
// 使用指定的键顺序
func convertMapSliceToSliceSliceWithKeys(mapSlice []map[string]interface{}, keys []string, parseTime bool) [][]interface{} {
	if len(mapSlice) == 0 {
		return [][]interface{}{}
	}

	// 创建结果切片
	result := make([][]interface{}, len(mapSlice))

	// 每一行：按指定键顺序的值
	for i, m := range mapSlice {
		result[i] = make([]interface{}, len(keys))
		for j, key := range keys {
			if value, exists := m[key]; exists {
				if t, ok := value.(time.Time); ok && parseTime {
					result[i][j] = t.UTC() // 如果是时间类型，转换为 UTC 格式
				} else {
					result[i][j] = value
				}
			} else {
				result[i][j] = nil // 如果键不存在，使用 nil
			}
		}
	}
