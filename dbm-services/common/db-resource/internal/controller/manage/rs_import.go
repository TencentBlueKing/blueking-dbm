...
func (p ImportMachParam) transHostInfoToDbModule(h *cc.Host, bkCloudId int, label []byte) model.TbRpDetail {
	return buildTbRpItem(h, p.ForBiz, p.BkBizId, bkCloudId, p.RsType, label)
}

func buildTbRpItem(h *cc.Host, forBizId, bkbizId, bkCloudId int, rsType string, label []byte) model.TbRpDetail {
	osType := h.BkOsType
