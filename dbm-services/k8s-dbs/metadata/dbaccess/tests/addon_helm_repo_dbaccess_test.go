...
func InitAddonHelmRepoTb() (*gorm.DB, error) {
	db, err := gorm.Open(mysql.Open(constant.MySQLTestURL), &gorm.Config{})
	if err != nil {
		fmt.Println("Failed to connect to database")
		return nil, err
	}
	if err := db.Exec("DROP TABLE IF EXISTS tb_addon_helm_repository;").Error; err != nil {
		fmt.Println("Failed to drop tb_addon_helm_repository table")
		return nil, err
	}
	if err := db.AutoMigrate(&model.AddonHelmRepoModel{}); err != nil {
		fmt.Println("Failed to migrate tb_addon_helm_repository table")
		return nil, err
	}
	return db, nil
}

func TestCreateAddonRepo(t *testing.T) {
	db, err := InitAddonHelmRepoTb()
	assert.NoError(t, err)
	dbAccess := dbaccess.NewAddonHelmRepoDbAccess(db)
	repo := &model.AddonHelmRepoModel{
...
func TestDeleteAddonRepo(t *testing.T) {
	db, err := InitAddonHelmRepoTb()
	assert.NoError(t, err)
	dbAccess := dbaccess.NewAddonHelmRepoDbAccess(db)
	repo := &model.AddonHelmRepoModel{
...
func TestUpdateAddonRepo(t *testing.T) {
	db, err := InitAddonHelmRepoTb()
	assert.NoError(t, err)
	dbAccess := dbaccess.NewAddonHelmRepoDbAccess(db)
	repo := &model.AddonHelmRepoModel{
...
func TestGetAddonRepo(t *testing.T) {
	db, err := InitAddonHelmRepoTb()
	assert.NoError(t, err)
	dbAccess := dbaccess.NewAddonHelmRepoDbAccess(db)
	repo := &model.AddonHelmRepoModel{
...
func TestListAddonRepo(t *testing.T) {
	db, err := InitAddonHelmRepoTb()
	assert.NoError(t, err)
...
func TestGetAddonRepoByParams(t *testing.T) {
	db, err := InitAddonHelmRepoTb()
	assert.NoError(t, err)
	dbAccess := dbaccess.NewAddonHelmRepoDbAccess(db)
	repo := &model.AddonHelmRepoModel{
