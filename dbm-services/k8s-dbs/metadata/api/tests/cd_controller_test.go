...
func SetupCdRouter() *gin.Engine {
	r := gin.Default()
	routerGroup := r.Group("/metadata")
	db, _ := initCdTable()
	cdDbaccess := dbaccess.NewK8sCrdClusterDefinitionDbAccess(db)
	cdProvider := provider.NewK8sCrdClusterDefinitionProvider(cdDbaccess)
	cdController := controller.NewCdController(cdProvider)
	{
		routerGroup.GET("/cd/:id", cdController.GetCd)
		routerGroup.DELETE("/cd/:id", cdController.DeleteCd)
		routerGroup.POST("/cd", cdController.CreateCd)
		routerGroup.PUT("/cd/:id", cdController.UpdateCd)
	}
	return r
}

func TestCreateCd(t *testing.T) {
	gin.SetMode(gin.TestMode)
	router := SetupCdRouter()
	// 解析时间字符串为 time.Time 对象
	addDateTime := "2025-01-01 12:00:00"
	layout := "2006-01-02 15:04:05"
	...
	request, err := http.NewRequest("POST", "/metadata/cd", bytes.NewBuffer(requestBody))
	assert.NoError(t, err)
...
func TestGetCd(t *testing.T) {
	...
	err := AddSampleCd()
	assert.NoError(t, err)
	request, _ := http.NewRequest("GET", "/metadata/cd/1", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, request)
	assert.Equal(t, http.StatusOK, w.Code)
	expected := `
...
func TestDeleteCd(t *testing.T) {
	gin.SetMode(gin.TestMode)
	router := SetupCdRouter()
	err := AddSampleCd()
	assert.NoError(t, err)
	request, _ := http.NewRequest("DELETE", "/metadata/cd/1", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, request)
	assert.Equal(t, http.StatusOK, w.Code)
...
func TestUpdateCd(t *testing.T) {
	...
	request, err := http.NewRequest("PUT", "/metadata/cd/1", bytes.NewBuffer(requestBody))
	assert.NoError(t, err)
	request.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	router.ServeHTTP(w, request)
