...
func SetupCmpdRouter() *gin.Engine {
	...
	db, _ := initCmpdTable()
	cmpdDbaccess := dbaccess.NewK8sCrdComponentDefinitionDbAccess(db)
	cmpdProvider := provider.NewK8sCrdComponentDefinitionProvider(cmpdDbaccess)
...
func TestCreateCmpd(t *testing.T) {
	...
	expected := `
	{
		...
		"data": {
			"id": 1,
			"addon_id": 1,
			"componentdefinition_name": "surrealdb",
			"metadata": "{\"namespace\":\"default\"}",
			"spec": "{\"replicas\":1}",
			"active": true,
			"description": "just for test",
