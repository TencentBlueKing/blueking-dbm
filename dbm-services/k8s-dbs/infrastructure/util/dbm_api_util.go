...
func AsyncDomainCreate(
	...
	g.Go(func() error {
		...
		createReq, err := BuildCreateDomainRequest(clusterEntity, serviceEntity)
		if err != nil {
			return fmt.Errorf("BuildCreateDomainRequest failed: %w", err)
		}

		if _, createErr := dbmAPIService.SyncDomainCreated(createReq); createErr != nil {
			return fmt.Errorf("SyncDomainCreated failed: %w", createErr)
		}
		createdDomain = createReq.Domain
		return nil
	})
	...
	go func() {
		...
		if serviceProvider == nil || createdDomain == "" {
			return
		}
