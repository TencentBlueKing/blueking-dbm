...
func GenReplaceReassignmentJSON(zk string, oldBrokerIds, newBrokerIds []string) error {

	newBrokersStr := strings.Join(newBrokerIds[:], ",")
	planJSONFile := cst.PlanJSONFile
	extraCmd := fmt.Sprintf("%s --topics-to-move-json-file %s/topic.json --generate --zookeeper %s --broker-list %s >%s",
		cst.DefaultReassignPartitionsBin, cst.DefaultKafkaEnv, zk, newBrokersStr, planJSONFile)
