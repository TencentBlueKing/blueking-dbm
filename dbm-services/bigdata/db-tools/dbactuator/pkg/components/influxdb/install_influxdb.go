...
func (i *InstallInfluxdbComp) InstallSupervisor() (err error) {
	...
	if err = sedEsConfig(i.InfluxdbEnvDir); err != nil {
		return err
	}
...
func sedEsConfig(influxdbEnvDir string) (err error) {
	extraCmd := fmt.Sprintf("sed -i 's/esenv/influxdbenv/g' %s",
		influxdbEnvDir+"/supervisor/check_supervisord.sh")
	if _, err = osutil.ExecShellCommand(false, extraCmd); err != nil {
		logger.Error("%s execute failed, %v", extraCmd, err)
		return err
	}
	extraCmd = fmt.Sprintf("sed -i 's/esenv/influxdbenv/g' %s",
		influxdbEnvDir+"/supervisor/conf/supervisord.conf")
	if _, err = osutil.ExecShellCommand(false, extraCmd); err != nil {
		logger.Error("%s execute failed, %v", extraCmd, err)
		return err
	}
	extraCmd = fmt.Sprintf("sed -i 's/esenv/influxdbenv/g' %s", influxdbEnvDir+"/supervisor/bin/supervisorctl")
	if _, err = osutil.ExecShellCommand(false, extraCmd); err != nil {
		logger.Error("%s execute failed, %v", extraCmd, err)
		return err
	}
	extraCmd = fmt.Sprintf("sed -i 's/esenv/influxdbenv/g' %s", influxdbEnvDir+"/pypy-5.9.0/bin/supervisord")
	if _, err = osutil.ExecShellCommand(false, extraCmd); err != nil {
		logger.Error("%s execute failed, %v", extraCmd, err)
		return err
	}
	extraCmd = fmt.Sprintf("sed -i 's/esenv/influxdbenv/g' %s", influxdbEnvDir+"/python/bin/supervisorctl")
	if _, err = osutil.ExecShellCommand(false, extraCmd); err != nil {
		logger.Error("%s execute failed, %v", extraCmd, err)
		return err
	}
	extraCmd = fmt.Sprintf("rm %s ", influxdbEnvDir+"/supervisor/conf/elasticsearch.ini")
	if _, err = osutil.ExecShellCommand(false, extraCmd); err != nil {
		logger.Error("%s execute failed, %v", extraCmd, err)
		return err
	}
