...
// ReloadVMSelectAct 定义了一个结构体，用于表示部署 vmselect 实例的行为。
type ReloadVMSelectAct struct {
...
// ReloadVMSelectCommand 创建并返回一个部署 vmselect 命令的实例。
func ReloadVMSelectCommand() *cobra.Command {
	act := ReloadVMSelectAct{
		BaseOptions: subcmd.GBaseOptions, // 初始化 BaseOptions
	}
	cmd := &cobra.Command{
		Use:     "reload_vmselect",                                                         // 命令的使用方式
		Short:   "reload vmselect实例",                                                       // 命令的简短描述
		Example: fmt.Sprintf(`dbactuator vm reload_vmselect %s`, subcmd.CmdBaseExapmleStr), // 命令的示例
...
func (d *ReloadVMSelectAct) Validate() (err error) {
	return d.BaseOptions.Validate() // 调用 BaseOptions 的验证方法
}

// Init 初始化函数，用于初始化操作。
func (d *ReloadVMSelectAct) Init() (err error) {
	logger.Info("ReloadVMSelectAct Init")                   // 记录日志
...
func (d *ReloadVMSelectAct) Rollback() (err error) {
...
// Run TODO
func (d *ReloadVMSelectAct) Run() (err error) {
	steps := subcmd.Steps{
		{
			FunName: "Reload vmselect",
			Func:    d.Service.ReloadVMSelect,
		},
	}
	...
	logger.Info("reload_vmselect successfully")
	return nil
}
