...
func StopProcessCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "stop_process",                                                            // 命令用法
		Short:   "停止doris进程",                                                               // 命令简短描述
		Example: fmt.Sprintf(`dbactuator doris stop_process %s`, subcmd.CmdBaseExapmleStr), // 命令示例
...
func (d *StopProcessAct) Run() (err error) {
	...
	logger.Info("stop_process successfully")
	return nil
}
