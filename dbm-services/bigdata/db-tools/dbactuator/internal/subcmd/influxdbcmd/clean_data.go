...
type CleanDataAct struct {
	*subcmd.BaseOptions
	Service influxdb.CleanDataComp
}

// CleanDataCommand TODO
func CleanDataCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "clean_data",
		Short:   "清理influxdb目录",
		Example: fmt.Sprintf(`dbactuator influxdb clean_data %s`, subcmd.CmdBaseExapmleStr),
...
func (d *CleanDataAct) Run() (err error) {
	...
	logger.Info("clean_data successfully")
	return nil
}
