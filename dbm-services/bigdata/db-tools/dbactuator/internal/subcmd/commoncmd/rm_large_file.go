...
func (p RMLargeFileParam) PreCheck() error {
	if !util.FileExists(p.Filename) {
		return errors.Errorf("file not exists %s", p.Filename)
	} else if util.IsDirectory(p.Filename) {
...
func (p RMLargeFileParam) Start() error {
	if err := osutil.TruncateFile(p.Filename, p.BWLimitMB); err != nil {
...
func RMLargeFileCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "rm-file",
		Short: "限速删除大文件",
		Example: fmt.Sprintf(`dbactuator common rm-file %s %s`,
