...
func (w *WINSOSUser) CreateUser(isTranAdmin bool) error {
	...
	if _, err := StandardPowerShellCommand(
		fmt.Sprintf("New-LocalUser -Name \"%s\" -Password (ConvertTo-SecureString \"%s\" -AsPlainText -Force) "+
			"-Description \"%s\" -AccountNeverExpires -PasswordNeverExpires", w.User, w.Pass, w.Comment),
	); err != nil {
		return err
	}
