...
func (job *Job) runOneServer(svr *config.ConfServerItem, timeout time.Duration, bucket string) error {
	...
	logger.Info("dbtablesize done",
		zap.Int("records", written),
		zap.Int("failed_dbs", failed),
		zap.String("report", reportFile))
	if failed > 0 {
		// 部分库失败仍尽量写出已采集数据，但采集成功指标必须为 0，避免监控误报健康。
		return fmt.Errorf("collect failed for %d database(s), first error: %w", failed, firstErr)
	}
	return nil
}
...
func newSizeRecord(
	...
	return SizeRecord{
		...
		SetName:       svr.SetName,
		SetSn:         strings.TrimPrefix(svr.SetName, svr.ClusterName+"-"),
		DB:            dbName,
