...
func (job *Job) Run() {
	...
	for i := range job.MyConf.Servers {
		...
		info, err := job.collectOne(svr)
		if err != nil {
			job.Logger.Warn("collect datadir failed",
	...
	for _, info := range infos {
		n := deviceCount[deviceKey(info.major, info.minor)]
		if n <= 0 {
			n = 1
		}
...
func (job *Job) collectOne(svr *config.ConfServerItem) (*instanceDiskInfo, error) {
	dbPath, err := resolveDbPath(svr.Port)
	if err != nil {
		return nil, err
	}
	used, err := duDirBytes(dbPath)
	if err != nil {
		return nil, err
	}
