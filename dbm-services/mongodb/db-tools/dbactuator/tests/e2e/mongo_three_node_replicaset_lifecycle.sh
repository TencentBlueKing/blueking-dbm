...
# E2E lifecycle test for 3-node replica set (install flow mirrors
# dbm-ui/backend/flow/engine/bamboo/scene/mongodb/sub_task/replicaset_install.py):
# 1) parallel mongod_install (auth=true) on 3 nodes
# 2) init_replicaset
# 3) add_user dba (root on admin), matching get_add_manager_user_kwargs
# 4) mongo_execute_script create_extra_user (appdba/monitor/appmonitor)
# 5) mongo_execute_script replicaset_init (applyOps/heartbeatOps roles)
# 6) deinstall all members
# Skipped vs prod: save_conf, password service, ExecAddPasswordToDB, default user/DNS/dbmon.
#
# Options:
#   --keep-instance    Keep instances after step 4 (skip deinstall)
#   --deinstall-only   Skip steps 1-4 and run only deinstall
...
WORK_DIR="${TEST_WORK_DIR:-/tmp/mongo-rs3-e2e-${PORT1}}"
...
wait_rs_healthy_noauth() {
  echo "==> Wait rs healthy (noauth)"
  local attempt=0
  while [[ ${attempt} -lt 30 ]]; do
    local total healthy
    total="$($(mongo_shell_bin) --host "${NODE1_IP}" --port "${PORT1}" --quiet --eval "rs.status().members.length" || true)"
    healthy="$($(mongo_shell_bin) --host "${NODE1_IP}" --port "${PORT1}" --quiet --eval "rs.status().members.filter(function(m){return m.stateStr==='PRIMARY'||m.stateStr==='SECONDARY';}).length" || true)"
  ...
  echo "ERROR: replica set not healthy(noauth) within timeout" >&2
  exit 1
}

wait_rs_healthy_auth() {
  echo "==> Wait rs healthy (auth)"
  local attempt=0
...
build_mongo_execute_script_json() {
  ...
  SCRIPT_FILE="${script_file}" OUT_JSON="${out_json}" SCRIPT_NAME="${script_name}" \
    EXEC_IP="${exec_ip}" EXEC_PORT="${PORT1}" EXEC_DBA_USER="${DBA_USER}" EXEC_DBA_PASS="${DBA_PASS}" python3 - <<'PY'
import json
import os
...
build_mongo_execute_script_json "${WORK_DIR}/mongo_execute_script_extra_user.json" \
  "${WORK_DIR}/mongo_extra_user.js" "create_extra_user" "${NODE1_IP}"
build_mongo_execute_script_json "${WORK_DIR}/mongo_execute_script_init_set.json" \
  "${WORK_DIR}/mongo_replicaset_init.js" "replicaset_init" "${NODE1_IP}"

if [[ "${DEINSTALL_ONLY}" == "1" ]]; then
  echo "==> Deinstall-only mode"
  for p in "${PORTS[@]}"; do
    run_atom "e2e-rs3-deinstall-${p}" "mongo_deinstall" "${WORK_DIR}/mongo_deinstall_${p}.json" "e2e-node-${p}"
  done
  cp -f "${ROOT_DIR}"/logs/mongo_actuator_*.log "${LOG_SNAPSHOT_DIR}/" 2>/dev/null || true
...
echo "==> Install 3 nodes (parallel)"
run_os_init "e2e-node-${PORT1}"
install_pids=()
for p in "${PORTS[@]}"; do
  ( run_atom "e2e-rs3-install-${p}" "mongod_install" "${WORK_DIR}/mongod_install_${p}.json" "e2e-node-${p}" ) &
  install_pids+=($!)
done
...
run_atom "e2e-rs3-init-${PORT1}" "init_replicaset" "${WORK_DIR}/init_replicaset.json" "e2e-node-${PORT1}"
wait_rs_healthy_noauth

echo "==> Add dba user (manager)"
run_atom "e2e-rs3-add-user-${PORT1}" "add_user" "${WORK_DIR}/add_user.json" "e2e-node-${PORT1}"

echo "==> mongo_execute_script: create_extra_user"
run_atom "e2e-rs3-exec-extra-user-${PORT1}" "mongo_execute_script" \
  "${WORK_DIR}/mongo_execute_script_extra_user.json" "e2e-node-${PORT1}"

echo "==> mongo_execute_script: replicaset_init (roles)"
run_atom "e2e-rs3-exec-init-set-${PORT1}" "mongo_execute_script" \
