...
# E2E primary node replace test for replica set:
# 1) create 3-node rs (node1 primary, node3 hidden)
# 2) create dba + extra users + replicaset_init scripts
# 3) install target mongod (node4)
# 4) run mongod_replace: source=node1(primary) -> target=node4
# 5) assert node4 joined and old primary node1 removed
...
SET_ID="${TEST_SET_ID:-utRsReplacePrimary}"
...
PORT1="${TEST_PORT1:-28217}"
PORT2="${TEST_PORT2:-28218}"
PORT3="${TEST_PORT3:-28219}"
PORT4="${TEST_PORT4:-28220}"
NODE1_IP="${TEST_NODE1_IP:-127.0.0.6}"
NODE2_IP="${TEST_NODE2_IP:-127.0.0.7}"
NODE3_IP="${TEST_NODE3_IP:-127.0.0.8}"
NODE4_IP="${TEST_NODE4_IP:-127.0.0.9}"
PORTS=("${PORT1}" "${PORT2}" "${PORT3}" "${PORT4}")
NODE_IPS=("${NODE1_IP}" "${NODE2_IP}" "${NODE3_IP}" "${NODE4_IP}")

ACTUATOR_BIN="${ACTUATOR_BIN:-${BIN_DIR}/mongo-dbactuator}"
WORK_DIR="${TEST_WORK_DIR:-/tmp/mongo-rs-replace-primary-e2e-${PORT1}}"
...
run_atom() {
  ...
  "${ACTUATOR_BIN}" \
    --uid="${uid}" \
    --root_id="e2e-root-rs-replace-primary-${PORT1}" \
    --node_id="${node_id}" \
    --version_id="v1" \
    --atom-job-list="${atom}" \
...
run_os_init() {
  ...
  "${ACTUATOR_BIN}" \
    --uid="e2e-rs-replace-primary-os-init-${PORT1}" \
    --root_id="e2e-root-rs-replace-primary-${PORT1}" \
    --node_id="${node_id}" \
    --version_id="v1" \
...
wait_rs_healthy_auth() {
  ...
  while [[ ${attempt} -lt 60 ]]; do
    local total1 healthy1 total2 healthy2
    total1="$($(mongo_shell_bin) -u "${DBA_USER}" -p "${DBA_PASS}" --authenticationDatabase=admin --host "${host1}" --port "${port1}" --quiet --eval "rs.status().members.length" || true)"
    healthy1="$($(mongo_shell_bin) -u "${DBA_USER}" -p "${DBA_PASS}" --authenticationDatabase=admin --host "${host1}" --port "${port1}" --quiet --eval "rs.status().members.filter(function(m){return m.stateStr==='PRIMARY'||m.stateStr==='SECONDARY';}).length" || true)"
    if [[ "${total1}" =~ ^[0-9]+$ && "${healthy1}" =~ ^[0-9]+$ && "${total1}" -ge 3 && "${healthy1}" -ge 3 ]]; then
      return 0
    fi
    total2="$($(mongo_shell_bin) -u "${DBA_USER}" -p "${DBA_PASS}" --authenticationDatabase=admin --host "${host2}" --port "${port2}" --quiet --eval "rs.status().members.length" || true)"
    healthy2="$($(mongo_shell_bin) -u "${DBA_USER}" -p "${DBA_PASS}" --authenticationDatabase=admin --host "${host2}" --port "${port2}" --quiet --eval "rs.status().members.filter(function(m){return m.stateStr==='PRIMARY'||m.stateStr==='SECONDARY';}).length" || true)"
    if [[ "${total2}" =~ ^[0-9]+$ && "${healthy2}" =~ ^[0-9]+$ && "${total2}" -ge 3 && "${healthy2}" -ge 3 ]]; then
      return 0
    fi
    attempt=$((attempt + 1))
    sleep 2
...
print_rs_status_after_replace() {
  local host="$1"
  local port="$2"
  echo "==> Current rs members after replace (${host}:${port})"
  "$(mongo_shell_bin)" -u "${DBA_USER}" -p "${DBA_PASS}" --authenticationDatabase=admin \
    --host "${host}" --port "${port}" --quiet \
    --eval "rs.status().members.forEach(function(m){print(m.name + ' state=' + m.stateStr + ' health=' + m.health);})"
}
...
cat > "${WORK_DIR}/mongod_replace_primary.json" <<EOF
{
  "ip": "${NODE2_IP}",
  "port": ${PORT2},
  "sourceIP": "${NODE1_IP}",
...
if [[ "${DEINSTALL_ONLY}" == "1" ]]; then
  ...
  for p in "${PORTS[@]}"; do
    run_atom "e2e-rs-replace-primary-deinstall-${p}" "mongo_deinstall" "${WORK_DIR}/mongo_deinstall_${p}.json" "e2e-node-${p}"
  done
  cp -f "${ROOT_DIR}"/logs/mongo_actuator_*.log "${LOG_SNAPSHOT_DIR}/" 2>/dev/null || true
...
for p in "${PORT1}" "${PORT2}" "${PORT3}"; do
  run_atom "e2e-rs-replace-primary-install-${p}" "mongod_install" "${WORK_DIR}/mongod_install_${p}.json" "e2e-node-${p}"
done

echo "==> Init replica set"
run_atom "e2e-rs-replace-primary-init-${PORT1}" "init_replicaset" "${WORK_DIR}/init_replicaset.json" "e2e-node-${PORT1}"

echo "==> Add dba and init scripts"
run_atom "e2e-rs-replace-primary-add-user-${PORT1}" "add_user" "${WORK_DIR}/add_user.json" "e2e-node-${PORT1}"
run_atom "e2e-rs-replace-primary-extra-user-${PORT1}" "mongo_execute_script" \
  "${WORK_DIR}/mongo_execute_script_extra_user.json" "e2e-node-${PORT1}"
run_atom "e2e-rs-replace-primary-init-set-${PORT1}" "mongo_execute_script" \
  "${WORK_DIR}/mongo_execute_script_init_set.json" "e2e-node-${PORT1}"
wait_rs_healthy_auth "${NODE1_IP}" "${PORT1}" "${NODE2_IP}" "${PORT2}"

echo "==> Install target node"
run_atom "e2e-rs-replace-primary-install-${PORT4}" "mongod_install" "${WORK_DIR}/mongod_install_${PORT4}.json" "e2e-node-${PORT4}"

print_rs_status "${NODE2_IP}" "${PORT2}"

echo "==> Replace source(primary) node with target"
run_atom "e2e-rs-replace-primary-run-${PORT2}" "mongod_replace" "${WORK_DIR}/mongod_replace_primary.json" "e2e-node-${PORT2}"

print_rs_status_after_replace "${NODE2_IP}" "${PORT2}"

echo "==> Assert replace result"
members="$($(mongo_shell_bin) -u "${DBA_USER}" -p "${DBA_PASS}" --authenticationDatabase=admin --host "${NODE2_IP}" --port "${PORT2}" --quiet --eval "rs.status().members.map(function(m){return m.name + ':' + m.stateStr;}).join(',')")"
if [[ "${members}" != *"${NODE4_IP}:${PORT4}"* ]]; then
  echo "ERROR: target node not found in rs.status(): ${members}" >&2
  exit 1
fi
if [[ "${members}" == *"${NODE1_IP}:${PORT1}"* ]]; then
  echo "ERROR: source primary node still exists in rs.status(): ${members}" >&2
  exit 1
fi
