package master_slave_heartbeat
...
func (c *Checker) updateHeartbeat() error {
	...
	updateSQL := fmt.Sprintf(
		`UPDATE %s 
  SET master_time=now(),slave_time=sysdate(),delay_sec=timestampdiff(SECOND, now(),sysdate()) 
  ...
  VALUES('%s', @@server_id, now(), sysdate(), timestampdiff(SECOND, now(),sysdate()))`,
	...
	/*
			...
			updateSlave := fmt.Sprintf(
				`UPDATE %s
		  SET beat_sec = timestampdiff(SECOND, master_time, now()) WHERE slave_server_id=@@server_id and master_server_id='%s'`,
				c.heartBeatTable, masterServerId)
