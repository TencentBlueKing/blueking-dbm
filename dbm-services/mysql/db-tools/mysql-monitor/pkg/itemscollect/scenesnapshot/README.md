...
过期快照在**产生新快照时**顺带清理，保留最近 `snapshot_keep_hours` 小时。清理精确到文件，目录被删空后会一并移除。
...
|`snapshot_keep_hours`|48|快照保留小时数|
...
三个按需条件是 **or** 关系，命中任一即采集。判断长查询时会自动跳过 `Sleep` 空闲连接、复制线程(`system user`)、`Binlog Dump`，这些连接的 `Time` 天然很大。

### 快照配置示例

- 示例 1：默认行为，每分钟都留一份现场，保留 48 小时。持续抓快照
```yaml
- name: scene-snapshot
  enable: true
  schedule: '@every 1m'
...
- 示例 2：只在实例有异常时才采集，降低磁盘开销，省下来的空间用于拉长保留期
```yaml
  options:
    snapshot_on_demand: true
    snapshot_keep_hours: 168
    snapshot_long_query_time: 30
    snapshot_threads_running: 50
    snapshot_free_connections: 10
...
- name: scene-snapshot
  enable: false
```
scene-snapshot `enable:false` 之后，**不会触发任何快照以及 query kill 的逻辑**。
...
|`match_query`|**正则，大小写敏感**| 执行的 SQL, 对应 processlist Info 列            |
|`time_gt`|-| `Time` 大于 N 秒                             |
...
- 示例 2：用户没控制好连接池，清理长时间空闲连接，并限制单轮最多 kill 20 个
```yaml
  options:
    query_kill_enable: true
...
- 示例 2：先用 `print` 观察，确认命中范围符合预期再改成 `kill`
```yaml
  options:
    query_kill_enable: true
    query_kill_rules:
...
- `match_query` 是**子串搜索**，不是全串匹配。配 `sleep` 会同时命中 `select * from sleeping_logs`，
  要精确限定请用 `^` / `$` / `\b` 锚定，比如 `'^\s*select\s+sleep\s*\('`. sql 是换行的，多行匹配得用 `(?s)select...`
