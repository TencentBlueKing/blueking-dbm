...
func TestExecutePartitionByMySQLClient(t *testing.T) {
	host := "1.1.1.1"
	port := 20000
	user := "xfan"
	pwd := "xfan"
	...
	err = e.ExecutePartitionByMySQLClient(sqlDB, partitionsql, lock)
	if err != nil {
...
func TestExecuteInitPartition(t *testing.T) {
	e := mysqlutil.ExecuteSqlAtLocal{
	...
	err := e.ExecuteCommand(command, false)
	if err != nil {
