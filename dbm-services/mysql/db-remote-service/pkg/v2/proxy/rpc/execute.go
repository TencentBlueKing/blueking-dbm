...
func (c *ProxyRPCRequest) execute(ctx context.Context, user, password string) (res []ProxyOneAddressRPCResponse, err error) {
	rChan := make(chan ProxyOneAddressRPCResponse, len(c.Addresses))
...
func runOneAddress(ctx context.Context, c *ProxyRPCRequest, addr, user, password string, rChan chan<- ProxyOneAddressRPCResponse) {
	addrRes := ProxyOneAddressRPCResponse{
		Address:    addr,
		CmdResults: []ProxyCmdRPCResponse{},
	}

	if err := config.GlobalSemaphore.Acquire(ctx, 1); err != nil {
		metric.Id(apm.AddressesTotal).Inc("throttled")
		slog.Warn("v2 proxy semaphore acquire failed",
